(function exposeHafizePromptUsage(root) {
  ...
  const api = Object.freeze({ STORAGE_KEY, mount, summarize, usageOf, loadFillAssets }); root.HafizePromptLibraryUsage = api;
  const start = () => mount(root.document, root); if (root.document?.readyState === 'loading') root.document.addEventListener('DOMContentLoaded', start, { once: true }); else start();
