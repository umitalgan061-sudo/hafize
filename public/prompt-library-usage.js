(function exposeHafizePromptUsage(root) {
  ...
  const button = (documentRef, label) => {
    ...
    node.type = 'button'; node.className = 'mini-btn prompt-library-usage-toggle'; node.textContent = label; return node;
  };
  function readItems(rootRef) {
    try { const raw = rootRef.localStorage?.getItem(STORAGE_KEY) || '[]'; const items = JSON.parse(raw); return Array.isArray(items) ? items.filter((item) => item && typeof item === 'object') : []; }
    catch { return []; }
  }
  ...
  function summarize(items) {
    const valid = items.filter((item) => typeof item.id === 'string'); const totalUses = valid.reduce((sum, item) => sum + usageOf(item), 0); const used = valid.filter((item) => usageOf(item) > 0);
    const top = used.slice().sort((a, b) => usageOf(b) - usageOf(a) || dateValue(b) - dateValue(a)).slice(0, MAX_ITEMS); const recent = used.slice().sort((a, b) => dateValue(b) - dateValue(a) || usageOf(b) - usageOf(a)).slice(0, MAX_ITEMS);
  ...
  function mount(documentRef = root.document, rootRef = root) {
    ...
    const heading = documentRef.createElement('div'); heading.className = 'prompt-library-usage-head'; const title = documentRef.createElement('strong'); title.id = 'promptLibraryUsageTitle'; title.textContent = 'Kullanım istatistikleri';
    const toggle = button(documentRef, 'Gizle'); toggle.setAttribute('aria-expanded', 'true'); heading.append(title, toggle);
    ...
    let hidden = false; const onToggle = () => { hidden = !hidden; body.hidden = hidden; toggle.textContent = hidden ? 'Göster' : 'Gizle'; toggle.setAttribute('aria-expanded', String(!hidden)); }; toggle.addEventListener('click', onToggle);
    let timer = 0; const refresh = () => { rootRef.clearTimeout?.(timer); timer = rootRef.setTimeout?.(render, 80) || 0; }; const observer = typeof MutationObserver === 'function' ? new MutationObserver(refresh) : null; const list = documentRef.getElementById('promptLibraryList'); observer?.observe(list || card, { childList: true, subtree: true });
    const onStorage = (event) => { if (event.key === STORAGE_KEY) render(); }; rootRef.addEventListener?.('storage', onStorage); render();
...
(function loadPromptLibraryTrustTools(root) {
  'use strict';
  const assets = [['/prompt-library-import-preview.css', 'style'], ['/prompt-library-import-preview.js', 'script'], ['/prompt-library-diagnostics.js', 'script']];
