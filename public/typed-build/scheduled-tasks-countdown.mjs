...
let timer;
let observer;
export function countdownLabel(timestamp) {
    const target = Date.parse(timestamp || '');
    if (!Number.isFinite(target))
        return '';
    const delta = target - Date.now();
    if (delta <= 0)
        return 'Şimdi çalışması bekleniyor';
    ...
    if (hours > 0)
        return `${hours} saat ${minutes % 60} dk kaldı`;
    return `${Math.max(1, minutes)} dk kaldı`;
}
export function refreshCountdowns(documentRef = root.document) {
