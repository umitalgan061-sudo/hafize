import { hafizeApi } from './hafize-api.mjs';
import { connectivityFromHealth } from './hafize-types.mjs';
...
function ensureElements() {
    ...
    if (!topbar || document.getElementById(STATUS_ID))
        return null;
    ...
    if (themeToggle)
        topbar.insertBefore(status, themeToggle);
    else
...
function connectivityIcon(value) {
...
function render(elements, snapshot) {
...
export function mountHafizeRuntime() {
    ...
    const online = () => { void refresh(); showToast('Bağlantı geri geldi.', 'success'); };
    const offline = () => {
        snapshot = Object.freeze({ ...snapshot, connectivity: 'offline', apiReachable: false, checkedAt: new Date().toISOString() });
        render(elements, snapshot);
    ...
    const keyboard = (event) => {
        if (event.key === 'Escape' && !elements.details.hidden)
            close();
    };
...
if (controller)
    globalThis.dispatchEvent(new CustomEvent('hafize:runtime-ready'));
