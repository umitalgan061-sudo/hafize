...
export function searchPromptLibrary(query) {
...
function mount(documentRef = root.document, rootRef = root) {
    ...
    if (!documentRef || !input || documentRef.getElementById(PALETTE_ID))
        return null;
    ...
    const render = () => {
        ...
        current.forEach((item, index) => {
            ...
            option.append(make(documentRef, 'span', item.title, 'prompt-command-palette-name'), make(documentRef, 'span', `${item.favorite ? '★ ' : ''}${item.tags?.slice(0, 2).join(' · ') || 'etiketsiz'}${variableCount(item) ? ` · ${variableCount(item)} değişken` : ''}`, 'prompt-command-palette-meta'));
    ...
    const onQueryInput = () => { activeIndex = 0; render(); };
    ...
    const onShortcut = (event) => {
        ...
        event.preventDefault();
        if (palette.hidden)
            open(input.selectionStart ?? input.value.length);
        else
            close();
    };
