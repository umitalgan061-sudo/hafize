(function installHafizeMessageActions(root) {
  ...
  const button = (doc, label, action) => {
    ...
    node.type = 'button';
    node.className = 'message-action';
    node.textContent = label;
    node.dataset.messageAction = action;
  ...
  function getSource(article) {
    ...
    return content?.getAttribute?.('data-markdown-source') || content?.textContent || '';
  }
  function showStatus(article, message) {
    ...
    root.clearTimeout?.(status.__timer);
    status.__timer = root.setTimeout?.(() => { status.textContent = ''; }, 2200);
  }
  async function copy(article) {
    const text = getSource(article).slice(0, MESSAGE_LIMIT);
    try {
      if (!root.navigator?.clipboard?.writeText) throw new Error('CLIPBOARD');
      await root.navigator.clipboard.writeText(text);
      showStatus(article, 'Yanıt kopyalandı.');
    } catch {
      showStatus(article, 'Yanıt kopyalanamadı.');
    }
  }
  function toggleRaw(article, toggle) {
    ...
    if (raw) {
      content.replaceChildren(root.document.createTextNode(getSource(article).slice(0, MESSAGE_LIMIT)));
      toggle.textContent = 'Biçimli görünüm';
    } else if (root.HafizeMarkdown) {
      const rendered = root.HafizeMarkdown.render(root.document, getSource(article));
      content.replaceChildren(...rendered.childNodes);
      toggle.textContent = 'Ham metin';
    }
  ...
  function toggleLong(article, toggle) {
    ...
    toggle.textContent = collapsed ? 'Yanıtı daralt' : 'Yanıtı aç';
  ...
  function enhance(article) {
    ...
    actions.append(button(root.document, 'Kopyala', 'copy'));
    const raw = button(root.document, 'Ham metin', 'raw');
    raw.setAttribute('aria-pressed', 'false');
    actions.append(raw);
    if (getSource(article).length > COLLAPSE_CHARS) {
      const collapse = button(root.document, 'Yanıtı daralt', 'collapse');
  ...
  function scan() {
    root.document?.querySelectorAll?.('#messages .message.assistant').forEach(enhance);
  }
  function onClick(event) {
    const target = event.target?.closest?.(`[${ACTION}]`);
    if (!target) return;
    const article = getArticle(target);
    const action = target.dataset.messageAction;
    if (!article) return;
    if (action === 'copy') copy(article);
    else if (action === 'raw') toggleRaw(article, target);
    else if (action === 'collapse') toggleLong(article, target);
  }
  function install() {
  ...
  let retry = 0;
  function wait() { if (install()) return; if (retry >= 12) return; retry += 1; root.setTimeout?.(wait, 50 * retry); }
  wait();
