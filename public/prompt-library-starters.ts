export interface PromptStarterRecord {
...
interface PromptLibraryCore {
  readonly loadItems?: (storage: Storage) => PromptStarterItem[];
  readonly normalizeItem?: (value: PromptStarterItem) => PromptStarterItem | null;
  readonly saveItems?: (storage: Storage, items: readonly PromptStarterItem[]) => boolean;
}

export interface PromptStarterItem {
  readonly id: string;
  readonly title: string;
  readonly body: string;
  readonly tags?: readonly string[];
...
interface StarterRoot extends Window {
  HafizePromptLibrary?: PromptLibraryCore;
  HafizePromptLibraryStarters?: Readonly<{ STARTERS: readonly PromptStarterRecord[]; seed: (options?: { force?: boolean }) => boolean }>;
}
...
export function seed({ force = false }: { force?: boolean } = {}): boolean {
  ...
  const additions = STARTERS
    .filter((starter) => !existingTitles.has(starter.title))
    .map((starter, index) => core.normalizeItem?.({
      id: `starter-${index + 1}-${Date.now()}`,
      title: starter.title,
      body: starter.body,
      tags: starter.tags,
      favorite: false,
    ...
    .filter((item): item is PromptStarterItem => Boolean(item));
