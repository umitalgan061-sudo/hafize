...
interface CalendarCell { day: number; month: number; year: number; outside: boolean; selected: boolean }
interface CalendarTarget { year: number; month: number; day: number }
interface ShellController { getTheme(): Theme; renderCalendar(options?: { focusSelected?: boolean }): void; sidebarDisclosure: unknown; destroy(): void }
(function exposeHafizeUiShell(root, factory) {
  ...
  function resolveTheme(stored: unknown, prefersDark: boolean): Theme {
    if (stored === 'light' || stored === 'dark') return stored;
    return prefersDark ? 'dark' : 'light';
  }

  function createMonthCells(year: number, month: number, selectedDay: number): CalendarCell[] {
  ...
  function moveCalendarDate(year: number, month: number, day: number, key: string): CalendarTarget | null {
  ...
  function installSidebarDisclosure(documentRef: Document) {
  ...
  function installChatAccessibility(documentRef: Document): boolean {
  ...
  function install(documentRef: Document, root: typeof globalThis): ShellController | null {
