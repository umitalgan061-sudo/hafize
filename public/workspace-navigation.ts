type Workspace = 'chat' | 'tasks' | 'connections';
interface WorkspaceController { mount(): boolean; destroy(): boolean; setWorkspace(value: Workspace, options?: { focus?: boolean; emit?: boolean }): boolean; getWorkspace(): Workspace; syncCards(): void }
(function exposeHafizeWorkspaceNavigation(root, factory) {
  ...
  function allowedCardIds(workspace: Workspace): readonly string[] {
    const key = normalizeWorkspace(workspace);
    return CARD_IDS[key] || Object.freeze([]);
  }

  function isWorkspaceCard(node: Element | null, workspace: Workspace): boolean {
    const id = typeof node?.id === 'string' ? node.id : '';
    return Boolean(id && allowedCardIds(workspace).includes(id));
  }

  function workspaceCopy(workspace: Workspace) {
    const key = normalizeWorkspace(workspace);
    return COPY[key] || null;
  }

  function resolveNavigation(documentRef: Document) {
  ...
  function snapshotAttribute(node: Element | null, name: string) {
    const value = node?.getAttribute?.(name);
    return Object.freeze({ present: value !== null && value !== undefined, value });
  }

  function restoreAttribute(node: Element | null, name: string, snapshot: any): void {
    if (!node || !snapshot) return;
    if (snapshot.present) node.setAttribute?.(name, snapshot.value ?? '');
    else node.removeAttribute?.(name);
  }

  function ensureStyle(documentRef: Document) {
  ...
  function createIntro(documentRef: Document) {
  ...
  function dispatchWorkspaceEvent(rootRef: typeof globalThis, workspace: Workspace): boolean {
  ...
  function createController({
  ...
  }: { documentRef?: Document; rootRef?: typeof globalThis; MutationObserverImpl?: typeof MutationObserver } = {}): WorkspaceController {
    ...
    function snapshotCard(node: HTMLElement | null): void {
    ...
    function renderHost({ focus = false }: { focus?: boolean } = {}): void {
    ...
    function setWorkspace(value: Workspace, { focus = false, emit = true }: { focus?: boolean; emit?: boolean } = {}): boolean {
    ...
    function onNav(workspace: Workspace): EventListener {
      return (event) => {
  ...
  function mount(documentRef: Document, rootRef: typeof globalThis): WorkspaceController | null {
    try {
