...
function apiPath(value:RequestInfo|URL):boolean{try{const u=new URL(typeof value==='string'?value:value instanceof URL?value.toString():value.url,
  location.href);return u.origin===location.origin&&u.pathname.startsWith('/api/');}catch{return false;}}
...
function openModal():AuthModal{
 ...
 style.textContent=[
   ...
   '.hafize-auth-card{width:min(420px,100%);box-sizing:border-box;border:1px solid rgba(120,125,130,.24);border-radius:24px;padding:26px;background:var(--panel,#fff);box-shadow:0 24px 80px rgba(0,0,0,.22)}',
   '.hafize-auth-card h2{margin:0 0 8px}',
   '.hafize-auth-card p{margin:0 0 18px;opacity:.72;line-height:1.5}',
   '.hafize-auth-card label{display:block;font-size:13px;font-weight:700;margin-bottom:8px}',
   '.hafize-auth-card input{width:100%;box-sizing:border-box;border:1px solid rgba(120,125,130,.35);border-radius:14px;padding:12px 14px;background:transparent;color:inherit}',
   '.hafize-auth-card button{width:100%;margin-top:14px;border:0;border-radius:14px;padding:12px;font-weight:700;cursor:pointer}',
   '.hafize-auth-error{min-height:20px;margin-top:10px;font-size:13px;color:#a52323}'
 ].join('');
...
async function refresh():Promise<AuthState>{
 ...
 state.loading=(async()=>{const response=await rawFetch('/api/auth/session',{credentials:'same-origin',headers:{Accept:'application/json'},cache:'no-store'}); if(!response.ok)throw new Error('AUTH_SESSION_FAILED');
   const payload=await response.json(); state.required=Boolean(payload?.required); state.authenticated=Boolean(payload?.authenticated); state.csrf=typeof payload?.csrf==='string'?payload.csrf:''; return state; })();
...
async function hafizeFetch(input:RequestInfo|URL, init:RequestInit={}){if(!apiPath(input)||authPath(input))return rawFetch(input,init); await ensureAuth(); const next:RequestInit={...init,credentials:'same-origin'};
  const method=String(init.method||(input instanceof Request?input.method:'GET')).toUpperCase();
  if(!['GET','HEAD'].includes(method)){const headers=new Headers(next.headers||{});if(state.csrf)headers.set('X-Hafize-CSRF',state.csrf);next.headers=headers;}let response=await rawFetch(input,next);
  if(response.status!==401)return response; state.authenticated=false; state.csrf=''; await ensureAuth();
  if(!['GET','HEAD'].includes(method)){const headers=new Headers(next.headers||{});if(state.csrf)headers.set('X-Hafize-CSRF',state.csrf);next.headers=headers;}return rawFetch(input,next); }
