import { Disposer, on, query, safeJsonParse, boundedText, writeStorage } from './browser-platform.ts';
...
export function normalizeSettings(value: unknown): Settings { const data = value && typeof value === 'object' ? value as { enabled?: unknown; maxItems?: unknown } : {}; const maxItems = Number(data.maxItems); return { enabled: data.enabled !== false, maxItems: COMPOSER_HISTORY_RETENTION.includes(maxItems) ? maxItems : COMPOSER_HISTORY_LIMITS.maxItems }; }
export function loadSettings(storage: Storage = localStorage): Settings { try { return normalizeSettings(safeJsonParse<unknown>(storage.getItem(COMPOSER_HISTORY_SETTINGS_KEY), {})); } catch { return { enabled: true, maxItems: COMPOSER_HISTORY_LIMITS.maxItems }; } }
...
export function saveSettings(next: Partial<Settings>, storage: Storage = localStorage): boolean { const settings = normalizeSettings({ ...loadSettings(storage), ...next }); try { storage.setItem(COMPOSER_HISTORY_SETTINGS_KEY, JSON.stringify(settings)); if (!settings.enabled || settings.maxItems === 0) storage.removeItem(COMPOSER_HISTORY_STORAGE_KEY); return true; } catch { return false; } }

export function mountComposerHistory(documentRef: Document = document, rootRef: Window = window): Readonly<{ getItems: () => string[]; add: (value: string) => boolean; navigate: (direction: number) => string | null; clear: () => boolean; getCursor: () => number; getSettings: () => Settings; setSettings: (next: Partial<Settings>) => boolean; destroy: () => void }> | null {
  ...
  const navigate = (direction: number) => { if (!items.length) return null; rememberDraft(); navigating = true; cursor = cursor < 0 ? direction < 0 ? 0 : items.length - 1 : Math.max(0, Math.min(items.length - 1, cursor + direction)); const value = items[cursor]; insert(value); navigating = false; return value; };
  ...
  const onSettings = (event: Event) => { if ((event as CustomEvent<{ clear?: boolean }>).detail?.clear) { items = []; save(items, rootRef.localStorage); } refresh(); };
  on(input, 'input', onInput, undefined, disposer); on(input, 'keydown', onKeydown, undefined, disposer); on(input, 'compositionstart', onComposition, undefined, disposer); on(input, 'compositionend', onComposition, undefined, disposer); on(form, 'submit', onSubmit, undefined, disposer); on(rootRef, 'storage', onStorage, undefined, disposer); on(rootRef, 'hafize:composer-history-settings-changed', onSettings, undefined, disposer);
  rootRef.HafizeComposerHistoryController = Object.freeze({ getItems: () => items.slice(), add, navigate, clear: () => { items = []; return save(items, rootRef.localStorage); }, getCursor: () => cursor, getSettings: () => ({ ...loadSettings(rootRef.localStorage) }), setSettings: (next: Partial<Settings>) => { const ok = saveSettings(next, rootRef.localStorage); refresh(); return ok; }, destroy: () => disposer.flush() });
  return rootRef.HafizeComposerHistoryController;
}
