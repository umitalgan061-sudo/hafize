...
function attachmentList(composer: ComposerWithAttachments): Attachment[] { if (!composer._hafizeAttachments) composer._hafizeAttachments = []; return composer._hafizeAttachments; }
...
function renderAttachmentStrip(composer: ComposerWithAttachments, documentRef: Document): void {
  ...
  files.forEach((file, index) => {
    ...
    remove.addEventListener('click', () => {
      const input = query<HTMLTextAreaElement>(documentRef, '#messageInput'); const at = input?.value.lastIndexOf(file.block) ?? -1;
...
export async function addFile(file: File, context: { composer: ComposerWithAttachments; input: HTMLTextAreaElement; documentRef?: Document }): Promise<boolean> {
  ...
  try {
    ...
    const before = input.value; const nextValue = `${before}${composeAttachment(file, source)}`.slice(0, input.maxLength || 12000); const appended = nextValue.slice(before.length);
...
function retryPrompt(article: HTMLElement, documentRef: Document): string {
  let cursor = article.previousElementSibling as HTMLElement | null;
  while (cursor) { if (cursor.classList.contains('user')) return cursor.querySelector<HTMLElement>('.content')?.textContent?.trim() || ''; cursor = cursor.previousElementSibling as HTMLElement | null; }
  return '';
}
...
function enhanceArticle(article: HTMLElement, input: HTMLTextAreaElement, composer: ComposerWithAttachments, documentRef: Document): void {
  ...
  const actions = text<HTMLDivElement>(documentRef, 'div', '', 'message-actions'); const copy = text<HTMLButtonElement>(documentRef, 'button', 'Kopyala', 'message-action'); copy.type = 'button'; copy.setAttribute('aria-label', 'Hafize yanıtını kopyala');
  ...
  retry.addEventListener('click', () => { if (input.disabled) return announce(documentRef, 'Yanıt sürerken yeniden denenemez.'); const prompt = retryPrompt(article, documentRef); if (!prompt) return announce(documentRef, 'Tekrar gönderilecek kullanıcı mesajı bulunamadı.'); input.value = prompt; input.dispatchEvent(new Event('input', { bubbles: true })); input.focus(); composer.requestSubmit(); });
...
export function mountComposerFeatures(documentRef: Document = document, _rootRef: Window = window): Readonly<{ destroy: () => void }> | null {
  ...
  observer.observe(messages, { childList: true }); disposer.add(() => observer.disconnect());
  ...
  return Object.freeze({ destroy: () => { disposer.flush(); composer._hafizeAttachments = []; composer.querySelector('.attachment-strip')?.remove(); } });
}
