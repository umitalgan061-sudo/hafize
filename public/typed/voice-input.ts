...
type RecognitionInstance=SpeechRecognition&{onstart:(()=>void)|null;onresult:((event:RecognitionEventLike)=>void)|null;onerror:((event:RecognitionEventLike)=>void)|null;onend:(()=>void)|null};
type RecognitionCtor=new()=>RecognitionInstance;
const DEFAULT_LANGUAGE='tr-TR',TOAST_DURATION_MS=4200;
export function getSpeechRecognitionConstructor(root:typeof globalThis):RecognitionCtor|null{const candidate=(root as typeof globalThis & {SpeechRecognition?:RecognitionCtor;webkitSpeechRecognition?:RecognitionCtor});return candidate.SpeechRecognition||candidate.webkitSpeechRecognition||null;}
export function normalizeTranscript(value:unknown):string{return typeof value==='string'?value.replace(/\s+/g,' ').trim():'';}
export function mergeTranscript(prefix:unknown,transcript:unknown,maxLength=12000):string{const before=typeof prefix==='string'?prefix.replace(/\s+$/g,''):'';const spoken=normalizeTranscript(transcript);const joined=before&&spoken?`${before} ${spoken}`:before||spoken;const limit=Number.isInteger(maxLength)&&maxLength>0?maxLength:12000;return joined.slice(0,limit);}
export function readRecognitionText(event:RecognitionEventLike):string{if(!event?.results)return'';const chunks:string[]=[];const start=Number.isInteger(event.resultIndex)?event.resultIndex:0;for(let i=start;i<event.results.length;i++){const alt=event.results[i]?.[0];if(typeof alt?.transcript==='string')chunks.push(alt.transcript);}return normalizeTranscript(chunks.join(' '));}
...
export function installVoiceInput(documentRef:Document,root:typeof globalThis):VoiceInputController|null{
 ...
 let timeoutId:ReturnType<typeof setTimeout>|null=null;const announce=(message:string)=>{if(!toast||!message)return;toast.textContent=message;toast.classList.remove('hidden');if(timeoutId)clearTimeout(timeoutId);timeoutId=setTimeout(()=>toast.classList.add('hidden'),TOAST_DURATION_MS);};
 const render=()=>{micButton.disabled=input.disabled;micButton.setAttribute('aria-pressed',String(listening));micButton.setAttribute('aria-label',!Recognition?'Sesli giriş bu tarayıcıda desteklenmiyor':listening?'Sesli girişi durdur':'Sesli giriş');micButton.textContent=listening?'●':'◉';};
 const stop=()=>{if(!recognition||!listening)return;try{recognition.stop();}catch{listening=false;recognition=null;render();}};
 const start=()=>{if(!Recognition||input.disabled||listening)return;prefix=input.value||'';recognition=new Recognition();recognition.lang=documentRef.documentElement.lang||root.navigator.language||DEFAULT_LANGUAGE;recognition.interimResults=true;recognition.continuous=false;recognition.maxAlternatives=1;
 ...
 const observer=new MutationObserver(()=>{if(input.disabled&&listening)stop();render()});observer.observe(input,{attributes:true,attributeFilter:['disabled']});render();
 return Object.freeze({isSupported:Boolean(Recognition),isListening:()=>listening,start,stop,destroy(){observer.disconnect();if(timeoutId)clearTimeout(timeoutId);try{recognition?.abort();}catch{}recognition=null;listening=false;micButton.removeEventListener('click',onClick,true);documentRef.removeEventListener('visibilitychange',onVisibility);}});
}
