type PromptStorage = Storage | { getItem(key: string): string | null; setItem(key: string, value: string): void };
type PromptRecord = { id: string; title: string; body: string; tags: string[]; variables: string[]; favorite: boolean; useCount: number; createdAt: string; updatedAt: string };
type PromptState = { query: string; tag: string; favoriteOnly: boolean; sort: string };
type PromptLibraryOptions = { storage?: PromptStorage };
(function exposeHafizePromptLibrary(root, factory) {
  ...
  function normalizeItem(input: Record<string, unknown>): PromptRecord | null {
  ...
  function normalizeCollection(value: unknown[]): PromptRecord[] {
  ...
  function safeState(value: Record<string, unknown>): PromptState {
  ...
  function filterItems(items: PromptRecord[], state: PromptState): PromptRecord[] { return sortItems(items.filter((item) => matches(item, state)), state.sort); }
  ...
  function extractVariables(body: string): string[] { return safeVariables(String(body || ''), null); }
  function replaceVariables(body: string, values: Record<string, unknown>): string {
  ...
  function mount(documentRef: Document = globalThis.document, rootRef: typeof globalThis = globalThis, options: PromptLibraryOptions = {}) {
