(function installScheduledTasksWorkspace(root) {
  ...
  function buildCreateForm() {
    ...
    const heading = make('div', editingId ? 'Görevi düzenle' : 'Yeni görev planla', 'scheduled-tasks-section-title');
    ...
    const actions = make('div', undefined, 'scheduled-tasks-form-actions');
    const submit = button(editingId ? 'Değişiklikleri kaydet' : 'Görevi planla', 'save', 'soft-btn'); submit.classList.add('primary');
  ...
  function build() {
    panel = make('section', undefined, 'scheduled-tasks-overlay'); panel.id = PANEL_ID; panel.hidden = true; panel.setAttribute('role', 'dialog'); panel.setAttribute('aria-modal', 'true'); panel.setAttribute('aria-labelledby', 'scheduledTasksTitle');
    const shell = make('div', undefined, 'scheduled-tasks-shell'); const head = make('div', undefined, 'scheduled-tasks-head');
    const title = make('strong', 'Zamanlanmış görevler', 'scheduled-tasks-title'); title.id = 'scheduledTasksTitle'; head.append(title, button('Yenile', 'refresh', 'mini-btn'), button('Kapat', 'close', 'mini-btn'));
    const formHost = make('div', undefined, 'scheduled-tasks-form-host'); shell.append(head, formHost);
    const statusNode = make('div', '', 'scheduled-tasks-status'); statusNode.setAttribute('role', 'status'); statusNode.setAttribute('aria-live', 'polite'); shell.append(statusNode);
    const listSection = make('section', undefined, 'scheduled-tasks-list-section'); listSection.setAttribute('aria-label', 'Planlanmış görevler listesi'); listSection.append(make('div', 'Planlanan görevler', 'scheduled-tasks-section-title'));
    const filter = make('div', undefined, 'scheduled-tasks-filter'); const filterSelect = doc().createElement('select'); filterSelect.setAttribute('aria-label', 'Görev durumuna göre filtrele'); [['all','Tümü'],['scheduled','Planlandı'],['running','Çalışıyor'],['completed','Tamamlandı'],['failed','Başarısız'],['cancelled','İptal edildi']].forEach(([value,label]) => { const option = make('option', label); option.value = value; filterSelect.append(option); }); const filterInfo = make('span', '', 'scheduled-tasks-filter-info'); filter.append(filterSelect, filterInfo);
    const list = make('div', undefined, 'scheduled-tasks-list'); list.setAttribute('role','list'); listSection.append(filter, list); shell.append(listSection); panel.append(shell); doc().body.append(panel);

    formHost.replaceChildren(buildCreateForm());
    filterSelect.addEventListener('change', () => { selectedStatus = filterSelect.value; applyFilter(list, filterInfo); });
    panel.addEventListener('click', onClick); panel.addEventListener('keydown', (event) => { if (event.key === 'Escape') { event.preventDefault(); close(); } }); panel.addEventListener('click', (event) => { if (event.target === panel) close(); });
    wireForm();
  }

  function currentEditEntry() { const list = panel?.querySelector('.scheduled-tasks-list'); const row = [...(list?.children || [])].find((node) => node.dataset?.scheduleId === editingId); return row ? row._scheduleEntry : null; }
  function fillForm(entry) {
    const form = panel?.querySelector('.scheduled-tasks-create'); if (!form) return;
    form.querySelector('#scheduledTaskAgent').value = entry.agentId; form.querySelector('textarea').value = entry.task; const when = form.querySelector('input[type="datetime-local"]'); when.value = localInputFromIso(entry.runAt); when.min = localDateTimeValue(); form.querySelector('.scheduled-tasks-form-grid select:last-of-type').value = String(entry.maxAttempts);
  }
  function wireForm() {
    const form = panel?.querySelector('.scheduled-tasks-create'); if (!form) return;
    form.addEventListener('submit', async (event) => {
      event.preventDefault();
      const agent = form.querySelector('#scheduledTaskAgent'); const task = form.querySelector('textarea'); const when = form.querySelector('input[type="datetime-local"]'); const attempts = form.querySelector('.scheduled-tasks-form-grid select:last-of-type'); const runAt = isoFromLocal(when.value);
      const taskText = task.value.trim(); if (!agent.value) return status('Geçerli bir ajan seçmelisin.', 'error'); if (!taskText) return status('Görev metni boş olamaz.', 'error'); if (!runAt || Date.parse(runAt) <= Date.now()) return status('Çalıştırma zamanı gelecekte olmalı.', 'error');
      const submit = form.querySelector('[data-task-action="save"]'); submit.disabled = true;
      try {
        if (editingId) { await request(`${API_PATH}/${encodeURIComponent(editingId)}`, { method: 'PATCH', body: JSON.stringify({ agentId: agent.value, task: taskText, runAt, maxAttempts: Number(attempts.value) }) }); status('Görev değişiklikleri kaydedildi.', 'success'); editingId = null; panel.querySelector('.scheduled-tasks-form-host').replaceChildren(buildCreateForm()); wireForm(); }
        else { await request(API_PATH, { method: 'POST', body: JSON.stringify({ agentId: agent.value, task: taskText, runAt, maxAttempts: Number(attempts.value) }) }); task.value = ''; when.value = localDateTimeValue(); status('Görev planlandı.', 'success'); }
        await refresh();
      } catch (error) {
        status(error.status === 401 ? 'Oturum açılması gerekiyor.' : error.message === 'SCHEDULE_CAPACITY_REACHED' ? 'Görev kapasitesi dolu.' : error.message === 'SCHEDULE_NOT_EDITABLE' ? 'Bu görev artık düzenlenemez.' : error.message === 'SCHEDULE_TASK_CREDENTIAL_NOT_ALLOWED' ? 'Görev metninde düz metin credential bulunamaz.' : 'Görev kaydedilemedi.', 'error');
  ...
  function row(entry) {
    ...
    const actions = make('div', undefined, 'scheduled-task-actions'); if (entry.status === 'scheduled') actions.append(button('Düzenle','edit','mini-btn'), button('İptal et','cancel','mini-btn'), button('Tekrar planla','duplicate','mini-btn'));
  ...
  function applyFilter(list, info) { let visible = 0; [...list.children].forEach((node) => { if (!node.classList.contains('scheduled-task-row')) return; const show = selectedStatus === 'all' || node.dataset.status === selectedStatus; node.hidden = !show; if (show) visible += 1; }); info.textContent = `${visible} görev gösteriliyor.`; }
  ...
  async function duplicateTask(entry) {
    if (!entry || entry.status !== 'scheduled') return;
    const runAt = localDateTimeValue(10); try { await request(API_PATH, { method:'POST', body:JSON.stringify({ agentId:entry.agentId, task:entry.task, runAt:isoFromLocal(runAt), maxAttempts:entry.maxAttempts }) }); status('Görev yeni zamanla tekrar planlandı.','success'); await refresh(); }
    catch (error) { status(error.message === 'SCHEDULE_CAPACITY_REACHED' ? 'Görev kapasitesi dolu.' : 'Görev tekrar planlanamadı.','error'); }
  }
  function onClick(event) {
    const target = event.target?.closest?.('[data-task-action]'); if (!target) return; const action = target.dataset.taskAction; if (action === 'close') return close(); if (action === 'refresh') return refresh(); if (action === 'edit-cancel') { editingId = null; panel.querySelector('.scheduled-tasks-form-host').replaceChildren(buildCreateForm()); wireForm(); status('Düzenleme iptal edildi.','info'); return; } if (action === 'cancel') return cancelTask(target.closest('.scheduled-task-row')?.dataset.scheduleId); if (action === 'edit') return startEdit(target.closest('.scheduled-task-row')?._scheduleEntry); if (action === 'duplicate') return duplicateTask(target.closest('.scheduled-task-row')?._scheduleEntry); if (action === 'trace') return status(`Trace ID: ${target.title}`,'info');
  }
  ...
  root.ScheduledTasksWorkspace = Object.freeze({ open, close, refresh, request, startEdit });
