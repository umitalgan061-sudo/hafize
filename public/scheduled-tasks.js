(function installScheduledTasksWorkspace(root) {
  ...
  function build() {
    ...
    const shell = make('div', undefined, 'scheduled-tasks-shell'); const head = make('div', undefined, 'scheduled-tasks-head'); const title = make('strong', 'Zamanlanmış görevler', 'scheduled-tasks-title'); title.id = 'scheduledTasksTitle'; head.append(title, button('Yenile', 'refresh', 'mini-btn'), button('Kapat', 'close', 'mini-btn'));
    const formHost = make('div', undefined, 'scheduled-tasks-form-host'); shell.append(head, formHost); const statusNode = make('div', '', 'scheduled-tasks-status'); statusNode.setAttribute('role', 'status'); statusNode.setAttribute('aria-live', 'polite'); shell.append(statusNode);
    ...
    formHost.replaceChildren(buildCreateForm()); filterSelect.addEventListener('change', () => { selectedStatus = filterSelect.value; applyFilter(list, filterInfo); });
    panel.addEventListener('click', onClick); panel.addEventListener('keydown', (event) => { if (event.key === 'Escape') { event.preventDefault(); close(); } }); panel.addEventListener('click', (event) => { if (event.target === panel) close(); }); wireForm();
  }
  ...
  function wireForm() {
    ...
    form.addEventListener('submit', async (event) => {
      event.preventDefault(); const agent = form.querySelector('#scheduledTaskAgent'); const task = form.querySelector('textarea'); const when = form.querySelector('input[type="datetime-local"]'); const attempts = form.querySelector('.scheduled-tasks-form-grid select:last-of-type'); const runAt = isoFromLocal(when.value); const taskText = task.value.trim();
      ...
      try {
        ...
        await refresh();
      } catch (error) { status(error.status === 401 ? 'Oturum açılması gerekiyor.' : error.message === 'SCHEDULE_CAPACITY_REACHED' ? 'Görev kapasitesi dolu.' : error.message === 'SCHEDULE_NOT_EDITABLE' ? 'Bu görev artık düzenlenemez.' : error.message === 'SCHEDULE_TASK_CREDENTIAL_NOT_ALLOWED' ? 'Görev metninde düz metin credential bulunamaz.' : 'Görev kaydedilemedi.', 'error'); }
  ...
  function onClick(event) {
    ...
    if (action === 'edit-cancel') { editingId = null; panel.querySelector('.scheduled-tasks-form-host').replaceChildren(buildCreateForm()); wireForm(); status('Düzenleme iptal edildi.','info'); return; }
    if (action === 'cancel') return cancelTask(target.closest('.scheduled-task-row')?.dataset.scheduleId);
    if (action === 'edit') return startEdit(target.closest('.scheduled-task-row')?._scheduleEntry);
