(function exposeHafizePromptRevisions(root) {
  ...
  function readAll() {
    try { const value = JSON.parse(storage()?.getItem?.(STORAGE_KEY) || '{}'); return value && typeof value === 'object' && !Array.isArray(value) ? value : {}; } catch { return {}; }
  }
  ...
  function writeAll(value) { try { storage()?.setItem?.(STORAGE_KEY, JSON.stringify(normalizeStore(value))); return true; } catch { return false; } }
  function list(promptId) { const id = clamp(promptId, 120).trim(); if (!id) return []; const store = normalizeStore(readAll()); return (store[id] || []).slice().sort((a, b) => b.savedAt.localeCompare(a.savedAt)); }

  function capture(item, reason = 'before-edit') {
    const normalized = normalizeRevision({ ...item, id: makeId(), savedAt: now(), reason });
    if (!normalized) return false;
    const store = normalizeStore(readAll()); const current = store[normalized.promptId] || [];
    if (current.some((revision) => revision.title === normalized.title && revision.body === normalized.body && JSON.stringify(revision.tags) === JSON.stringify(normalized.tags))) return false;
  ...
  function remove(promptId, revisionId) {
    ...
    const store = normalizeStore(readAll()); const next = (store[id] || []).filter((revision) => revision.id !== rid);
    if (next.length) store[id] = next; else delete store[id]; return writeAll(store);
  }

  function clear(promptId) { const id = clamp(promptId, 120).trim(); if (!id) return false; const store = normalizeStore(readAll()); delete store[id]; return writeAll(store); }

  function exportPrompt(promptId) {
    const id = clamp(promptId, 120).trim(); const revisions = list(id);
  ...
  function dispatchCoreRefresh(api, items) {
    try {
      if (typeof root.StorageEvent === 'function') root.dispatchEvent(new root.StorageEvent('storage', { key: api.STORAGE_KEY, newValue: JSON.stringify(items), storageArea: storage() }));
  ...
  function insertRevisionIntoPrompt(revision) {
    ...
    dispatchCoreRefresh(api, nextItems); return { ok: true };
  }

  function button(doc, label, action) {
    const node = doc.createElement('button'); node.type = 'button'; node.className = 'soft-btn prompt-revision-action'; node.textContent = label;
    node.dataset.promptRevisionAction = action; node.setAttribute('aria-label', label); return node;
  }

  function mount(documentRef = root.document, rootRef = root) {
    const card = documentRef?.getElementById?.(CARD_ID); if (!documentRef || !card || card.dataset.revisionReady === 'true') return null;
    card.dataset.revisionReady = 'true';
    const panel = documentRef.createElement('section'); panel.id = 'promptLibraryRevisionPanel'; panel.hidden = true; panel.className = 'prompt-revision-panel'; panel.setAttribute('role', 'dialog'); panel.setAttribute('aria-modal', 'true'); panel.setAttribute('aria-labelledby', 'promptRevisionTitle'); panel.setAttribute('aria-describedby', 'promptRevisionDescription');
    ...
    const close = button(documentRef, 'Kapat', 'close'); close.className = 'mini-btn prompt-revision-action'; head.append(title, close);
    ...
    const exportButton = button(documentRef, 'Geçmişi dışa aktar', 'export'); const clearButton = button(documentRef, 'Geçmişi temizle', 'clear'); actions.append(exportButton, clearButton);
    shell.append(head, description, currentTitle, currentPreview, listNode, actions, status); panel.append(shell); card.append(panel);

    let activePrompt = null; let lastFocus = null; let observer = null;
    ...
    function render() {
      ...
      currentTitle.textContent = `Mevcut: ${activePrompt.title}`; currentPreview.textContent = activePrompt.body.slice(0, MAX_BODY);
      const revisions = list(activePrompt.id); description.textContent = `${revisions.length} kayıtlı sürüm. Geri yükleme favori ve kullanım sayısını korur.`; listNode.replaceChildren();
    ...
    function compare(revision) { const current = activePrompt?.body || ''; if (current === revision.body) return report('Değişiklik yok: mevcut içerik bu sürümle aynı.'); report(`Mevcut ${current.length} karakter · sürüm ${revision.body.length} karakter. Önizleme: ${revision.body.slice(0, 150)}`); }
    function handleClick(event) {
      ...
      if (action === 'restore') { if (!rootRef.confirm?.(`“${revision.title}” sürümü geri yüklensin mi?`)) return; capture({ ...activePrompt }, 'manual'); const result = insertRevisionIntoPrompt(revision); if (!result.ok) return report('Sürüm geri yüklenemedi.'); activePrompt = core().loadItems(storage()).find((item) => item.id === activePrompt.id) || activePrompt; render(); report('Sürüm geri yüklendi.'); }
    }
    function open(prompt) { const current = prompt && core()?.loadItems?.(storage())?.find?.((item) => item.id === prompt.id); if (!current) return; lastFocus = documentRef.activeElement; activePrompt = current; panel.hidden = false; render(); close.focus(); }
    function onKeydown(event) { if (!panel.hidden && event.key === 'Escape') { event.preventDefault(); closePanel(); return; } if (panel.hidden || event.key !== 'Tab') return; const focusables = [...panel.querySelectorAll('button')].filter((node) => !node.disabled && !node.hidden); if (!focusables.length) return; const first = focusables[0]; const last = focusables[focusables.length - 1]; if (event.shiftKey && documentRef.activeElement === first) { event.preventDefault(); last.focus(); } else if (!event.shiftKey && documentRef.activeElement === last) { event.preventDefault(); first.focus(); } }
    function onEdit(event) { const target = event.target?.closest?.('.prompt-item-actions button'); if (!target || target.textContent?.trim() !== 'Düzenle') return; const id = target.closest('.prompt-item')?.dataset?.promptId; const item = id ? core()?.loadItems?.(storage())?.find?.((candidate) => candidate.id === id) : null; if (item) capture(item); }
    function enhanceRows() { card.querySelectorAll('.prompt-item').forEach((row) => { const id = row.dataset.promptId; const actions = row.querySelector('.prompt-item-actions'); if (!id || !actions || actions.querySelector('[data-prompt-revision-action="history"]')) return; const history = button(documentRef, 'Geçmiş', 'history'); history.dataset.promptRevisionId = id; actions.append(history); }); }
    function onRowAction(event) { const target = event.target?.closest?.('[data-prompt-revision-action="history"]'); if (!target) return; event.preventDefault(); event.stopImmediatePropagation(); const id = target.dataset.promptRevisionId; const item = id ? core()?.loadItems?.(storage())?.find?.((candidate) => candidate.id === id) : null; if (item) open(item); }
    observer = typeof MutationObserver === 'function' ? new MutationObserver(enhanceRows) : null; observer?.observe(card, { childList: true, subtree: true });
    enhanceRows(); card.addEventListener('click', onEdit, true); card.addEventListener('click', onRowAction, true); panel.addEventListener('click', handleClick); documentRef.addEventListener('keydown', onKeydown);
    rootRef.addEventListener?.('storage', (event) => { if (event.key === STORAGE_KEY && activePrompt) render(); });
    return Object.freeze({ mounted: true, open, close: closePanel, list, capture, normalizeRevision, exportPrompt, destroy: () => { observer?.disconnect(); card.removeEventListener('click', onEdit, true); card.removeEventListener('click', onRowAction, true); panel.removeEventListener('click', handleClick); documentRef.removeEventListener('keydown', onKeydown); closePanel(); panel.remove(); delete card.dataset.revisionReady; } });
  }
