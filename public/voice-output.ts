...
const normalizeLimit = (value: unknown, max: number): string => String(value ?? '').slice(0, max);
...
export function splitSpeechText(value: unknown, maxLength = MAX_CHUNK_LENGTH): string[] {
  ...
  for (const sentence of sentences) { const clean = sentence.trim(); if (!clean) continue; const candidate = current ? `${current} ${clean}` : clean; if (candidate.length <= limit) { current = candidate; continue; } if (current) chunks.push(current); if (clean.length <= limit) { current = clean; continue; } current = ''; for (const word of clean.split(' ')) { const next = current ? `${current} ${word}` : word; if (next.length > limit && current) { chunks.push(current); current = word; } else current = next; } }
  if (current) chunks.push(current); return chunks.map((item) => normalizeLimit(item, limit)).filter(Boolean);
}
...
export function installVoiceOutput(documentRef: Document, root: VoiceOutputWindow): VoiceOutputController | null {
  ...
  const speakNext = (): void => { if (!enabled || !supported || !queue.length || !Utterance || !synth) { speaking = false; render(); return; } const utterance = new Utterance(queue.shift() ?? ''); utterance.lang = 'tr-TR'; utterance.rate = 0.98; utterance.pitch = 1; const voice = findTurkishVoice(); if (voice) utterance.voice = voice; utterance.onend = speakNext; utterance.onerror = () => { queue = []; speaking = false; render(); }; speaking = true; thinking = false; render(); try { synth.speak(utterance); } catch { queue = []; speaking = false; render(); } };
  ...
  const streamObserver = messageInput ? new MutationObserver(syncStreamState) : null; streamObserver?.observe(messageInput, { attributes: true, attributeFilter: ['disabled'] }); render();
