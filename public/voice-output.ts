type VoiceOutputWindow = Window & typeof globalThis & {
  HafizeVoiceOutput?: VoiceOutputApi;
};

type VoiceOutputApi = Readonly<{
  readonly STORAGE_KEY: string;
  readonly normalizeSpeechText: (value: unknown) => string;
  readonly splitSpeechText: (value: unknown, maxLength?: number) => string[];
  readonly installVoiceOutput: (documentRef: Document, root: VoiceOutputWindow) => VoiceOutputController | null;
}>;

export type VoiceOutputController = Readonly<{
  readonly isSupported: boolean;
  readonly isEnabled: () => boolean;
  readonly isSpeaking: () => boolean;
  readonly setEnabled: (next: boolean) => boolean;
  readonly speak: (value: unknown) => boolean;
...
function readStoredEnabled(storage: Storage | null | undefined): boolean {
  try {
...
function writeStoredEnabled(storage: Storage | null | undefined, enabled: boolean): void {
  try {
    storage?.setItem(STORAGE_KEY, String(enabled));
  } catch {
    // Local persistence is optional; in-memory state remains usable.
  }
}
...
function installVoiceOutput(documentRef: Document, root: VoiceOutputWindow): VoiceOutputController | null {
  ...
  const supported = typeof synth?.speak === 'function' && typeof Utterance === 'function';
  ...
  const render = (): void => {
    ...
    toggle.textContent = supported ? (enabled ? 'Sesli yanıt açık' : 'Sesli yanıt kapalı') : 'Sesli yanıt desteklenmiyor';
    toggle.title = supported ? 'Hafize yanıtlarını bu cihazın yerleşik sesiyle oku' : 'Bu tarayıcı Speech Synthesis API desteklemiyor';
  ...
  const findTurkishVoice = (): SpeechSynthesisVoice | null => {
    try {
      return [...(synth?.getVoices() ?? [])].find((voice) => String(voice.lang).toLowerCase().startsWith('tr')) ?? null;
    } catch {
      return null;
    }
  };
  ...
  const speakNext = (): void => {
    if (!enabled || !supported || !queue.length || !Utterance || !synth) {
      speaking = false;
      render();
      return;
    }
    ...
    thinking = false;
    render();
    try { synth.speak(utterance); } catch { utterance.onerror(new SpeechSynthesisErrorEvent('error')); }
  };

  const speak = (value: unknown): boolean => {
  ...
  const handleToggle = (): void => { setEnabled(!enabled); };
  const handleSubmit = (): void => { cancelSpeech(); };
  const handleVisibility = (): void => { if (documentRef.hidden) cancelSpeech(); };

  toggle.addEventListener('click', handleToggle);
  composer?.addEventListener('submit', handleSubmit, true);
  documentRef.addEventListener('visibilitychange', handleVisibility);

  const micObserver = micButton ? new MutationObserver(() => {
  ...
  const streamObserver = messageInput ? new MutationObserver(syncStreamState) : null;
...
export const HafizeVoiceOutput: VoiceOutputApi = Object.freeze({
  STORAGE_KEY,
  normalizeSpeechText,
  splitSpeechText,
  installVoiceOutput
...
const root = globalThis as VoiceOutputWindow;
root.HafizeVoiceOutput = HafizeVoiceOutput;
const boot = (): void => { void installVoiceOutput(document, root); };
