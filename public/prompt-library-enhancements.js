(function installPromptLibraryEnhancements(root) {
  ...
  function onClick(event) {
    ...
    const itemForTarget = () => load().find((candidate) => candidate.id === target.closest('.prompt-item')?.dataset.promptId);
    if (action === 'copy') { const item = itemForTarget(); if (item) copyItem(item); return; }
    if (action === 'duplicate') { const item = itemForTarget(); if (item) duplicateItem(item); return; }
  ...
  function enhance() {
    const card = root.document?.querySelector?.('#promptLibraryCard'); const list = card?.querySelector?.('#promptLibraryList'); if (!card || !list) return;
    ...
    if (hasSelection && !existingBulk) { const bulk = root.document.createElement('div'); bulk.className = 'prompt-library-enhancement-bulk'; bulk.append(makeButton('Seçilenleri sil', 'bulk-delete'), makeButton('Seçimi kaldır', 'bulk-clear')); list.prepend(bulk); bulk.addEventListener('click', onClick); }
  ...
  function boot() {
    ...
    root.addEventListener?.('hafize:prompt-library-changed', syncCore); listeners.push(() => root.removeEventListener?.('hafize:prompt-library-changed', syncCore));
  }
