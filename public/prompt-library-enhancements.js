(function installPromptLibraryEnhancements(root) {
  ...
  function clearFilters() {
    ...
    const search = root.document.querySelector('#promptLibrarySearch'); search?.focus?.();
  ...
  function onClick(event) {
    ...
    if (action === 'copy') { const item = load().find((candidate) => candidate.id === target.closest('.prompt-item')?.dataset.promptId); if (item) copyItem(item); return; }
    if (action === 'duplicate') { const item = load().find((candidate) => candidate.id === target.closest('.prompt-item')?.dataset.promptId); if (item) duplicateItem(item); return; }
