...
const UPSTREAM_TIMEOUT_MS = boundedEnvInteger(process.env.HAFIZE_UPSTREAM_TIMEOUT_MS, 60_000, 5_000, 300_000);
...
function setSecurityHeaders(res: ServerResponse): void {
  ...
  res.setHeader('Permissions-Policy', 'camera=(), geolocation=(), usb=(), payment=()');
...
function sendSseContent(res: ServerResponse, content: string): void {
  startSse(res);
  if (content) {
...
function writeToolActivitySse(res: ServerResponse, activity: unknown): void {
  if (!activity) return;
...
async function nvidiaFetch(pathname: string, init: RequestInit = {}): Promise<Response> {
  if (!NVIDIA_API_KEY) {
    const error = new Error('NVIDIA_NOT_CONFIGURED') as HafizeError;
    error.status = 503;
    throw error;
  }
...
async function nvidiaJsonCompletion(payload: NvidiaPayload, signal?: AbortSignal): Promise<unknown> {
  ...
  if (!upstream.ok) {
    const error = new Error('NVIDIA_CHAT_ERROR') as HafizeError;
  ...
  try {
    ...
    const error = new Error('INVALID_NVIDIA_RESPONSE') as HafizeError;
    error.status = 502;
    throw error;
  }
}

function normalizeRootCompletion(response: unknown) {
  try {
    return normalizeNvidiaChatCompletion(response);
  } catch (error) {
    const normalized = new Error('INVALID_NVIDIA_RESPONSE') as HafizeError;
    normalized.code = error instanceof Error ? error.message : 'INVALID_NVIDIA_RESPONSE';
    normalized.status = 502;
    throw normalized;
  }
}

function boundedMaxTokens(body: JsonObject): number {
  const value = body.max_tokens;
  return typeof value === 'number' && Number.isInteger(value) ? Math.min(Math.max(value, 1), 8192) : 2048;
}

const CONTEXT_COMPACTOR = createContextCompactor({
  contextLimitTokens: CONTEXT_LIMIT_TOKENS,
  async summarize({ model, source, messageCount, signal }: { model: string; source: string; messageCount: number; signal: AbortSignal }) {
...
async function prepareConversation(messages: unknown[], model: string, signal: AbortSignal, res: ServerResponse) {
...
const SCHEDULED_AGENT_EXECUTOR = createScheduledAgentExecutor({
  ...
  async complete(payload: NvidiaPayload) {
...
let scheduleTickPromise: Promise<void> | null = null;
...
function startScheduleWorkerLoop(): void {
  if (!NVIDIA_API_KEY || !SCHEDULE_EXECUTION_RUNTIME.configured) return;
  scheduleWorkerTimer = setInterval(() => { void runScheduleTick(); }, SCHEDULE_TICK_MS);
  scheduleWorkerTimer.unref?.();
}

function stopScheduleWorkerLoop(): void {
...
async function handleModels(res: ServerResponse): Promise<void> {
  ...
  const data = payload && typeof payload === 'object' && Array.isArray((payload as JsonObject).data)
    ? (payload as JsonObject).data
    : [];
  const models = data.map((item) => (item && typeof item === 'object' && typeof (item as JsonObject).id === 'string' ? (item as JsonObject).id : null))
...
async function handleAgentRun(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const delegator = createAgentDelegator({
    ...
    runLedger,
    async executeAgent({ delegatedAgent, task, traceId: delegatedTraceId, parentTaskId: delegatedParentTaskId }: { delegatedAgent: unknown; task: string; traceId: string; parentTaskId: string }) {
      return runDelegatedAgent({
        ...
        githubReadConfigured: GITHUB_READ_CONFIGURED,
        githubReadFile: GITHUB_READ_FILE,
        complete: (payload: NvidiaPayload) => nvidiaJsonCompletion(payload, controller.signal)
      });
    }
  });
  ...
  const firstPayload: NvidiaPayload = {
    model,
  ...
  const assistant: NormalizedAssistant = {
    role: 'assistant',
    content: first.content,
    tool_calls: first.toolCalls.map((call: { id: string; name: string; arguments: string }) => ({
      id: call.id,
      type: 'function',
  ...
  const normalizedCalls: NormalizedToolCall[] = toolCalls
    .filter((call: NormalizedToolCall) => Boolean(call?.id && call?.function?.name))
  ...
  for (const call of normalizedCalls) {
    ...
    const result = await executeNvidiaToolCall(agent, call, {
      ...
      githubReadConfigured: GITHUB_READ_CONFIGURED,
      githubReadFile: GITHUB_READ_FILE,
      delegateAgent: (args: unknown) => delegator.delegate(args, { depth: 0 }),
  ...
  const secondPayload: NvidiaPayload = {
    model,
    messages: [...conversation, assistant, ...toolMessages],
...
const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  res.once('finish', () => {
    ...
    res.setHeader('X-Hafize-Response-Time-Ms', elapsed);
  });
  try {
