export interface ReleaseCheck { name: string; pass: boolean }
export interface ReleaseReadiness { ready: boolean; blocked?: string[]; warnings?: string[]; components?: Record<string, { status?: unknown }> }
export interface ReleaseManifest { version: string; commit: string; generatedAt: string; requiredGates: readonly string[]; readiness: { blocked: string[]; warnings: string[] }; checks: readonly ReleaseCheck[]; releaseable: boolean }
...
export function createReleaseManifest({ version, commit, readiness, checks = [] }: { version?: unknown; commit?: unknown; readiness?: ReleaseReadiness; checks?: unknown[] } = {}): ReleaseManifest {
...
export function missingReleaseGates(readiness: ReleaseReadiness): string[] {
