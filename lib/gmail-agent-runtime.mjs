...
export function createGmailAgentRuntime({
  ...
  const bearerConfiguredCount = [authToken, authSubject].filter(Boolean).length;
  if (bearerConfiguredCount !== 0 && bearerConfiguredCount !== 2) fail('config');
  if (typeof fetchImpl !== 'function') fail('fetch');

  const sessionRuntime = requireFactory(createSessionRuntime, 'createSessionRuntime')({ env });
  const cloudAuthenticator = sessionRuntime?.configured ? sessionRuntime.authenticator : null;
  if (sessionRuntime?.configured && typeof cloudAuthenticator?.authenticate !== 'function') fail('cloudSession');
  const browserSessionConfigured = Boolean(sessionRuntime?.configured && cloudAuthenticator);
  const bearerConfigured = bearerConfiguredCount === 2;

  if (!ownerKey && !bearerConfigured && !browserSessionConfigured) {
  ...
  if (!ownerKey || (!bearerConfigured && !browserSessionConfigured)) fail('config');
  ...
  const bearerAuthenticator = bearerConfigured
    ? requireFactory(createAuthenticator, 'createAuthenticator')({ token: authToken, subject: authSubject })
    : null;
  ...
  function authenticate(headers) {
    if (cloudAuthenticator) {
      const principal = validPrincipal(cloudAuthenticator.authenticate({ headers }));
      if (principal) return principal;
    }
    if (bearerAuthenticator) return validPrincipal(bearerAuthenticator.authenticate({ headers }));
    return null;
  }
  ...
  function requestContext({ headers } = {}) {
    ...
    return Object.freeze({
      ...
      gmailReadTool: Object.freeze({
        execute(args, { signal = null } = {}) {
          return boundary.execute(args, { principal, signal });
        }
      })
    });
  }
