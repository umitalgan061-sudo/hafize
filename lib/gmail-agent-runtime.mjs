...
export function createGmailAgentRuntime({
  ...
  createReadBoundary = createGmailReadToolBoundary,
  ...
  if (configuredCount === 0) {
    ...
    return Object.freeze({
      ...
      status() { return Object.freeze({ configured: false, sendConfigured: false, access: 'disabled' }); }
    });
  }
  ...
  const readBoundary = requireFactory(createReadBoundary, 'createReadBoundary')({ readClient, ownerResolver });
  ...
  function requestContext({ headers, gmailSendApprovalToken } = {}) {
    ...
    if (sendApproval && typeof gmailSendApprovalToken === 'string' && gmailSendApprovalToken) {
      ...
      context.gmailSendAuthenticated = true;
      context.gmailSendTool = Object.freeze({
        async execute(args) {
  ...
  async function issueSendApproval({ headers, message, userConfirmed } = {}) {
    ...
    const ownerId = resolveOwner(principal);
    const token = sendApproval.issue({ ownerId, message, userConfirmed });
  ...
  return Object.freeze({
    ...
    status() {
      return Object.freeze({
        configured: true,
        sendConfigured: Boolean(sendApproval),
        access: sendApproval ? 'authenticated-read-and-approved-send' : 'authenticated-read-only'
      });
    }
  });
}
