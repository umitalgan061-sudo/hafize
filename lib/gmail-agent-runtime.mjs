...
export function createGmailAgentRuntime({
  ...
  if (googleClientId) {
    const storage = typeof tokenStore?.status === 'function' ? tokenStore.status()?.storage : null;
    if (storage !== 'encrypted-redis') fail('googleRefreshTokenStorage');
  }
  ...
  async function close() {
    const results = await Promise.allSettled([
      refreshLease?.close(),
      typeof tokenStore?.close === 'function' ? tokenStore.close() : undefined
    ]);
    if (results.some((result) => result.status === 'rejected')) throw new Error('GMAIL_AGENT_RUNTIME_CLOSE_FAILED');
  }
