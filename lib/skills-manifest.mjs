...
function normalizeOptionalDescription(value) {
  ...
  if (typeof value !== 'string') fail('INVALID_SKILL_DESCRIPTION');
  return value.trim() ? normalizeDescription(value) : '';
}
...
function normalizeArguments(value) {
  ...
  for (const item of value) {
    ...
    args.push(Object.freeze({
      ...
      // An argument description is optional, so an absent or blank one is not an
      // error — only a present, malformed one is. This also keeps a normalized
      // manifest (blank descriptions included) re-normalizable.
...
function normalizeModel(value) {
  // An absent model and an explicitly empty model both mean "inherit the runtime
  // default". Accepting '' keeps normalization idempotent: a normalized manifest
  // (which always carries `model: ''`) can be re-registered without failing.
  if (value === undefined || value === null) return '';
  const model = typeof value === 'string' ? value.trim() : '';
  if (!model) {
    if (typeof value !== 'string') fail('INVALID_SKILL_MODEL');
    return '';
  }
