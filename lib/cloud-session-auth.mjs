...
const MAX_SCRYPT_WORK_UNITS = 16 * 1024 * 1024;
...
function validateScryptCost(N, r, p) {
  ...
  if (!Number.isSafeInteger(memoryBytes) || !Number.isSafeInteger(workUnits) || !Number.isSafeInteger(maxmem)) {
    fail('INVALID_CLOUD_SESSION_AUTH:passwordHash.cost');
  }
  if (maxmem > MAX_SCRYPT_MEMORY_BYTES || workUnits > MAX_SCRYPT_WORK_UNITS) {
    fail('INVALID_CLOUD_SESSION_AUTH:passwordHash.cost');
  }
