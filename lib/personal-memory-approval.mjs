...
function canonicalBody(kind, body) {
  if (kind === 'write') {
    return Object.freeze({
      kind: body.kind,
  ...
  if (kind === 'delete-one') return Object.freeze({ exactMatch: true, explicitUserIntent: true });
  if (kind === 'delete-all') return Object.freeze({ explicitUserIntent: true, confirmDeleteAll: true });
  return Object.freeze({ explicitUserIntent: true });
}
...
export function normalizeMemoryApprovalCommand(input) {
  ...
  const body = canonicalBody(input.kind, input.body);
  if (input.kind === 'delete-one') {
    ...
    return Object.freeze({ kind: input.kind, memoryId, body });
  }
  if (input.memoryId !== undefined) fail('INVALID_MEMORY_APPROVAL_COMMAND');
  return Object.freeze({ kind: input.kind, body });
}
