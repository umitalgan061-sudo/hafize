import { createCanvaOAuthUpstream } from './canva-oauth-upstream.mjs';

const TOKEN_ENDPOINT = 'https://api.canva.com/rest/v1/oauth/token';
const OWNER_PATTERN = /^[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}$/;
const SCOPE_PATTERN = /^[A-Za-z0-9][A-Za-z0-9:._-]{0,127}$/;
const MAX_SCOPES = 32;

function text(value, field, { min = 1, max = 4096 } = {}) {
  const normalized = typeof value === 'string' ? value.trim() : '';
  if (normalized.length < min || normalized.length > max) throw new Error(`INVALID_CANVA_TOKEN_REFRESH:${field}`);
  return normalized;
}

function normalizeScopes(value, field) {
  if (!Array.isArray(value) || value.length > MAX_SCOPES) throw new Error(`INVALID_CANVA_TOKEN_REFRESH:${field}`);
  const seen = new Set();
  const scopes = [];
  for (const raw of value) {
    if (typeof raw !== 'string') throw new Error(`INVALID_CANVA_TOKEN_REFRESH:${field}`);
    const scope = raw.trim();
    if (!scope || !SCOPE_PATTERN.test(scope) || seen.has(scope)) throw new Error(`INVALID_CANVA_TOKEN_REFRESH:${field}`);
    seen.add(scope);
    scopes.push(scope);
  }
  return scopes.sort();
}

function assertSameScopes(actualScopes, previousScopes) {
  const actual = normalizeScopes(actualScopes, 'responseScopes');
  const previous = normalizeScopes(previousScopes, 'previousScopes');
  if (actual.length !== previous.length || actual.some((scope, index) => scope !== previous[index])) throw new Error('CANVA_TOKEN_REFRESH_SCOPE_MISMATCH');
  return actual;
}

function normalizeResponse(value, previousScopes) {
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new Error('CANVA_TOKEN_REFRESH_FAILED:response');
  const accessToken = text(value.access_token, 'accessToken', { max: 4096 });
  const refreshToken = text(value.refresh_token, 'refreshToken', { max: 4096 });
  const tokenType = text(value.token_type, 'tokenType', { max: 32 });
  if (tokenType.toLowerCase() !== 'bearer') throw new Error('CANVA_TOKEN_REFRESH_FAILED:tokenType');
  const expiresIn = Number(value.expires_in);
  if (!Number.isInteger(expiresIn) || expiresIn < 60 || expiresIn > 86400) throw new Error('CANVA_TOKEN_REFRESH_FAILED:expiresIn');
  const previous = normalizeScopes(previousScopes, 'previousScopes');
  const scopes = value.scope == null ? previous : assertSameScopes(text(value.scope, 'scope', { max: 4096 }).split(/\s+/).filter(Boolean), previous);
  return Object.freeze({ accessToken, refreshToken, tokenType: 'Bearer', expiresIn, scopes: Object.freeze(scopes) });
}

function upstreamFailure(error) {
  if (error?.code === 'CANVA_OAUTH_UPSTREAM_TIMEOUT') return new Error('CANVA_TOKEN_REFRESH_FAILED:timeout');
  if (error?.code === 'CANVA_OAUTH_UPSTREAM_CANCELLED') return new Error('CANVA_TOKEN_REFRESH_FAILED:cancelled');
  if (error?.code === 'CANVA_OAUTH_UPSTREAM_INVALID_RESPONSE') return new Error('CANVA_TOKEN_REFRESH_FAILED:response');
  return error;
}

export function createCanvaTokenRefresh({ clientId, clientSecret, tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now(), timeoutMs, AbortControllerImpl = globalThis.AbortController, setTimeoutImpl = globalThis.setTimeout, clearTimeoutImpl = globalThis.clearTimeout } = {}) {
  const safeClientId = text(clientId, 'clientId', { max: 512 });
  const safeClientSecret = text(clientSecret, 'clientSecret', { max: 2048 });
  if (typeof tokenStore?.load !== 'function' || typeof tokenStore?.save !== 'function') throw new Error('INVALID_CANVA_TOKEN_REFRESH:tokenStore');
  if (typeof now !== 'function') throw new Error('INVALID_CANVA_TOKEN_REFRESH:now');
  const authorization = `Basic ${Buffer.from(`${safeClientId}:${safeClientSecret}`, 'utf8').toString('base64')}`;
  const upstream = createCanvaOAuthUpstream({ fetchImpl, AbortControllerImpl, setTimeoutImpl, clearTimeoutImpl, ...(timeoutMs == null ? {} : { timeoutMs }) });

  async function refresh({ ownerId, signal } = {}) {
    const owner = text(ownerId, 'ownerId', { max: 128 });
    if (!OWNER_PATTERN.test(owner)) throw new Error('INVALID_CANVA_TOKEN_REFRESH:ownerId');
    const current = await tokenStore.load({ ownerId: owner, provider: 'canva' });
    if (!current || Array.isArray(current) || typeof current !== 'object') throw new Error('CANVA_TOKEN_REFRESH_FAILED:missingRecord');
    const refreshToken = text(current.refreshToken, 'refreshToken', { max: 4096 });
    const previousScopes = normalizeScopes(current.scopes, 'previousScopes');
    const body = new URLSearchParams({ grant_type: 'refresh_token', refresh_token: refreshToken });
    let result;
    try {
      result = await upstream.postForm(TOKEN_ENDPOINT, { authorization, body: body.toString(), signal, json: true });
    } catch (error) {
      throw upstreamFailure(error);
    }
    if (!result.ok) throw new Error('CANVA_TOKEN_REFRESH_FAILED:http');
    const tokens = normalizeResponse(result.payload, previousScopes);
    const issuedAt = Number(now());
    if (!Number.isFinite(issuedAt)) throw new Error('INVALID_CANVA_TOKEN_REFRESH:now');
    const expiresAt = issuedAt + tokens.expiresIn * 1000;
    await tokenStore.save({ ownerId: owner, provider: 'canva', tokenRecord: { accessToken: tokens.accessToken, refreshToken: tokens.refreshToken, tokenType: tokens.tokenType, scopes: tokens.scopes, expiresAt } });
    return Object.freeze({ provider: 'canva', ownerId: owner, scopes: tokens.scopes, expiresAt, rotated: true });
  }

  return Object.freeze({ refresh });
}

export { TOKEN_ENDPOINT as CANVA_REFRESH_TOKEN_ENDPOINT };
