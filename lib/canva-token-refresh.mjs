...
function requestObject(value, allowed) {
  ...
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new Error('INVALID_CANVA_TOKEN_REFRESH:request');
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) throw new Error(`INVALID_CANVA_TOKEN_REFRESH:request.${key}`);
  }
  return value;
}
...
export function createCanvaTokenRefresh({ clientId, clientSecret, tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now() } = {}) {
  ...
  async function refresh(request) {
    const { ownerId } = requestObject(request, new Set(['ownerId']));
