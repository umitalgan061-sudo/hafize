...
function assertSameScopes(actualScopes, previousScopes) {
  ...
  if (actual.length !== previous.length || actual.some((scope, index) => scope !== previous[index])) {
    throw new Error('CANVA_TOKEN_REFRESH_SCOPE_MISMATCH');
  }
  return actual;
}
...
function normalizeResponse(value, previousScopes) {
  ...
  const scopes = value.scope == null
    ? previous
    : assertSameScopes(text(value.scope, 'scope', { max: 4096 }).split(/\s+/).filter(Boolean), previous);
