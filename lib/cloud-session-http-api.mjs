...
function completeLoginGate(ticket, authenticated) {
  if (!ticket || typeof ticket.complete !== 'function') return;
  ticket.complete({ authenticated: authenticated === true });
}
    ...
    try {
      ...
      if (pathname === LOGIN_PATH) {
        ...
        if (gateTicket?.ok === false) {
          const retryAfter = Number.isSafeInteger(gateTicket.retryAfterSeconds) && gateTicket.retryAfterSeconds > 0 ? gateTicket.retryAfterSeconds : 1;
