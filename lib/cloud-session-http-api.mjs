import { requireEmptyCloudSessionBody } from './cloud-session-empty-body-policy.mjs';
  ...
  async function handle({ request, method, pathname, headers } = {}) {
    ...
    try {
      ...
      if (
        ...
        || code === 'INVALID_CLOUD_SESSION_HTTP:body'
        || code === 'CLOUD_SESSION_INVALID_UTF8'
        || code === 'CLOUD_SESSION_INVALID_CONTENT_LENGTH'
        || code === 'CLOUD_SESSION_LENGTH_MISMATCH'
        || code === 'CLOUD_SESSION_BODY_NOT_ALLOWED'
      ) {
        const closeConnection = code === 'CLOUD_SESSION_INVALID_CONTENT_LENGTH' || code === 'CLOUD_SESSION_LENGTH_MISMATCH' || code === 'CLOUD_SESSION_BODY_NOT_ALLOWED';
        return response(400, { error: 'INVALID_REQUEST' }, closeConnection ? { connection: 'close' } : {});
      }
      ...
      if (code === 'CLOUD_SESSION_UNSUPPORTED_MEDIA_TYPE' || code === 'CLOUD_SESSION_UNSUPPORTED_CONTENT_ENCODING') {
