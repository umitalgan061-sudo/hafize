...
function optionalObject(value, reason) {
  if (value === undefined) return {};
  if (!value || Array.isArray(value) || typeof value !== 'object') throw invalid(reason);
  return value;
}

export function createGmailReadToolBoundary(dependencies) {
  const { readClient, ownerResolver } = optionalObject(dependencies, 'dependencies');
  ...
  async function execute(args, context) {
    ...
    const ownership = ownerResolver.resolve(optionalObject(context, 'context').principal);
