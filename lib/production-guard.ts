...
if (!globalThis.__HAFIZE_PRODUCTION_GUARD__) {
  ...
  function protectedPath(path: string): boolean {
    return ['/api/models','/api/agents','/api/chat','/api/agent/run','/api/connectors/canva/status','/api/connectors/gmail/status','/api/github/workspace','/api/github/workspace/directory','/api/github/workspace/compare'].includes(path);
  }
