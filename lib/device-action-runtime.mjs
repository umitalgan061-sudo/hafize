...
/**
 * @typedef {{ ok: false; error: string; reason?: string }} DeviceActionFailure
 * @typedef {{ action?: string; [key: string]: any }} DeviceActionRequest
 * @typedef {{ ownerId: string; traceId: string; expiresAt: number }} ReviewSession
 *
 * Dönüş tipleri ayrımlı birleşimdir: `ok` bir literal olduğu için çağıran
 * taraf `ok === true` ile daraltarak hangi alanların var olduğunu görebilir.
 *
 * @typedef {DeviceActionFailure | { ok: true; review: Record<string, any> }} BeginReviewResult
 * @typedef {DeviceActionFailure | { ok: true; [key: string]: any }} ExecutionResult
 * @typedef {DeviceActionFailure | { ok: true }} CancelReviewResult
 */
 ...
 *
 * @param {DeviceActionRequest | null | undefined} request
 */
...
/**
 * @param {{ allowedApps?: unknown } | null | undefined} deviceBridge
 * @returns {Set<string>}
 */
...
/**
 * @param {{ resolve: (principal: unknown) => { ownerId?: unknown } | null }} ownerResolver
...
/**
 * @param {((event: unknown) => unknown) | undefined} auditSink
 * @param {unknown} event
 * @returns {Promise<{ ok: true } | DeviceActionFailure>}
 */
 ...
 *   approvalStore?: any;
 *   reviewStore?: any;
 *   auditSink?: (event: unknown) => unknown;
 *   now?: () => number;
 *   maxActiveReviewsPerOwner?: number;
 * }} [options]
 */
export function createDeviceActionRuntime({ deviceBridge, ownerResolver, approvalStore = createDeviceApprovalLeaseStore(), reviewStore, auditSink, now = () => Date.now(), maxActiveReviewsPerOwner } = {}) {
  ...
  /**
   * @param {any} agent
   * @param {DeviceActionRequest | null} request
   * @param {{ traceId?: string; principal?: unknown }} [context]
   ...
   * @param {any} agent
   * @param {DeviceActionRequest | null} request
   * @param {{ traceId?: string; principal?: unknown; ttlMs?: number }} [context]
   * @returns {Promise<BeginReviewResult>}
   */
   ...
   * @param {any} agent
   * @param {DeviceActionRequest | null} request
   * @param {{ reviewId?: string; traceId?: string; principal?: unknown }} [context]
   * @returns {Promise<ExecutionResult>}
   */
   ...
   * @returns {Promise<CancelReviewResult>}
   */
