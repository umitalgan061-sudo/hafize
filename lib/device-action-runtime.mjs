...
export function createDeviceActionRuntime({ deviceBridge, approvalStore = createDeviceApprovalLeaseStore(), reviewStore, auditSink } = {}) {
  ...
  async function executeReadOnly(agent, request, { traceId } = {}) {
    try { cleanTraceId(traceId); } catch (error) { return fail(error.message); }
  ...
  async function beginReview(agent, request, { traceId, ttlMs } = {}) {
    ...
    try { normalizedTraceId = cleanTraceId(traceId); } catch (error) { return fail(error.message); }
  ...
  async function confirmAndExecute(agent, request, { reviewId, traceId } = {}) {
    ...
    const result = await executeDeviceToolRequest(agent, authorization.request, {
      deviceBridge, approvalStore, approvalToken: token, traceId: normalizedTraceId
    });
  ...
  return Object.freeze({ executeReadOnly, beginReview, confirmAndExecute, cancelReview });
}
