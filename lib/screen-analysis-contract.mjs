...
export function validateScreenAnalysisRequest(value) {
  ...
  if (model.startsWith('local:')) return invalid('nvidia_model_required');

  const prompt = typeof value.prompt === 'string' ? value.prompt.replace(/\r\n?/g, '\n').trim() : '';
  if (!prompt || prompt.length > MAX_PROMPT_LENGTH || /[\0\u000b\u000c]/.test(prompt)) return invalid('invalid_prompt');
  if (containsPlaintextCredential(prompt)) return invalid('plaintext_credential_not_allowed');
