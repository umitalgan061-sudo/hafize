...
function invalid(reason: string): HafizeCodedError {
  const error = (new Error(`INVALID_GMAIL_READ_TOOL:${reason}`) as HafizeCodedError);
...
export function createGmailReadToolBoundary({ readClient, ownerResolver }: { readClient?: any; ownerResolver?: any } = {}) {
