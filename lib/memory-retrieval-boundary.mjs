...
export function normalizeMemoryRetrieval(input) {
  try {
    // Sözleşme gereği bu fonksiyon asla throw etmez; bu yüzden girdi
    // doğrulaması da destructuring yerine try bloğunun içinde yapılır.
    const source = input === undefined ? {} : input;
    if (!source || Array.isArray(source) || typeof source !== 'object') {
      throw new Error('INVALID_MEMORY_RETRIEVAL:input');
    }
    const { ownerId, records } = source;
