import {
  ...
  containsPlaintextMemoryCredential
...
function rejectPlaintextCredential(value) {
  if (containsPlaintextMemoryCredential(value)) {
    throw new Error('MEMORY_RETRIEVAL_PLAINTEXT_CREDENTIAL_BLOCKED');
  }
  return value;
}
...
function normalizeRecord(record, ownerId) {
  ...
  const content = rejectPlaintextCredential(
    text(record.content, 'content', PERSONAL_MEMORY_CONTRACT.maxContentLength)
  );
  const sourceRef = record.sourceRef == null
    ? null
    : rejectPlaintextCredential(text(record.sourceRef, 'sourceRef', 300));
  return {
    memoryId,
    kind,
    content,
