...
export function normalizeMemoryRetrieval(input = {}) {
  try {
    // Girdi doğrulaması try içinde kalmalı: bu sınır asla fırlatmaz, her zaman
    // { ok, error } döndürür. Parametre destructuring'i null girdide bu
    // sözleşmeyi kodsuz bir TypeError ile deliyordu.
    if (!input || Array.isArray(input) || typeof input !== 'object') {
      throw new Error('INVALID_MEMORY_RETRIEVAL:input');
    }
    const { ownerId, records } = input;
