...
function safeCredentialText(value, label, maxLength, errorCode) {
  const result = text(value, label, maxLength);
  if (containsPlaintextCredential(result)) throw new Error(errorCode);
  return result;
}

function normalizeRecord(record, ownerId) {
  if (!record || Array.isArray(record) || typeof record !== 'object') throw new Error('INVALID_MEMORY_RETRIEVAL:record');
  ...
  if (!PERSONAL_MEMORY_CONTRACT.sourceTypes.includes(sourceType)) throw new Error('INVALID_MEMORY_RETRIEVAL:sourceType');
  return {
    memoryId,
    kind,
    content: safeCredentialText(record.content, 'content', PERSONAL_MEMORY_CONTRACT.maxContentLength, 'MEMORY_RETRIEVAL_CREDENTIAL_BLOCKED'),
    sourceType,
    sourceRef: record.sourceRef == null ? null : safeCredentialText(record.sourceRef, 'sourceRef', 300, 'MEMORY_RETRIEVAL_CREDENTIAL_BLOCKED')
  };
}
