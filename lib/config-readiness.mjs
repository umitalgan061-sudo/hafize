...
export function evaluateConfigReadiness(env = process.env) {
  if (!env || typeof env !== 'object' || Array.isArray(env)) {
    return Object.freeze({ state: 'blocked', findings: Object.freeze([problem('INVALID_ENV', 'env')]), production: false, authRequired: true, secretVariables: SECRET_VARS });
  }

  const production = text(env.NODE_ENV).toLowerCase() === 'production' || text(env.HOST) === '0.0.0.0';
  const authRequired = booleanValue(env.HAFIZE_AUTH_REQUIRED, production);
  const findings = [];
  ...
  return Object.freeze({ state, production, authRequired, findings: Object.freeze(findings), secretVariables: SECRET_VARS });
}
