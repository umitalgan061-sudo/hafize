  ...
  http.createServer = function hafizeCreateServer(...args) {
    ...
    args[i] = async (req, res) => {
      ...
      if (required && protectedPath(path)) {
        ...
        if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method) && !csrf(req, current)) return send(res, 403, { error: 'CSRF_REQUIRED' });
        const limit = (path === '/api/chat' || path === '/api/agent/run') ? chatLimit : apiLimit;
        const decision = limit.check(`${current.principal.subject}:${path === '/api/chat' || path === '/api/agent/run' ? 'chat' : 'api'}`);
