...
if (!globalThis.__HAFIZE_PRODUCTION_GUARD__) {
  ...
  http.createServer = function hafizeCreateServer(...args) {
    ...
    args[i] = async (req, res) => {
      ...
      try { res.setHeader('X-Hafize-Request-Id', security.record({ event: 'request.received', requestId, route: path, method: req.method, outcome: 'received' }).requestId); } catch {}
  ...
  function authenticateProtected(req, path) {
    ...
    if (current?.ok) return { ok: true, session: true, subject: current.principal.subject };
    if (path === '/api/agent/run' && connectorAuth) {
