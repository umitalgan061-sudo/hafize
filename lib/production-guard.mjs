...
if (!globalThis.__HAFIZE_PRODUCTION_GUARD__) {
  ...
  http.createServer = function hafizeProtectedCreateServer(...args) {
    ...
    args[listenerIndex] = async function hafizeProtectedRequest(req, res) {
      ...
      if (authRequired && isProtectedApi(pathname)) {
        ...
        const isChat = pathname === '/api/chat' || pathname === '/api/agent/run';
        const key = `${session.principal.subject}:${isChat ? 'chat' : 'api'}`;
        const limiter = isChat ? chatLimiter : apiLimiter;
      ...
      if (!authRequired && (pathname === '/api/chat' || pathname === '/api/agent/run')) {
  ...
  async function handleLogin(req, res) {
    ...
    try {
      ...
      if (!sessionAuth.verifyCredential(candidate)) return rejectAuth(res);
