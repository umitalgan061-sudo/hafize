...
import { createDeviceApprovalAuditEvent } from './device-approval-audit.mts';
...
function normalizeReviewTtl(value: number | null | undefined): number {
...
function normalizeRequest(input: { action?: unknown; [key: string]: any } | null | undefined) {
...
/**
 * Yan etkili cihaz eylemleri için açık onay incelemesi tutar.
 */
export function createDeviceApprovalReviewStore({ approvalStore, now = () => Date.now(), randomId = randomUUID }: { approvalStore?: any; now?: () => number; randomId?: () => string } = {}) {
  ...
  function begin({ traceId, request, ttlMs }: { traceId?: string; request?: any; ttlMs?: number } = {}): { ok: false; error: string } | { ok: true; review: Record<string, any>; audit: Record<string, any> } {
    try {
      ...
      reviews.set(reviewId, Object.freeze({
        reviewId, traceId: normalizedTraceId, action: normalized.request.action,
        target: normalized.target, issuedAt, expiresAt
      }));
      return {
        ok: true as const,
        ...
        audit: createDeviceApprovalAuditEvent({
          stage: 'review_started', traceId: normalizedTraceId,
          action: normalized.request.action, target: normalized.target, timestamp: issuedAt
        })
      };
    } catch (error) {
  ...
  function confirm({ reviewId, traceId }: { reviewId?: string; traceId?: string } = {}) {
    try {
      ...
      return {
        ...
        audit: createDeviceApprovalAuditEvent({
          stage: 'approval_issued', traceId: entry.traceId,
          action: entry.action, target: entry.target, timestamp: currentTime
        })
      };
    } catch (error) {
      return fail(error.message);
  ...
  function cancel(reviewId: string) {
