...
function rejectUnsupportedLocalCompaction(model, messages, thresholdTokens) {
  if (!model.startsWith(LOCAL_MODEL_PREFIX)) return;
  if (estimateMessageTokens(messages) <= thresholdTokens) return;
...
export function createModelProviderChatPreparer({
  ...
  async function prepare(body, { signal = null } = {}) {
    ...
    rejectUnsupportedLocalCompaction(model, conversation, compactor.thresholdTokens);
