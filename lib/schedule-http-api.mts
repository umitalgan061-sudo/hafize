import { normalizeApiError } from './api-error-contract.mts';

const SCHEDULES_PATH = '/api/schedules';

function response(status: number, body: unknown, headers: Record<string, string> = {}) {
  return { matched: true as const, status, body, headers };
}

function commandErrorStatus(error: unknown) {
...
function requestIdFromHeaders(headers: Record<string, any> | null | undefined) {
  const value = headers?.['x-hafize-request-id'] || headers?.['X-Hafize-Request-Id'];
  return typeof value === 'string' && value.trim() ? value.trim() : null;
}

function commandResponse(output: Record<string, any> | null | undefined, successStatus: number, requestId: string | null) {
...
function scheduleIdFromPath(pathname: string) {
...
/**
 * Zamanlanmış görev uçlarını kullanıcı oturumundan ayrı bir kimlikle sarar.
 */
export function createScheduleHttpApi({ authenticator, commands, readJson }: { authenticator?: any; commands?: any; readJson?: (request: any) => Promise<unknown> } = {}) {
  ...
  async function handle({ request, method, pathname, headers }: { request?: any; method?: string; pathname?: string; headers?: Record<string, any> } = {}) {
    ...
    if (!root && scheduleId === null) return { matched: false as const };
    if (!root && !scheduleId) {
