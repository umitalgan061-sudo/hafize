...
export function createGmailReadClient({
  ...
  async function loadRecord(owner, signal) {
    return await waitForCaller(tokenStore.load({ ownerId: owner, provider: 'google' }), signal);
  }

  async function loadFreshRecord(owner, signal) {
    assertSignal(signal);
    let record = await loadRecord(owner, signal);
    assertSignal(signal);
    ...
    if (expiresAt <= timestamp + 30_000) {
      ...
      assertSignal(signal);
      try {
        // Refresh may be shared/coalesced across callers. Caller cancellation only
        // releases this reader; it does not abort the shared refresh operation.
        await waitForCaller(refreshAccessToken({ ownerId: owner }), signal);
      } catch (error) {
        assertSignal(signal);
        if (error?.code === 'GMAIL_READ_FAILED:cancelled') throw error;
        throw new Error('GMAIL_READ_REAUTH_REQUIRED');
      }
      assertSignal(signal);
      record = await loadRecord(owner, signal);
      assertSignal(signal);
      if (!record || Array.isArray(record) || typeof record !== 'object') throw new Error('GMAIL_READ_REAUTH_REQUIRED');
