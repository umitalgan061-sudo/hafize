import { requireObjectInput } from './boundary-input.mjs';
...
const inputObject = (value, field) => requireObjectInput(value, field, fail);
...
export function createGmailReadClient(options) {
  const { tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now(), maxJsonBytes = MAX_JSON_BYTES } = inputObject(options, 'options');
  ...
  async function read(request) {
    const { ownerId, operation, params } = inputObject(request, 'request');
