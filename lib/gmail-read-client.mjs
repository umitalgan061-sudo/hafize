...
export function createGmailReadClient({ tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now(), maxJsonBytes = MAX_JSON_BYTES } = {}) {
  ...
  async function read(request) {
    // `= {}` yalnız `undefined` için devreye girer; `null` çağrısı ham TypeError'a
    // düşerdi. Sınır her geçersiz girdi için kendi sözleşme hatasını üretmelidir.
    const { ownerId, operation, params } = strictObject(
      request === undefined ? {} : request,
      'request',
      new Set(['ownerId', 'operation', 'params'])
    );
