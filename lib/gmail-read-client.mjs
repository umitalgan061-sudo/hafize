...
const REQUEST_FIELDS = new Set(['ownerId', 'operation', 'params']);
...
export function createGmailReadClient({ tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now(), maxJsonBytes = MAX_JSON_BYTES } = {}) {
  ...
  async function read(request) {
    const input = request === undefined ? {} : strictObject(request, 'request', REQUEST_FIELDS);
    const owner = text(input.ownerId, 'ownerId', { max: 128 });
    if (!OWNER_PATTERN.test(owner)) fail('ownerId');
    const op = text(input.operation, 'operation', { max: 32 });
    ...
    const response = await fetchImpl(resolveUrl(op, input.params), {
