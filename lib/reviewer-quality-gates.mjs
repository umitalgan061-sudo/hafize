...
// Reviewer-specific additions on top of the shared credential policy, so a fix
// to the shared detector benefits this gate instead of having to be duplicated.
...
function hasSecret(text) {
  ...
  return containsPlaintextCredential(text) || SECRET_PATTERNS.some((pattern) => pattern.test(text));
}
