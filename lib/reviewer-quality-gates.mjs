const SECRET_PATTERNS = [
  // The armour dashes are often stripped when key material is pasted into a diff or note.
  /(?:-----)?BEGIN [A-Z ]*PRIVATE KEY(?:-----)?/,
  /\b(sk|gh[pousr]|nvapi)-[A-Za-z0-9_-]{12,}/,
  // Vendor-prefixed names (NVIDIA_API_KEY=…) must be caught as well as bare ones.
  /(?:^|[^a-z0-9])(?:[a-z0-9]{1,32}[_ -]){0,4}(?:api[-_ ]?key|client[-_ ]?secret|access[-_ ]?token|refresh[-_ ]?token|password)\b\s*[:=]\s*\S/i
];
