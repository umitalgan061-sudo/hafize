...
export function normalizeDeviceBridgeCommand(input: Record<string, any> | null | undefined) {
  try {
    requireObject(input);
    ...
    if (action === 'system.info') {
      ...
      return { ok: true as const, command: { action } };
    }
    ...
    if (action === 'browser.open') {
      ...
      return { ok: true as const, command: { action, url: normalizeUrl(input.url) } };
    }

    if ('url' in input) throw new Error('INVALID_DEVICE_BRIDGE_COMMAND:field');
    return { ok: true as const, command: { action, appId: normalizeAppId(input.appId) } };
  } catch (error) {
    return fail(error.message);
  }
}

export function normalizeSystemInfo(input: Record<string, any> | null | undefined) {
  try {
    ...
    // Alanlar döngüyle doldurulur; `platform` ve `arch` zorunlu, gerisi değil.
    const result: { platform?: string; arch?: string; [key: string]: string | undefined } = {};
    ...
    return { ok: true as const, info: result };
  } catch (error) {
    return fail(error.message);
  }
}

export function createDeviceBridge({ systemInfo, openExternal, openApp, allowedApps = [] }: { systemInfo?: Function; openExternal?: Function; openApp?: Function; allowedApps?: string[] } = {}) {
  ...
  async function execute(input: Record<string, any>) {
    ...
    try {
      if (command.action === 'system.info') {
        ...
        return info.ok ? { ok: true as const, action: command.action, info: info.info } : info;
      }
      ...
      return { ok: true as const, action: command.action, appId: command.appId };
    } catch (error) {
