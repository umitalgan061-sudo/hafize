...
export function createGoogleOAuthNodeServerRuntime({
  ...
  const guardedTokenStore = Object.freeze({
    save(input) {
      const scopes = input?.tokenRecord?.scopes;
      if (!Array.isArray(scopes) || scopes.length < 1 || scopes.some((scope) => !allowedScopes.has(scope))) {
        throw new Error('GOOGLE_OAUTH_SCOPE_ESCALATION');
      }
      return tokenStore.save(input);
    }
  });
    ...
    if (pathname === START_PATH) {
      ...
      try {
        const started = await flowRuntime.start({
          ...
          extraParams: { access_type: 'offline', include_granted_scopes: 'false', prompt: 'consent' }
        });
