...
function normalizeRecipients(value) {
  if (!Array.isArray(value) || value.length < 1 || value.length > MAX_RECIPIENTS) fail('INVALID_GMAIL_SEND_RECIPIENTS');
  const recipients = [];
  const seen = new Set();
  for (const item of value) {
    const address = typeof item === 'string' ? item.trim() : '';
    if (!address || address.length > 254 || /[\r\n\0]/.test(address) || !EMAIL_PATTERN.test(address)) fail('INVALID_GMAIL_SEND_RECIPIENT');
...
export function normalizeGmailSendRequest(input = {}) {
