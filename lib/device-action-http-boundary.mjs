...
function publicExecution(result) {
  ...
  if (value.action === 'app.open' && typeof value.appId === 'string' && APP_ID_PATTERN.test(value.appId)) return { ok: true, action: 'app.open', appId: value.appId };
...
export function createDeviceActionHttpBoundary({ runtime } = {}) {
   ...
   * @param {{ agent?: any; principal?: unknown; traceId?: string }} [context]
   */
  async function handle({ method, pathname, body } = {}, { agent, principal, traceId } = {}) {
    try {
      ...
      if (verb === 'POST' && pathname === `${BASE_PATH}/reviews`) {
        ...
        return result?.ok === true ? { status: 201, body: { ok: true, review: publicReview(result.review) } } : publicFailure(result);
      }
