...
export function createScheduleWorker({ store, registry, executeAgentTask, now = () => new Date(), retryDelayMs = 60_000, maxBatch = 4 }: { store?: any; registry?: any; executeAgentTask?: Function; now?: () => Date; retryDelayMs?: number; maxBatch?: number; } = {}) {
  ...
  function retryAtFromResult(value: string | number | Date) {
  ...
  async function runDue({ limit = batchLimit }: { limit?: number } = {}) {
    ...
    for (const schedule of claimed) {
      ...
      if (!agent) {
        ...
        results.push({ scheduleId: schedule.scheduleId, ok: false as const, error: 'SCHEDULE_AGENT_NOT_FOUND' });
        continue;
      }
      ...
      try {
        ...
        result = { ok: false as const, error: 'SCHEDULE_EXECUTION_FAILED' };
      }

      if (result?.ok) {
        await store.complete(schedule.scheduleId);
        results.push({ scheduleId: schedule.scheduleId, ok: true as const });
        continue;
      }
      ...
      results.push({ scheduleId: schedule.scheduleId, ok: false as const, error, retryScheduled: canRetry });
    }
