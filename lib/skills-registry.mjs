...
// Skill yetkisi kesişimdir: ajanın zaten sahip olduğu araçların dışına çıkan
// hiçbir araç verilmez. Ajanın izinli olduğu araçlardan en az biri varsa skill
// bu daraltılmış kümeyle çalışır; hiçbiri yoksa skill tümüyle reddedilir
// (`SKILL_TOOL_ESCALATION`). Böylece bir skill ne yetki kazanır ne de sırf
// yardımcı bir araç eksik diye tamamen kullanılamaz hâle gelir.
function classifyTools(agent, tools, approvalGranted) {
  ...
  if (!granted.length && !approvalPending.length) fail(`SKILL_TOOL_ESCALATION:${denied[0]}`);
  return { granted: Object.freeze(granted), approvalPending, denied };
}
...
export function createSkillsRegistry({ allowedProjects = [] } = {}) {
  ...
  function listForAgent(agent) {
    ...
    for (const entry of skills.values()) {
      ...
      // Ajanın hiçbir aracını çalıştıramadığı skill listelenmez; en az bir araç
      // izinli veya onay bekliyorsa skill kesişim kümesiyle görünür kalır.
