...
function classifyTools(agent, tools, approvalGranted) {
  ...
  // Skill hiçbir zaman ajan policy'sinin üstüne yetki eklemez: reddedilen araç
  // sessizce düşürülür ve prompt yalnız gerçekten verilen araçları duyurur.
  // Ancak hiçbir araç verilemiyorsa skill yetki yükseltmeye çalışıyor demektir.
  if (!granted.length && !approvalPending.length && denied.length) fail(`SKILL_TOOL_ESCALATION:${denied[0]}`);
...
export function createSkillsRegistry({ allowedProjects = [] } = {}) {
  ...
  function listForAgent(agent) {
    ...
    for (const entry of skills.values()) {
      ...
      // Ajan skill'in araçlarından en az birini kullanabiliyorsa skill görünür;
      // hiçbirini kullanamıyorsa modele hiç duyurulmaz.
