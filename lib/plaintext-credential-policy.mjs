// `\b` does not fire between `_` and a letter, so a `\b`-anchored name missed the
// most common env-var spellings entirely — NVIDIA_API_KEY=..., GITHUB_ACCESS_TOKEN=...
// The boundary here treats `_` (and any other non-alphanumeric) as a separator.
const CREDENTIAL_ASSIGNMENT_PATTERN = /(?:^|[^\p{L}\p{N}])(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|auth[_ -]?token|password|passwd|client[_ -]?secret|secret)(?![\p{L}\p{N}])\s*(?:=|:)\s*["']?[^\s"']{6,}/iu;
