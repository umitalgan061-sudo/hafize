// `NVIDIA_API_KEY=...` gibi ön ekli ortam değişkeni adlarını da yakalamak için
// anahtar kelimenin önünde `_`/`-` ile biten bir tanımlayıcı ön eki kabul edilir.
const CREDENTIAL_ASSIGNMENT_PATTERN = /(?:\b|_)(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|auth[_ -]?token|password|passwd|client[_ -]?secret|secret)\b\s*(?:=|:)\s*["']?[^\s"']{6,}/i;
...
export function isPlaintextCredentialField(name, value) {
  ...
  // `nvidiaApiKey` / `GOOGLE_REFRESH_TOKEN` gibi ön ekli alan adları da
  // credential alanıdır; sondaki anahtar kelime eşleşmesi yeterlidir.
  const isCredentialName = CREDENTIAL_FIELD_NAMES.has(normalized)
    || [...CREDENTIAL_FIELD_NAMES].some((field) => normalized.endsWith(field));
  if (!isCredentialName) return false;
