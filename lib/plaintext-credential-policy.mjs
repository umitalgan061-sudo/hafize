// `\b`, credential adının önündeki `_` veya `.` ayırıcısını sınır saymaz; bu
// yüzden `NVIDIA_API_KEY=...` gibi ön ekli adlar hiç yakalanmıyordu. Sınırı
// yalnız harf/rakam komşuluğuna göre tanımlamak ön ekli adları da kapsar ve
// "secretary" gibi kelime içi eşleşmeleri dışarıda bırakmayı sürdürür.
const CREDENTIAL_ASSIGNMENT_PATTERN = /(?<![a-z0-9])(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|auth[_ -]?token|password|passwd|client[_ -]?secret|secret)(?![a-z0-9])\s*(?:=|:)\s*["']?[^\s"']{6,}/i;
