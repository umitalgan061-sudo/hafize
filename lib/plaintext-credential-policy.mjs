// `\b` would not fire inside a prefixed env var name such as NVIDIA_API_KEY,
// because `_` is itself a word character; underscores and dashes are treated as
// separators here so vendor-prefixed assignments are still detected.
const CREDENTIAL_ASSIGNMENT_PATTERN = /(?:^|[^a-z0-9])(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|auth[_ -]?token|password|passwd|client[_ -]?secret|secret)\s*(?:=|:)\s*["']?[^\s"']{6,}/i;
