// Anahtar adı çoğu zaman bir sağlayıcı öneki taşır (NVIDIA_API_KEY, GITHUB_TOKEN).
// Baştaki `\b` bu biçimi kaçırıyordu: `_` de kelime karakteri olduğu için
// `NVIDIA_API_KEY` içinde `API` öncesinde sınır oluşmuyor. Önek açıkça tüketilir.
// Çıplak `token` sözcüğü sıradan metinde de geçtiği için yalnız önekli
// biçimde (GITHUB_TOKEN, slack-token) credential sayılır; yanlış pozitifle
// meşru kullanıcı görevlerini engellememek için kapsam dar tutulur.
const CREDENTIAL_ASSIGNMENT_PATTERN = /(?:^|[^a-z0-9])(?:(?:[a-z0-9]+[_-])*(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|auth[_ -]?token|password|passwd|client[_ -]?secret|secret)|(?:[a-z0-9]+[_-])+token)\b\s*(?:=|:)\s*["']?[^\s"']{6,}/i;
