// (?<![a-z0-9]) instead of \b so prefixed names such as NVIDIA_API_KEY or
// github_access_token are matched too: `_` is a word character, so \b never
// fires between the prefix and the credential word.
const CREDENTIAL_ASSIGNMENT_PATTERN = /(?<![a-z0-9])(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|auth[_ -]?token|password|passwd|client[_ -]?secret|secret)\b\s*(?:=|:)\s*["']?[^\s"']{6,}/i;
...
function isCredentialFieldName(name) {
  ...
  // Suffix match so nvidiaApiKey / github_access_token are treated like the
  // bare field names they end with.
  for (const known of CREDENTIAL_FIELD_NAMES) if (normalized === known || normalized.endsWith(known)) return true;
  return false;
}
