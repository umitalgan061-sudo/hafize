// `NVIDIA_API_KEY=...` gibi önekli isimler de yakalanmalıdır: `_` bir kelime karakteri
// olduğu için düz `\b` öneki bu yaygın env-var biçimini kaçırıyordu.
const CREDENTIAL_ASSIGNMENT_PATTERN = /(?:^|[^a-z0-9])(?:[a-z0-9]+[_-])*(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|auth[_ -]?token|password|passwd|client[_ -]?secret|secret)\b\s*(?:=|:)\s*["']?[^\s"']{6,}/i;
...
export function isPlaintextCredentialField(name, value) {
  ...
  // `nvidiaApiKey`, `github_access_token` gibi önekli alan adları da credential sayılır.
  const isCredentialName = CREDENTIAL_FIELD_NAMES.has(normalized)
    || [...CREDENTIAL_FIELD_NAMES].some((field) => normalized.length > field.length && normalized.endsWith(field));
