// `\b` does not fire after an underscore, so env-style names such as NVIDIA_API_KEY=…
// slipped past the guard; a non-alphanumeric prefix covers both `api key:` and `_API_KEY=`.
const CREDENTIAL_ASSIGNMENT_PATTERN = /(?:^|[^a-z0-9])(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|auth[_ -]?token|password|passwd|client[_ -]?secret|secret)s?\s*(?:=|:)\s*["']?[^\s"']{6,}/i;
