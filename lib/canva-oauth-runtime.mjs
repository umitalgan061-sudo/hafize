...
export function createCanvaOAuthRuntime({ clientId, redirectUri, flowRuntime = createOAuthFlowRuntime() } = {}) {
  ...
  async function start(input) {
    ...
    const started = await flowRuntime.start({ provider: 'canva', authorizationEndpoint: CANVA_AUTHORIZATION_ENDPOINT, clientId: safeClientId, redirectUri: safeRedirectUri, scopes: policy.scopes });
  ...
  async function finish(callback) {
    const result = await flowRuntime.finish(callback);
