...
function fail(error: string, status: number = 400) {
  return { status, body: { ok: false as const, error } };
}

/**
 * Fazladan alan taşıyan gövdeyi reddeder.
 */
function exactObject(value: unknown, allowedFields: ReadonlySet<string>, label: string): Record<string, any> {
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new Error(`INVALID_DEVICE_HTTP_${label}`);
  for (const key of Object.keys(value)) if (!allowedFields.has(key)) throw new Error(`INVALID_DEVICE_HTTP_${label}`);
  return (value as Record<string, any>);
}

function normalizeRequest(value: Record<string, any> | null | undefined) {
...
function reviewIdFromPath(pathname: string, suffix: string = '') {
...
function publicFailure(result: Record<string, any> | null | undefined) {
  const error = typeof result?.error === 'string' && result.error.startsWith('DEVICE_') ? result.error : 'DEVICE_REQUEST_FAILED';
  // `reason` yalnızca onay gerektiren arızalarda eklenir.
  const body: { ok: false; error: string; reason?: string } = { ok: false as const, error };
  if (result?.reason === 'approval_required') body.reason = 'approval_required';
  return { status: statusForError(error), body };
}

function safeReviewTarget(action: string, value: unknown) {
  ...
  let url;
  try { url = new URL(value); } catch { throw new Error('INVALID_DEVICE_HTTP_RUNTIME_RESULT'); }
  if (url.protocol !== 'https:' || url.username || url.password || url.search || url.hash) throw new Error('INVALID_DEVICE_HTTP_RUNTIME_RESULT');
  return `${url.origin}${url.pathname}`;
}

function publicReview(review: Record<string, any>) {
  ...
  if (!id || !action || !title || title.length > 200 || !Number.isFinite(expiresAt) || review.requiresExplicitConfirmation !== true) throw new Error('INVALID_DEVICE_HTTP_RUNTIME_RESULT');
  // `detail` isteğe bağlıdır: çalışma zamanı her zaman göndermiyor.
  const output: {
    id: string;
    action: unknown;
    title: string;
    ...
    detail?: string;
  } = { id, action, title, target: safeReviewTarget(action, review.target), expiresAt, requiresExplicitConfirmation: true };
  if (typeof review.detail === 'string' && review.detail) output.detail = review.detail.slice(0, 500);
  return Object.freeze(output);
}

function publicExecution(result: Record<string, any> | null | undefined) {
  const value = result?.value;
  if (!value || typeof value !== 'object' || Array.isArray(value) || value.ok !== true) throw new Error('INVALID_DEVICE_HTTP_RUNTIME_RESULT');
  if (value.action === 'system.info') {
    ...
    const safeInfo: { platform?: string; arch?: string; [key: string]: string | undefined } = {};
    for (const [key, fieldValue] of Object.entries(info)) {
      if (!SYSTEM_INFO_FIELDS.has(key) || typeof fieldValue !== 'string' || !fieldValue || fieldValue.length > 200) throw new Error('INVALID_DEVICE_HTTP_RUNTIME_RESULT');
      safeInfo[key] = fieldValue;
    }
    if (!safeInfo.platform || !safeInfo.arch) throw new Error('INVALID_DEVICE_HTTP_RUNTIME_RESULT');
    return { ok: true as const, action: 'system.info', info: safeInfo };
  }
  if (value.action === 'browser.open') return { ok: true as const, action: 'browser.open' };
  if (value.action === 'app.open' && typeof value.appId === 'string' && APP_ID_PATTERN.test(value.appId)) return { ok: true as const, action: 'app.open', appId: value.appId };
  throw new Error('INVALID_DEVICE_HTTP_RUNTIME_RESULT');
}

/**
 * Cihaz eylemleri için HTTP sınırı. Onay jetonu hiçbir yanıtta görünmez.
 */
export function createDeviceActionHttpBoundary({ runtime }: { runtime?: any } = {}) {
  if (!runtime || typeof runtime.executeReadOnly !== 'function' || typeof runtime.beginReview !== 'function' || typeof runtime.confirmAndExecute !== 'function' || typeof runtime.cancelReview !== 'function') {
    throw new Error('INVALID_DEVICE_HTTP_BOUNDARY:runtime');
  }

  async function handle({ method, pathname, body }: { method?: string; pathname?: string; body?: unknown } = {}, { agent, principal, traceId }: { agent?: any; principal?: unknown; traceId?: string } = {}) {
    try {
      ...
      if (verb === 'POST' && pathname === `${BASE_PATH}/reviews`) {
        ...
        return result?.ok === true ? { status: 201, body: { ok: true as const, review: publicReview(result.review) } } : publicFailure(result);
      }
...
export const DEVICE_ACTION_HTTP_CONTRACT = Object.freeze({
  basePath: BASE_PATH,
  routes: Object.freeze(['POST /api/device/system-info', 'POST /api/device/reviews', 'POST /api/device/reviews/:reviewId/confirm', 'DELETE /api/device/reviews/:reviewId']),
