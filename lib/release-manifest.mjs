...
export function createReleaseManifest({ version, commit, readiness, checks = [] } = {}) {
  ...
  const normalizedChecks = Array.isArray(checks)
    ? checks.filter((check) => check && typeof check === 'object').map((check) => Object.freeze({ name: text(check.name, 120), pass: check.pass === true }))
    : [];
  ...
  return Object.freeze({
    ...
    releaseable: failed.length === 0 && readiness.ready === true
  });
}
