...
} from './memory-retrieval-boundary.mts';
import { rankMemoryRecords } from './memory-quality.mts';
...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null }: { maxEntries?: number; now?: () => Date; createId?: () => string; initialSnapshot?: Record<string, any> | null } = {}) {
  ...
  function read(input) {
    ...
    // A query returns matches only: recency alone must not pull unrelated
    // personal records into a search result (or into model context).
