...
function cancelled() {
  const error = new Error('CANVA_READ_FAILED:cancelled');
...
async function waitForCaller(work, signal) {
  assertSignal(signal);
  if (signal === null) return await work;
...
function scopesOf(record) {
  if (!Array.isArray(record.scopes) || record.scopes.length > MAX_TOKEN_SCOPES) fail('token.scopes');
  const scopes = new Set();
  for (const raw of record.scopes) {
    const scope = text(raw, 'token.scope', { max: 128 });
    if (!SCOPE_PATTERN.test(scope) || scopes.has(scope)) fail('token.scope');
    scopes.add(scope);
  }
  return scopes;
}
...
export function createCanvaReadClient({
  tokenStore,
  ...
  if (!Number.isSafeInteger(requestTimeoutMs) || requestTimeoutMs < 1 || requestTimeoutMs > 120_000) fail('requestTimeoutMs');

  async function read(input = {}) {
    if (!input || Array.isArray(input) || typeof input !== 'object') fail('input');
    const { ownerId, operation, params, signal = null } = input;
    ...
    assertSignal(signal);
    const record = await waitForCaller(tokenStore.load({ ownerId: owner, provider: 'canva' }), signal);
    assertSignal(signal);
    ...
    if (serialized.includes(accessToken)) throw new Error('CANVA_READ_FAILED:response');
...
export const CANVA_READ_LIMITS = Object.freeze({
  maxJsonBytes: MAX_JSON_BYTES,
