...
export function createCanvaReadClient({ tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now(), maxJsonBytes = MAX_JSON_BYTES } = {}) {
  ...
  async function read(request = {}) {
    // null/dizi/skaler istek de sınırda tipli hata üretmeli; ham TypeError sızmamalı.
    const { ownerId, operation, params } = strictObject(request, 'request', new Set(['ownerId', 'operation', 'params']));
