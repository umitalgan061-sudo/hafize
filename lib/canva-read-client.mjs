...
export function createCanvaReadClient({ tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now(), maxJsonBytes = MAX_JSON_BYTES } = {}) {
  ...
  async function read(input) {
    // null/dizi gibi geçersiz girdiler de doğrulama hatası üretmelidir;
    // varsayılan parametre yalnızca undefined için devreye girer.
    const { ownerId, operation, params } = input && !Array.isArray(input) && typeof input === 'object' ? input : {};
