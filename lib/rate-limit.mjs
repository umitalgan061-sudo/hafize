export function createRateLimiter({ windowMs = 60_000, max = 60, maxConcurrent = 0, maxEntries = 10_000 } = {}) {
  ...
  function check(key, now = Date.now()) {
    ...
    if (!entry) {
      ...
      // Requests that are still in flight keep consuming the new window's quota,
      // so a long-running caller cannot win a full fresh quota by straddling a
      // window boundary.
      entry.count = entry.concurrent;
