export function createRateLimiter({ windowMs = 60_000, max = 60, maxConcurrent = 0, maxEntries = 10_000 } = {}) {
  ...
  function check(key, now = Date.now()) {
    ...
    // Every attempt that passes the quota gate consumes quota, including the ones the
    // concurrency gate rejects below: otherwise a caller parked at the concurrency limit
    // could retry without bound inside the same window.
    entry.count += 1;
