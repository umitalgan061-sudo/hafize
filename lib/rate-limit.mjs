/**
 * Kota aşıldığında `retryAfterSeconds`, izin verildiğinde `release()` taşıyan
 * karar. `concurrent`, reddin eşzamanlılık sınırından geldiğini söyler.
 *
 * @typedef {Readonly<{ ok: false; retryAfterSeconds: number; concurrent?: boolean }>} RateLimitDenied
 * @typedef {Readonly<{ ok: true; retryAfterSeconds?: undefined; concurrent?: undefined; release: () => void }>} RateLimitAllowed
 * @typedef {RateLimitDenied | RateLimitAllowed} RateLimitDecision
 *
 * @typedef {{ count: number; concurrent: number; resetAt: number }} RateLimitEntry
 */

/**
 * Bellek içi, sınırlı kayıt sayılı hız sınırlayıcı.
 *
 * @param {{ windowMs?: number; max?: number; maxConcurrent?: number; maxEntries?: number }} [options]
 */
