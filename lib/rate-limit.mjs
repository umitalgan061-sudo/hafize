export function createRateLimiter({ windowMs = 60_000, max = 60, maxConcurrent = 0, maxEntries = 10_000 } = {}) {
  ...
  function check(key, now = Date.now()) {
    ...
    if (!entry) {
      ...
      // aksi hâlde tek bir uzun isteği açık tutmak kotayı bedavaya yeniler ve
      // eşzamanlılık sınırına takılan çağrılar limiti aşabilirdi.
    ...
    return Object.freeze({
      ...
      release() {
        ...
        // Date.now() ile karşılaştırmak test/simülasyon saatlerinde hâlâ geçerli
        // girdileri siliyordu. Süresi dolmuş girdileri prune() toplar.
      }
    });
  }
