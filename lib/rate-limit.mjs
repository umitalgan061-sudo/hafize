export function createRateLimiter({ windowMs = 60_000, max = 60, maxConcurrent = 0, maxEntries = 10_000 } = {}) {
  ...
  function check(key, now = Date.now()) {
    ...
    // Every attempt that passes the quota gate consumes quota, including one that is
    // then turned away for concurrency: otherwise a caller can hammer the endpoint
    // while a slot is busy and never spend its budget.
    entry.count += 1;
