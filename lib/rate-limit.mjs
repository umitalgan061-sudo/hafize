export function createRateLimiter({ windowMs = 60_000, max = 60, maxConcurrent = 0, maxEntries = 10_000 } = {}) {
  ...
  function check(key, now = Date.now()) {
    ...
    if (maxConcurrent > 0 && entry.concurrent >= maxConcurrent) {
      // Eşzamanlılık nedeniyle reddedilen deneme de pencere kotasını tüketir:
      // aksi hâlde bir istemci slotu tutarken sınırsız sayıda bedava istek
      // deneyebilir ve kota kontrolü o istemci için işlevsiz kalır.
      entry.count += 1;
