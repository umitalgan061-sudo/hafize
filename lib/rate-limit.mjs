export function createRateLimiter({ windowMs = 60_000, max = 60, maxConcurrent = 0, maxEntries = 10_000 } = {}) {
  ...
  function check(key, now = Date.now()) {
    ...
    if (maxConcurrent > 0 && entry.concurrent >= maxConcurrent) {
      // A request refused for over-concurrency still cost the server the work of
      // accepting, authenticating and classifying it, so it consumes the window
      // quota too. Otherwise a caller could hammer an endpoint indefinitely at
      // zero quota cost simply by keeping every attempt in flight.
      entry.count += 1;
