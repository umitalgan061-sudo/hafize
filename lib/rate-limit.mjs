export function createRateLimiter({ windowMs = 60_000, max = 60, maxConcurrent = 0, maxEntries = 10_000 } = {}) {
  ...
  function check(key, now = Date.now()) {
    ...
    // Charge the quota before the concurrency gate: a caller that keeps hitting the
    // concurrency limit is still making requests, so those attempts must consume
    // the window budget instead of being retried for free.
    entry.count += 1;
    ...
    return Object.freeze({
      ...
      release() {
        ...
        // Eviction is left to prune(), which uses the caller-supplied clock. Comparing
        // against Date.now() here could drop a live entry — and reset its quota — for
        // any caller that drives the limiter with an injected or offset clock.
      }
    });
  }
