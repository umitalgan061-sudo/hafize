...
function thrownRunError(error, signal) {
...
export function createScheduledAgentExecutor({
  ...
  function completeWithSignal(payload, signal) {
    if (!signal) return complete(payload, signal);
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise.resolve()
        .then(() => {
          ...
          return complete(payload, signal);
        })
  ...
  async function executeAgentTask({ traceId, agent, task, signal = null } = {}) {
    ...
    try {
      result = await runAgentTask({
        ...
        complete: (payload, childSignal) => completeWithSignal(payload, childSignal || signal),
        nvidiaConfigured: Boolean(nvidiaConfigured),
        githubReadConfigured: Boolean(githubReadConfigured),
        githubReadFile
      });
    } catch (error) {
      result = { ok: false, error: thrownRunError(error, signal) };
    }

    if (!result?.ok) {
      const error = signal?.aborted ? 'SCHEDULE_AGENT_RUN_CANCELLED' : cleanErrorCode(result?.error);
