 ...
 *   githubReadFile?: Function;
 *   runAgentTask?: Function;
 * }} [options]
 */
export function createScheduledAgentExecutor({
  ...
  /**
   * @param {{ traceId?: string; agent?: any; task?: string }} [input]
   */
  async function executeAgentTask({ traceId, agent, task } = {}) {
    ...
    if (containsPlaintextCredential(safeTask)) return { ok: false, error: 'SCHEDULE_AGENT_TASK_CREDENTIAL_BLOCKED' };
    ...
    if (containsPlaintextCredential(typeof result.content === 'string' ? result.content : '')) {
      runLedger.finish({ ok: false, detail: 'SCHEDULE_AGENT_RESULT_CREDENTIAL_BLOCKED' });
      return { ok: false, error: 'SCHEDULE_AGENT_RESULT_CREDENTIAL_BLOCKED', taskLedger: runLedger.snapshot() };
    }
