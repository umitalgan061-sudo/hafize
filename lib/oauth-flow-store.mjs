...
export function normalizeOAuthFlowState(state) {
  if (typeof state !== 'string' || !STATE_PATTERN.test(state)) throw new Error('INVALID_OAUTH_FLOW:state');
  return state;
}

export function normalizeOAuthFlowOwnerId(ownerId, { required = false } = {}) {
  if (ownerId == null || ownerId === '') {
    if (required) throw new Error('INVALID_OAUTH_FLOW:ownerId');
    return null;
  }
  const normalized = typeof ownerId === 'string' ? ownerId.trim() : '';
  if (!OWNER_PATTERN.test(normalized)) throw new Error('INVALID_OAUTH_FLOW:ownerId');
  return normalized;
}

export function normalizeOAuthFlow(input) {
  if (!input || Array.isArray(input) || typeof input !== 'object') throw new Error('INVALID_OAUTH_FLOW:input');
  const allowed = new Set(['state', 'verifier', 'provider', 'redirectUri', 'scopes', 'ownerId']);
  for (const key of Object.keys(input)) if (!allowed.has(key)) throw new Error('INVALID_OAUTH_FLOW:field');
  normalizeOAuthFlowState(input.state);
  ...
  if (typeof input.redirectUri !== 'string' || input.redirectUri.length > 2048) throw new Error('INVALID_OAUTH_FLOW:redirectUri');
  let redirectUri;
  ...
  return {
    ...
    scopes,
    ownerId: normalizeOAuthFlowOwnerId(input.ownerId)
  };
}
