...
const SAFE_LATE_STREAM_ERRORS = new Set([
...
function safeHeaderEntries(value) {
  ...
  for (const [rawName, rawValue] of Object.entries(value)) {
    ...
    if (!name || !text || name.length > 80 || text.length > 500 || /[\r\n]/.test(name + text) || FORBIDDEN_RESPONSE_HEADERS.has(lower)) {
...
function safeLateStreamError(error) {
  const code = typeof error?.code === 'string' ? error.code : '';
  return SAFE_LATE_STREAM_ERRORS.has(code) ? code : 'STREAM_INTERRUPTED';
}
...
export function createModelProviderNodeHttpRoute({
  ...
  sendJson,
  setSecurityHeaders,
  createStreamWriter = createSseNodeWriter
} = {}) {
  ...
  const streamWriterFactory = requireWriter(createStreamWriter, 'createStreamWriter');
  ...
  async function handle({ request, response, method, pathname, headers = {} } = {}) {
    ...
    if (!response || typeof response.end !== 'function' || typeof response.writeHead !== 'function') fail('INVALID_MODEL_PROVIDER_NODE_HTTP:response');

    const controller = new AbortController();
    const detachAbort = attachAbort(response, controller);
    try {
      const result = await providerRuntime.handle({ request, method, pathname, headers, signal: controller.signal });
      ...
      if (result.kind !== 'stream' || !isAsyncIterable(result.body)) fail('INVALID_MODEL_PROVIDER_NODE_HTTP_RESPONSE');

      secure(response);
      applyHeaders(response, result.headers);
      response.writeHead(result.status);
      const writer = streamWriterFactory({ response, signal: controller.signal });
      if (!writer || typeof writer.pipe !== 'function' || typeof writer.end !== 'function' || typeof writer.writeError !== 'function') {
        fail('INVALID_MODEL_PROVIDER_NODE_HTTP:streamWriter');
      }
      let interrupted = false;
      let aborted = false;
      let streamError = null;
      try {
        const pipeResult = await writer.pipe(result.body);
        aborted = pipeResult.aborted;
      } catch (error) {
        interrupted = true;
        streamError = safeLateStreamError(error);
        if (!controller.signal.aborted && writer.writable?.()) await writer.writeError(streamError);
      } finally {
        writer.end();
      }
      return Object.freeze({
        ...
        aborted: controller.signal.aborted || aborted,
        interrupted,
        streamError
      });
    } finally {
