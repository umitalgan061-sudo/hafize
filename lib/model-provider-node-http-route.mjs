  ...
  async function handle({ request, response, method, pathname, headers = {} } = {}) {
    ...
    try {
      ...
      try {
        ...
        interrupted = true;
        if (!controller.signal.aborted && writer.writable?.()) await writer.writeError('STREAM_INTERRUPTED');
      } finally {
        writer.end();
      }
      return Object.freeze({ matched: true, kind: 'stream', status: result.status, aborted: controller.signal.aborted || aborted, interrupted });
    } finally {
      detachAbort();
    }
  }
