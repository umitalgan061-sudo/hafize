...
function validLifecycle(lifecycle) {
  return typeof lifecycle?.start === 'function'
    && typeof lifecycle?.finish === 'function'
    && typeof lifecycle?.isCancelled === 'function';
}
...
export function createAgentDelegator({
  ...
  if (lifecycle != null && !validLifecycle(lifecycle)) throw new Error('INVALID_DELEGATION_RUNTIME:lifecycle');
  ...
  async function delegate(args, { depth = 0 } = {}) {
    ...
    // Alt koşu, iptal edilebilir ve eşzamanlılığı sınırlı bir yaşam döngüsüne bağlanır.
    let handle = null;
    if (lifecycle) {
      try {
        handle = lifecycle.start({ taskId: delegationTask.taskId, agentId: targetAgent.id });
      } catch (error) {
        const code = error?.code === 'AGENT_CONCURRENCY_EXCEEDED'
          ? 'DELEGATION_CONCURRENCY_EXCEEDED'
          : 'DELEGATION_CANCELLED';
        runLedger.recordDelegationFinish(delegationTask.taskId, { ok: false, error: code });
        return failed(code);
      }
    }
    ...
    // İptal kararı yarışı kazanır: geç gelen başarı sonucu kabul edilmez.
    if (lifecycle?.isCancelled(delegationTask.taskId)) {
      runLedger.recordDelegationFinish(delegationTask.taskId, { ok: false, error: 'DELEGATION_CANCELLED' });
      return failed('DELEGATION_CANCELLED');
    }

    if (!result?.ok) {
      const safeError = cleanText(result?.error, 120) || 'DELEGATED_AGENT_FAILED';
      lifecycle?.finish(delegationTask.taskId, { ok: false, error: safeError });
      runLedger.recordDelegationFinish(delegationTask.taskId, { ok: false, error: safeError });
      return failed(safeError);
    }

    lifecycle?.finish(delegationTask.taskId, { ok: true });
