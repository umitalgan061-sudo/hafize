...
export function createAgentDelegator({
  ...
  if (typeof runLedger?.recordDelegationStart !== 'function' || typeof runLedger?.recordDelegationFinish !== 'function') {
  ...
  async function delegate(args, { depth = 0 } = {}) {
    ...
    if (delegatedChildren >= maxFanOut) return failed('DELEGATION_FANOUT_EXCEEDED');
