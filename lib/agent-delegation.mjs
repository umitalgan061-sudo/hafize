...
export function createAgentDelegator({ registry, traceId, parentAgent, parentTaskId, runLedger, executeAgent, lifecycle = null, parentSignal = null } = {}) {
  ...
  if (typeof runLedger?.recordDelegationStart !== 'function' || typeof runLedger?.recordDelegationFinish !== 'function' || typeof runLedger?.snapshot !== 'function') throw new Error('INVALID_DELEGATION_RUNTIME:runLedger');
      ...
      if (lifecycle) {
        ...
        try {
          lifecycleRun = lifecycle.start({
            ...
            parentSignal,
            execute: ({ signal }) => executeAgent({ agent: targetAgent, task: handoff.task, traceId: traceId.trim(), depth: depth + 1, parentTaskId: delegationTask.taskId, signal })
          });
        } catch (error) {
          if (error?.code === 'AGENT_CONCURRENCY_EXCEEDED') throw Object.assign(new Error('DELEGATION_CONCURRENCY_EXCEEDED'), { code: 'DELEGATION_CONCURRENCY_EXCEEDED' });
          throw error;
        }
        ...
        else if (!result) result = failed(lifecycleRun.snapshot().error || 'DELEGATED_AGENT_FAILED');
      } else {
        result = await executeAgent({ agent: targetAgent, task: handoff.task, traceId: traceId.trim(), depth: depth + 1, parentTaskId: delegationTask.taskId });
      }
    } catch (error) {
      result = failed(error?.code === 'DELEGATION_CONCURRENCY_EXCEEDED' ? 'DELEGATION_CONCURRENCY_EXCEEDED' : 'DELEGATED_AGENT_FAILED');
    }
