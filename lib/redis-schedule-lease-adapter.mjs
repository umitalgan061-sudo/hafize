...
function cleanCompletionTtlMs(value) {
  if (!Number.isInteger(value) || value < MIN_COMPLETION_TTL_MS || value > MAX_COMPLETION_TTL_MS) {
    throw new Error('INVALID_REDIS_SCHEDULE_LEASE:completionTtlMs');
  }
  return value;
}
...
export function createRedisScheduleLeaseAdapter({
  redis,
  keyPrefix = 'hafize:schedule-lease',
  ...
  const completionTtl = cleanCompletionTtlMs(completionTtlMs);
  ...
  async function acquire({ scheduleId, holderId, leaseMs } = {}) {
    ...
    const result = cleanResult(await evalRedis(redis, ACQUIRE_SCRIPT, keysFor(prefix, id), [holder, ttl, completionTtl]));
  ...
  async function renew({ scheduleId, holderId, fence, leaseMs } = {}) {
    ...
    const ttl = cleanLeaseMs(leaseMs);
    const result = cleanResult(await evalRedis(redis, RENEW_SCRIPT, keysFor(prefix, id), [holder, safeFence, ttl, completionTtl]));
  ...
  async function complete({ scheduleId, holderId, fence, idempotencyKey } = {}) {
    ...
    const result = cleanResult(await evalRedis(
      redis,
      COMPLETE_SCRIPT,
      keysFor(prefix, id),
      [holder, safeFence, idempotency, completionTtl]
    ));
  ...
  async function release({ scheduleId, holderId, fence } = {}) {
    ...
    const holder = cleanId(holderId, 'holderId');
    const safeFence = cleanFence(fence);
    const result = cleanResult(await evalRedis(redis, RELEASE_SCRIPT, keysFor(prefix, id), [holder, safeFence, completionTtl]));
...
export const REDIS_SCHEDULE_COMPLETION_TTL_LIMITS = Object.freeze({
