...
const LOCAL_TOOL_PAYLOAD_FIELDS = Object.freeze(['tools', 'functions', 'tool_choice', 'function_call']);
...
function payloadHasToolFields(payload) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) return false;
  return LOCAL_TOOL_PAYLOAD_FIELDS.some((field) => Object.prototype.hasOwnProperty.call(payload, field));
}

function assertLocalToolBoundary(selected, payload, toolsRequired) {
  if (selected !== 'local') return;
  if (toolsRequired || payloadHasToolFields(payload)) fail('LOCAL_PROVIDER_TOOLS_UNSUPPORTED', 400);
}
