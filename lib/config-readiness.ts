export type ConfigReadinessState = 'ready' | 'degraded' | 'blocked';
export interface ConfigFinding { code: string; field: string }
export interface ConfigReadiness { state: ConfigReadinessState; production: boolean; publicRuntime: boolean; authRequired: boolean; findings: ReadonlyArray<ConfigFinding>; secretVariables: ReadonlyArray<string> }
...
function problem(code: string, field: string): ConfigFinding {
  return Object.freeze({ code, field });
}

export function evaluateConfigReadiness(env: Record<string, string | undefined> = process.env): ConfigReadiness {
  if (!env || typeof env !== 'object' || Array.isArray(env)) {
    return Object.freeze({
      state: 'blocked',
      findings: Object.freeze([problem('INVALID_ENV', 'env')]),
      production: false,
      publicRuntime: false,
      authRequired: true,
      secretVariables: SECRET_VARS
    });
  }
  ...
  if (booleanValue(env.HAFIZE_TRUST_PROXY, false) && !text(env.HAFIZE_PROXY_TRUST_DESCRIPTION)) {
    findings.push(problem('PROXY_TRUST_UNDOCUMENTED', 'HAFIZE_PROXY_TRUST_DESCRIPTION'));
  }
  for (const name of BOOLEAN_VARS) {
    if (text(env[name]) && !/^(1|0|true|false|yes|no|on|off)$/i.test(text(env[name]))) findings.push(problem('INVALID_BOOLEAN', name));
  }
  for (const name of SECRET_VARS) {
    if (typeof env[name] === 'string' && /[\r\n]/.test(env[name])) findings.push(problem('SECRET_CONTAINS_NEWLINE', name));
  }
