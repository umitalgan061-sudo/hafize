...
export type NormalizedToolCall = Readonly<{ id: string; name: string; arguments: string }>;

export type NormalizedUsage = Readonly<{ prompt_tokens?: number; completion_tokens?: number; total_tokens?: number }>;
...
const FINISH_REASONS = new Set((['stop', 'length', 'tool_calls', 'content_filter', 'unknown'] as ModelFinishReason[]));
...
function normalizeFinishReason(value: unknown): ModelFinishReason {
  ...
  const rawReason = (text(value, MAX_FINISH_REASON, 'INVALID_MODEL_FINISH_REASON').toLowerCase() as ModelFinishReason);
...
function normalizeToolCalls(toolCalls: unknown): readonly NormalizedToolCall[] {
 ...
 */
export function normalizeModelResponse(response: Record<string, any> = {}): Readonly<NormalizedModelResponse> {
 ...
 */
export function normalizeNvidiaChatCompletion(response: Record<string, any> = {}): Readonly<NormalizedModelResponse> {
  if (!response || typeof response !== 'object' || Array.isArray(response)) throw new Error('INVALID_MODEL_RESPONSE');
 ...
 */
export function isTerminalModelResponse(response: Record<string, any>): boolean {
