...
function upstreamFailure(error) {
  if (error?.code === 'CANVA_OAUTH_UPSTREAM_TIMEOUT') return new Error('CANVA_TOKEN_REVOKE_FAILED:timeout');
  if (error?.code === 'CANVA_OAUTH_UPSTREAM_CANCELLED') return new Error('CANVA_TOKEN_REVOKE_FAILED:cancelled');
  return error;
}

export function createCanvaTokenRevoke({ clientId, clientSecret, tokenStore, fetchImpl = globalThis.fetch, timeoutMs, AbortControllerImpl = globalThis.AbortController, setTimeoutImpl = globalThis.setTimeout, clearTimeoutImpl = globalThis.clearTimeout } = {}) {
  ...
  async function revoke({ ownerId, explicitUserIntent, signal } = {}) {
    ...
    let result;
    try {
      result = await upstream.postForm(REVOKE_ENDPOINT, {
        authorization,
    ...
    if (!result.ok) throw new Error('CANVA_TOKEN_REVOKE_FAILED:http');
