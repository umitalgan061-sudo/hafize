...
export function createCanvaTokenRevoke({ clientId, clientSecret, tokenStore, fetchImpl = globalThis.fetch } = {}) {
  ...
  async function revoke(request) {
    if (request != null && (typeof request !== 'object' || Array.isArray(request))) throw new Error('INVALID_CANVA_TOKEN_REVOKE:request');
    const { ownerId, explicitUserIntent } = request || {};
