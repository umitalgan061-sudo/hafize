...
function requestObject(value) {
  ...
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new Error('INVALID_CANVA_TOKEN_REVOKE:request');
  return value;
}
...
export function createCanvaTokenRevoke({ clientId, clientSecret, tokenStore, fetchImpl = globalThis.fetch } = {}) {
  ...
  async function revoke(request) {
    const { ownerId, explicitUserIntent } = requestObject(request);
