...
function normalizeItem(value: Record<string, any> | null | undefined) {
...
function normalizeQuery(input: Record<string, any> = {}) {
...
export function normalizeCalendarRead({ ownerId, query = {} }: { ownerId?: string; query?: Record<string, any> } = {}) {
...
export function normalizeCalendarItems({ ownerId, items }: { ownerId?: string; items?: unknown } = {}) {
  const safeOwnerId = text(ownerId, 200, 'INVALID_CALENDAR_OWNER');
...
export function calendarWriteApproval(action: unknown) {
...
export function assertCalendarWriteApproved({ action, approvalGranted = false }: { action?: string; approvalGranted?: boolean } = {}) {
