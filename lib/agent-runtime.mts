...
export async function loadAgentRegistry(fileUrl: URL | string = DEFAULT_REGISTRY_URL) {
...
export function listPublicAgents(registry: Record<string, any>): HafizeAgentSummary[] {
  return registry.agents.map(({ id, name, kind, description }) => ({ id, name, kind, description }));
}

/**
 *  Verilmezse kayıt dosyasındaki varsayılan ajan.
 */
export function resolveAgent(registry: Record<string, any>, agentId?: string) {
...
export function normalizeClientMessages(messages: unknown): HafizeWireMessage[] {
...
export function buildAgentSystemMessage(agent: Record<string, any>, traceId: string) {
  const lines = [
...
export function authorizeAgentTool(agent: Record<string, any>, toolName: string, { approvalGranted = false }: { approvalGranted?: boolean } = {}) {
