...
import { COUNTS_VIEW, normalizeScheduleListView, summarizeScheduleListOutput } from './schedule-list-summary.mjs';
...
function parseListQuery(request, serverLimit) {
  ...
  try {
    ...
    if (view === COUNTS_VIEW && (rawLimit != null || rawOffset != null || rawSnapshot != null || rawScope != null)) return null;
...
export function createScheduleHttpApi({
  ...
  async function handle({ request, method, pathname, headers } = {}) {
    ...
    if (root && verb === 'GET') {
      ...
      if (pagination.view === COUNTS_VIEW) {
        const metadataOnly = scopeCounts
          ? attachScheduleListScopeCounts({ ok: true, schedules: [], view: COUNTS_VIEW }, scopeCounts)
          : output;
        return commandResponse(metadataOnly, 200);
      }
