...
export function createScheduleHttpApi({ authenticator, commands, readJson } = {}) {
  ...
  async function handle({ request, method, pathname, headers } = {}) {
    ...
    if (root && verb === 'GET') {
      return commandResponse(commands.list({ principal: auth.principal }), 200);
    }
    if (root && verb === 'POST') {
      const input = await readJson(request);
      return commandResponse(commands.create({ principal: auth.principal, input }), 201);
    }
    if (!root && verb === 'DELETE') {
      return commandResponse(commands.cancel({ principal: auth.principal, scheduleId }), 200);
    }
    return response(405, { error: 'METHOD_NOT_ALLOWED' }, { Allow: root ? 'GET, POST' : 'DELETE' });
  }
