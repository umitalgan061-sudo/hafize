...
export function createScheduleHttpApi({ authenticator, commands, readJson } = {}) {
  ...
  async function handle(input) {
    // İstek yolundaki sınır asla fırlatmaz; tanımadığı girdi için `matched:false`
    // döner. `= {}` yalnız `undefined` için devreye girdiğinden `null` girdi
    // burada TypeError üretiyordu.
    const call = input && typeof input === 'object' && !Array.isArray(input) ? input : {};
    const { request, method, pathname, headers } = call;
