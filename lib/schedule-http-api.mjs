import { boundScheduleListOutput } from './schedule-list-boundary.mjs';
...
export function createScheduleHttpApi({
  ...
  async function handle({ request, method, pathname, headers } = {}) {
    ...
    if (root && verb === 'GET') {
      ...
      const bounded = boundScheduleListOutput(output, listLimit === undefined ? {} : { limit: listLimit });
      return commandResponse(bounded, 200);
    }
    if (root && verb === 'POST') {
