...
function commandErrorStatus(error) {
  ...
  if (error === 'SCHEDULE_NOT_CANCELLABLE' || error === 'SCHEDULE_NOT_EDITABLE') return 409;
...
export function createScheduleHttpApi({ authenticator, commands, readJson } = {}) {
  ...
  if (typeof commands?.create !== 'function' || typeof commands?.update !== 'function' || typeof commands?.list !== 'function' || typeof commands?.cancel !== 'function') throw new Error('INVALID_SCHEDULE_HTTP_API:commands');
  ...
  async function handle({ request, method, pathname, headers } = {}) {
    ...
    if (!root && !scheduleId) return response(404, normalizeApiError({ code: 'SCHEDULE_NOT_FOUND', status: 404, requestId }), requestId ? { 'X-Hafize-Request-Id': requestId } : {});
    let auth;
    try { auth = authenticator.authenticate({ headers }); } catch { auth = null; }
    if (!auth?.ok || !auth.principal) return response(401, normalizeApiError({ code: 'AUTH_REQUIRED', status: 401, requestId }), { ...(requestId ? { 'X-Hafize-Request-Id': requestId } : {}), 'WWW-Authenticate': 'Bearer' });
    if (root && verb === 'GET') return commandResponse(await commands.list({ principal: auth.principal }), 200, requestId);
    ...
    if (!root && verb === 'PATCH') return commandResponse(await commands.update({ principal: auth.principal, scheduleId, input: await readJson(request) }), 200, requestId);
    if (!root && verb === 'DELETE') return commandResponse(await commands.cancel({ principal: auth.principal, scheduleId }), 200, requestId);
    const normalized = normalizeApiError({ code: 'INVALID_SCHEDULE_COMMAND', status: 405, requestId });
    return response(405, normalized, { ...(requestId ? { 'X-Hafize-Request-Id': requestId } : {}), Allow: root ? 'GET, POST' : 'PATCH, DELETE' });
  }
