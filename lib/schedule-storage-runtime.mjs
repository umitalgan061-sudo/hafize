...
export async function createScheduleStorageRuntime({
  ...
  if (!storeOptions || Array.isArray(storeOptions) || typeof storeOptions !== 'object') throw new Error('INVALID_SCHEDULE_STORAGE_RUNTIME:storeOptions');

  const config = readConfig({ env });
  if (!config) return Object.freeze({ store: createMemoryStore(storeOptions), durable: false });

  const key = config.key;
  try {
    const adapter = createEncryptedAdapter({ filePath: config.filePath, key, maxFileBytes: config.maxFileBytes });
    ...
    await store.open();
    return Object.freeze({ store, durable: true, maxFileBytes: config.maxFileBytes });
  } catch {
