...
import { decryptOAuthTokenRecord, encryptOAuthTokenRecord } from './oauth-token-encryption.mts';
...
function normalizeText(value: unknown, field: string, pattern: RegExp): string {
...
function recordKey(ownerId: string, provider: string) {
  return createHash('sha256').update(`${ownerId}\0${provider}`, 'utf8').digest('hex');
}

/**
 * Yol kaçışını engeller: hedef her zaman kökün altında kalmalıdır.
 */
...
/**
 * OAuth jetonlarını şifreli olarak diskte tutar.
 */
export function createOAuthTokenFileStore({ directory, key, maxFileBytes, fs = { chmod, mkdir, open, readFile, rename, rm, stat }, createNonce = randomBytes }: { directory?: string; key?: unknown; maxFileBytes?: number; fs?: Record<string, Function>; createNonce?: (size: number) => Uint8Array; } = {}) {
  ...
  function pathFor(ownerId: string, provider: string) {
  ...
  async function save({ ownerId, provider, tokenRecord }: { ownerId?: string; provider?: string; tokenRecord?: unknown } = {}) {
  ...
  async function load({ ownerId, provider }: { ownerId?: string; provider?: string } = {}) {
    const target = pathFor(ownerId, provider);
    let metadata;
...
/**
 * Silme her zaman açık kullanıcı isteği gerektirir.
 */
  async function remove({ ownerId, provider, explicitUserIntent }: { ownerId?: string; provider?: string; explicitUserIntent?: boolean } = {}) {
    const target = pathFor(ownerId, provider);
