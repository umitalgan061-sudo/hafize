...
import { createPersonalMemoryApprovalBoundary } from './personal-memory-approval.mjs';
...
export async function createPersonalMemoryControlRuntime({
  ...
  createApprovalBoundary = createPersonalMemoryApprovalBoundary
} = {}) {
  ...
  const ownerSecret = ownerKey(keyText);
  const bearerAuthenticator = bearerConfigured
    ...
    : null;
  const resolver = factory(createOwnerResolver, 'createOwnerResolver')({ key: ownerSecret });
  const memory = factory(createMemoryRuntime, 'createMemoryRuntime')({ env });
  const approval = factory(createApprovalBoundary, 'createApprovalBoundary')({ secret: ownerSecret });
  ...
  if (typeof approval?.prepare !== 'function' || typeof approval?.consume !== 'function') fail('approval');
  ...
  return Object.freeze({ configured: true, authenticate, authorizeMutation, memory, approval });
}
