...
import { createPersonalMemoryWriteApprovalService } from './personal-memory-write-approval.mjs';
...
export async function createPersonalMemoryControlRuntime({
  ...
  createWriteApproval = createPersonalMemoryWriteApprovalService
} = {}) {
  ...
  const writeApproval = factory(createWriteApproval, 'createWriteApproval')({ memoryKeyBase64: memoryKey });
  ...
  if (typeof writeApproval?.issue !== 'function' || typeof writeApproval?.consume !== 'function') fail('writeApproval');
  ...
  return Object.freeze({ configured: true, authenticate, authorizeMutation, memory, writeApproval });
}
