...
const RESERVED_PARAMS = new Set(['client_id','redirect_uri','response_type','scope','state','code_challenge','code_challenge_method']);
const ALLOWED_AUTH_FIELDS = new Set(['authorizationEndpoint','clientId','redirectUri','scopes','state','codeChallenge','extraParams']);

export interface OAuthPkceRandomSource { (size: number): Uint8Array | Buffer; }
export interface PkceVerifierOptions { readonly random?: OAuthPkceRandomSource; }
export interface OAuthAuthorizationInput {
  ...
  readonly extraParams?: Readonly<Record<string,string>> | null;
}
...
function boundedString(value: unknown, label: string, limits: { readonly min?: number; readonly max?: number } = {}): string {
  const min = limits.min ?? 1;
  const max = limits.max ?? 512;
...
function normalizeScopes(value: unknown): string[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > 16) throw new Error('INVALID_OAUTH_PKCE:scopes');
...
function normalizeExtraParams(value: unknown): Record<string,string> {
  if (value == null) return {};
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new Error('INVALID_OAUTH_PKCE:extraParams');
  ...
  for (const [key, raw] of entries) {
    ...
    if (RESERVED_PARAMS.has(key)) throw new Error('INVALID_OAUTH_PKCE:reservedParam');
    output[key] = boundedString(raw, 'extraParam', { max: 256 });
  }
  return output;
}

export function createPkceVerifier({ random = randomBytes }: PkceVerifierOptions = {}): string {
...
export function createOAuthState({ random = randomBytes }: PkceVerifierOptions = {}): string {
  if (typeof random !== 'function') throw new Error('INVALID_OAUTH_PKCE:random');
  let raw: Buffer;
...
export function buildOAuthAuthorizationUrl(input: OAuthAuthorizationInput): string {
...
export const OAUTH_PKCE_LIMITS = Object.freeze({ verifierMin: 43, verifierMax: 128, maxScopes: 16, maxExtraParams: 8 });
