...
export function createGitHubReadFile({ token = '', allowedRepositories = [], baseUrl = DEFAULT_GITHUB_API, fetchImpl = globalThis.fetch, maxFileBytes = DEFAULT_MAX_FILE_BYTES }: { token?: string; allowedRepositories?: string[] | string; baseUrl?: string; fetchImpl?: HafizeFetch; maxFileBytes?: number; } = {}) {
  ...
  // Girdi bir araç sınırı: çağıranın verdiği her alan burada doğrulanıyor.
  return async function githubReadFile(input: UnvalidatedInput = {}) {
