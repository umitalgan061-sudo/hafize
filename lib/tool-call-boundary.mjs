...
// Boundary rejections carry their code on the error, not only in the message:
// callers map failures with sanitizeToolError(), which reads `error.code` and
// would otherwise flatten every rejection into a generic TOOL_EXECUTION_FAILED.
function fail(code) {
