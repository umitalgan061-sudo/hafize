...
// Boundary rejections carry a `code`, so sanitizeToolError() reports the real
// reason instead of collapsing every rejection into TOOL_EXECUTION_FAILED.
function fail(code) {
...
export function parseToolArguments(raw) {
  ...
  let value;
  // A model emitting malformed JSON is an invalid tool argument body, not a
  // malformed client request; the parser text itself never leaves the boundary.
  try { value = JSON.parse(raw); } catch { fail('INVALID_TOOL_ARGUMENTS'); }
