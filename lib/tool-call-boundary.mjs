...
function text(value, max, code) {
  ...
  if (!result || result.length > max || result.includes('\0')) throw new Error(code);
  return result;
}

export function normalizeToolCall(toolCall) {
  if (!toolCall || typeof toolCall !== 'object' || Array.isArray(toolCall)) throw new Error('INVALID_TOOL_CALL');
  ...
  if (typeof rawArguments !== 'string') throw new Error('INVALID_TOOL_ARGUMENTS');
  if (rawArguments.length > MAX_ARGUMENTS_LENGTH) throw new Error('TOOL_ARGUMENTS_TOO_LARGE');
  return Object.freeze({ id, type: 'function', function: Object.freeze({ name, arguments: rawArguments }) });
}

export function parseToolArguments(raw) {
  if (typeof raw !== 'string' || raw.length > MAX_ARGUMENTS_LENGTH) throw new Error('TOOL_ARGUMENTS_TOO_LARGE');
