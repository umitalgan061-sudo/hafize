...
// Boundary errors carry a machine-readable code so the tool runtime can report
// the real reason instead of collapsing everything into TOOL_EXECUTION_FAILED.
function fail(code) {
...
function text(value, max, code) {
  ...
  if (!result || result.length > max || result.includes('\0')) throw fail(code);
  return result;
}

export function normalizeToolCall(toolCall) {
  if (!toolCall || typeof toolCall !== 'object' || Array.isArray(toolCall)) throw fail('INVALID_TOOL_CALL');
  ...
  if (typeof rawArguments !== 'string') throw fail('INVALID_TOOL_ARGUMENTS');
  if (rawArguments.length > MAX_ARGUMENTS_LENGTH) throw fail('TOOL_ARGUMENTS_TOO_LARGE');
  return Object.freeze({ id, type: 'function', function: Object.freeze({ name, arguments: rawArguments }) });
}

export function parseToolArguments(raw) {
  if (typeof raw !== 'string' || raw.length > MAX_ARGUMENTS_LENGTH) throw fail('TOOL_ARGUMENTS_TOO_LARGE');
  if (!raw.trim()) return Object.freeze({});
  const value = JSON.parse(raw);
  if (!value || Array.isArray(value) || typeof value !== 'object') throw fail('INVALID_TOOL_ARGUMENTS');
