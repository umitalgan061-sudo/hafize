...
export function createScheduleExecutionRuntime({ executor, lease = null, renewIntervalMs, createGuard = createScheduleLeaseGuardedExecutor } = {}) {
  ...
  let guarded;
  try {
    guarded = createGuard({ lease, executeAgentTask: executor.executeAgentTask, renewIntervalMs });
  } catch {
  ...
  return Object.freeze({ configured: Boolean(executor.configured), leaseGuarded: true, executeAgentTask: workerFacingExecutor(guarded.executeAgentTask) });
}
