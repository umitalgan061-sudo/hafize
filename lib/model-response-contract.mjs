...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Accept both the provider wire shape ({ function: { name, arguments } }) and the
    // already-normalized shape, so normalization stays idempotent.
    const call_fn = call.function && typeof call.function === 'object' && !Array.isArray(call.function) ? call.function : call;
    const id = text(call.id, 200, 'INVALID_MODEL_TOOL_CALL_ID');
    if (!id) throw new Error('INVALID_MODEL_TOOL_CALL_ID');
    const name = text(call_fn.name, 120, 'INVALID_MODEL_TOOL_CALL_NAME');
    if (!name) throw new Error('INVALID_MODEL_TOOL_CALL_NAME');
    const args = call_fn.arguments;
