...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Accept both the provider shape ({ function: { name, arguments } }) and an already
    // normalized call, so normalization stays idempotent for helpers such as
    // isTerminalModelResponse() that re-normalize a response they were handed.
    const fn = call.function && typeof call.function === 'object' && !Array.isArray(call.function) ? call.function : call;
