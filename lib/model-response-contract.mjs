...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Accept both the provider wire shape ({ function: { name, arguments } })
    // and this contract's own normalized shape, so normalize -> predicate ->
    // normalize round-trips instead of rejecting its own output.
