...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Provider shape ({ function: { name, arguments } }) and already-normalized shape
    // ({ name, arguments }) are both accepted so normalization stays idempotent:
    // isTerminalModelResponse() re-normalizes whatever it is handed.
    const name = text(call.function?.name ?? call.name, 120, 'INVALID_MODEL_TOOL_CALL_NAME');
    const args = call.function?.arguments ?? call.arguments;
    // Report which part of the call is unusable instead of one opaque code.
