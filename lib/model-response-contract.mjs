...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Eksik id/ad genel koda düşmemelidir; her alan kendi teşhis kodunu döndürür.
...
// Hem ham provider cevabını hem `normalizeModelResponse()` çıktısını kabul eder.
// Yeniden normalize etmek idempotent değildi: normalize edilmiş tool call'lar
// `{ id, name, arguments }` şeklindedir ve ham `function` sarmalını taşımaz.
export function isTerminalModelResponse(response) {
  if (!response || typeof response !== 'object' || Array.isArray(response)) throw new Error('INVALID_MODEL_RESPONSE');
  const finishReason = normalizeFinishReason(response.finishReason);
  return finishReason !== 'tool_calls' && finishReason !== 'unknown';
}
