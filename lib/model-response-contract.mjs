...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Normalizasyon idempotent olmalıdır: isTerminalModelResponse() zaten
    // normalize edilmiş bir yanıtı yeniden normalize eder ve o yanıtta tool
    // call'lar `{ id, name, arguments }` düzlüğündedir. Ham provider şekli
    // (`{ id, function: { name, arguments } }`) da aynı yolla kabul edilir.
    const source = call.function && typeof call.function === 'object' && !Array.isArray(call.function) ? call.function : call;
