...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Normalize idempotent olmalıdır: sağlayıcı biçimi (`function.name`) da,
    // bu fonksiyonun kendi ürettiği düz biçim (`name`) de kabul edilir.
    // Aksi hâlde normalize edilmiş bir yanıtı yeniden doğrulayan her çağrı
    // (örn. isTerminalModelResponse) geçerli tool call'da hata fırlatıyordu.
