...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Accept the provider shape (`function.name`) and the already normalized
    // shape, so normalization stays idempotent: callers such as
    // isTerminalModelResponse re-validate a response they already normalized.
