...
function normalizeToolCalls(toolCalls) {
  ...
  return Object.freeze(toolCalls.map((call) => {
    ...
    // Normalization has to be idempotent: an already-normalized response is fed
    // back through this contract by isTerminalModelResponse and by the callers
    // that re-validate a response before acting on it. Accept the provider shape
    // (`function.name`) and the normalized shape (`name`) alike.
