import { containsPlaintextCredential, isPlaintextCredentialField } from './plaintext-credential-policy.mts';
...
function inspectValue(value, state, depth = 0) {
  // Every inspected value counts, so a single very wide object is bounded the
  // same way a deeply nested one is.
