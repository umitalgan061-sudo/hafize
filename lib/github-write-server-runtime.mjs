...
import { createPrivilegedPrincipalAuthenticator } from './privileged-principal-auth.mjs';
...
export function createGitHubWriteServerRuntime({
  ...
  if (cloudSessionAuthenticator != null && typeof cloudSessionAuthenticator?.authenticate !== 'function') fail('cloudSessionAuthenticator');
  const writeConfig = [
  ...
  const authenticator = createPrivilegedPrincipalAuthenticator({
    ...
    cloudSessionAuthenticator
  });
  if (!authenticator) fail('authenticator');
