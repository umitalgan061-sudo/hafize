...
export function createGitHubWriteServerRuntime({ env = process.env, readJson, fetchImpl = globalThis.fetch } = {}) {
  ...
  if (writeConfig.every((value) => !value)) {
    return Object.freeze({ configured: false, async handle() { return { matched: false }; } });
  }
