...
export function createOptionalScheduleSessionAuthenticator({
  ...
  const configured = [passwordHash, signingKey, subject, origin, ttlRaw, revocationFilePath].some(Boolean);
  if (!configured) {
  ...
  const auth = sharedAuthenticator || createAuth({
    ...
    subject,
    ...(ttlMs ? { ttlMs } : {}),
    ...(revocationFilePath ? { revocationFilePath } : {})
  });
  if (typeof auth?.authenticate !== 'function') fail('INVALID_SCHEDULE_SESSION_AUTH:auth');
  return Object.freeze({ authenticate: (input) => auth.authenticate(input), allowedOrigin });
}
...
export const SCHEDULE_SESSION_AUTH_ENV = Object.freeze({
  ...
  origin: ORIGIN_ENV,
  ttlMs: TTL_ENV,
  revocationFile: REVOCATION_FILE_ENV
});
