...
export function createAgentLifecycle({ maxConcurrent = DEFAULT_MAX_CONCURRENT, inboxLimit = DEFAULT_INBOX_LIMIT } = {}) {
  ...
  function start({ runId, parentRunId = '', parentSignal, execute } = {}) {
    ...
    // The executor starts synchronously so that a run reported as `running` has
    // really begun; deferring it to a microtask let callers observe (and cancel)
    // a run whose executor had not been entered yet. The async wrapper still turns
    // a synchronous throw into a rejection.
    const promise = run.state === STATES.cancelled
      ? Promise.resolve(Object.freeze({ value: undefined, snapshot: snapshot(run) }))
      : (async () => execute({ runId: id, signal: controller.signal }))()
