...
export function createAgentLifecycle({ maxConcurrent = DEFAULT_MAX_CONCURRENT, inboxLimit = DEFAULT_INBOX_LIMIT } = {}) {
  ...
  function snapshot(run) {
    return Object.freeze({
      ...
      acceptedMessages: run.acceptedMessages,
      rejectedMessages: run.rejectedMessages,
      error: run.error
    });
  }
  ...
  function start({ runId, parentRunId = '', parentSignal, execute } = {}) {
    ...
    const promise = Promise.resolve()
      .then(() => execute({ runId: id, signal: controller.signal }))
  ...
  return Object.freeze({
    ...
    list,
    liveCount: () => liveCount(),
    limits: Object.freeze({ maxConcurrent: concurrentLimit, inboxLimit: queueLimit }),
    states: STATES
  });
}
