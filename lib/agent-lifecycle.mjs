...
export function createAgentLifecycle({ maxConcurrent = DEFAULT_MAX_CONCURRENT, inboxLimit = DEFAULT_INBOX_LIMIT } = {}) {
  ...
  function start({ runId, parentRunId = '', parentSignal, execute } = {}) {
    ...
    // A run reported as `running` must already have entered its executor, so the
    // executor is invoked synchronously; a run cancelled before start never runs.
    const started = run.state === STATES.cancelled
      ? null
      : (() => {
        try {
          return Promise.resolve(execute({ runId: id, signal: controller.signal }));
        } catch (error) {
          return Promise.reject(error);
        }
      })();
    const promise = started === null
