...
export function createAgentLifecycle({ maxConcurrent = DEFAULT_MAX_CONCURRENT, inboxLimit = DEFAULT_INBOX_LIMIT } = {}) {
  ...
  function start({ runId, parentRunId = '', parentSignal, execute } = {}) {
    ...
    const promise = run.state === STATES.cancelled
      ...
      : Promise.resolve()
        .then(() => {
          // The executor starts on a later microtask, so the run can already be
          // cancelled by then; a cancelled run must never reach its executor.
          const current = runs.get(id);
          if (!current || current.state !== STATES.running) return undefined;
          return execute({ runId: id, signal: controller.signal });
        })
