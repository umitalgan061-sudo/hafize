/**
 * @typedef {'running' | 'completed' | 'failed' | 'cancelled'} AgentRunState
 *
 * @typedef {object} AgentRun
 * @property {string} runId
 * @property {string} parentRunId
 * @property {AgentRunState} state Çalışma sırasında `cancel()` tarafından değişir.
 * @property {number} startedAt
 * @property {number | null} finishedAt
 * @property {unknown[]} messages
 * @property {number} acceptedMessages
 * @property {number} rejectedMessages
 * @property {string | null} error
 * @property {AbortController} controller
 * @property {(() => void) | null} detachParent
 */
 ...
 *
 * @param {{ maxConcurrent?: number; inboxLimit?: number }} [options]
 */
export function createAgentLifecycle({ maxConcurrent = DEFAULT_MAX_CONCURRENT, inboxLimit = DEFAULT_INBOX_LIMIT } = {}) {
  ...
  /**
   * @param {{ runId?: string; parentRunId?: string; parentSignal?: AbortSignal; execute?: (context: { runId: string; signal: AbortSignal }) => unknown }} [input]
   */
  ...
  /** @param {HafizeCodedError | null | undefined} error */
