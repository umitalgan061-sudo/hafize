...
export function createAgentLifecycle({ maxConcurrent = DEFAULT_MAX_CONCURRENT, inboxLimit = DEFAULT_INBOX_LIMIT } = {}) {
  ...
  function start({ runId, parentRunId = '', parentSignal, execute } = {}) {
    ...
    // Executor senkron başlatılır: `start()` döndükten sonra çalışan bir
    // microtask'a bırakılırsa, arada iptal edilen koşu yine de yürütülür ve
    // çağıran koşunun gerçekten başladığını gözlemleyemez. Async IIFE senkron
    // fırlatılan hataları da reddedilen promise'e çevirir.
