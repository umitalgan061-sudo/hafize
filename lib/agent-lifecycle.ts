export const AGENT_STATES = Object.freeze({ running: 'running', completed: 'completed', failed: 'failed', cancelled: 'cancelled' } as const);
export type AgentState = typeof AGENT_STATES[keyof typeof AGENT_STATES];
export interface AgentRunSnapshot { runId: string; state: AgentState; startedAt: number; finishedAt: number | null; parentRunId: string; messages: number; acceptedMessages: number; rejectedMessages: number; error: string | null }
export interface AgentRunStartOptions { runId?: unknown; parentRunId?: string; parentSignal?: AbortSignal; execute?: (input: { runId: string; signal: AbortSignal }) => unknown | Promise<unknown> }
export interface AgentLifecycleOptions { maxConcurrent?: number; inboxLimit?: number }
export interface AgentMessage { id: string; content: string; receivedAt: number }
...
export function createAgentLifecycle({ maxConcurrent = DEFAULT_MAX_CONCURRENT, inboxLimit = DEFAULT_INBOX_LIMIT }: AgentLifecycleOptions = {}) {
  ...
  function snapshot(run: any): AgentRunSnapshot {
  ...
  function finish(runId: string, state: AgentState, error: string | null = null, value?: unknown) {
  ...
  function cancel(runId: string, reason = 'AGENT_CANCELLED'): boolean {
    const run = runs.get(runId);
    if (!run) return false;
  ...
  function sendMessage(runId: string, message: unknown): AgentMessage {
    const run = runs.get(runId);
  ...
  function get(runId: unknown): AgentRunSnapshot | null {
    const run = runs.get(typeof runId === 'string' ? runId : '');
    return run ? snapshot(run) : null;
  }

  function list(): ReadonlyArray<AgentRunSnapshot> {
