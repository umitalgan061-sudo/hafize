const ALLOWED_FIELDS = new Set(['explicitUserIntent', 'mimeType', 'byteLength', 'width', 'height']);
...
function requireObject(input) {
  ...
  for (const key of Object.keys(input)) if (!ALLOWED_FIELDS.has(key)) throw new Error('INVALID_SCREEN_CAPTURE_METADATA:field');
}

function positiveInt(value, label, max) {
  if (!Number.isInteger(value) || value < 1 || value > max) throw new Error(`INVALID_SCREEN_CAPTURE_METADATA:${label}`);
  return value;
}
