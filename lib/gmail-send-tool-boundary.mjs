...
export function createGmailSendToolBoundary(options) {
  const { sendClient, ownerResolver } = normalizeRequestInput(options, failInvalid, 'options');
  ...
  async function execute(args, context) {
    const { principal, approvalGranted = false } = normalizeRequestInput(context, failInvalid, 'context');
