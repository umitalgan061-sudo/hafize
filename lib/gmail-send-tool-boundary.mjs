...
export function createGmailSendToolBoundary(options) {
  const { sendClient, ownerResolver } = requireObjectInput(options, 'options', invalid);
  ...
  async function execute(args, context) {
    const { principal, approvalGranted = false } = requireObjectInput(context, 'context', invalid);
