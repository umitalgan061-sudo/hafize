...
export function createGmailSendToolBoundary({ sendClient, ownerResolver } = {}) {
  ...
  async function execute(args, context) {
    if (context !== undefined && (!context || Array.isArray(context) || typeof context !== 'object')) {
      throw invalid('context');
    }
    const command = normalizeGmailSendRequest(args, { approvalGranted: context?.approvalGranted === true });
    const ownership = ownerResolver.resolve(context?.principal);
