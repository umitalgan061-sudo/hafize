...
// Node counting alone only bounds objects, so a single very wide object or array
// of primitives could still cross the egress boundary unbounded.
...
function inspectValue(value, state, depth = 0) {
  ...
  if (Array.isArray(value)) {
    if (value.length > MAX_ENTRIES) throw new ToolExecutionResultPolicyError('TOOL_RESULT_COMPLEXITY_BLOCKED');
  ...
  if (entries.length > MAX_ENTRIES) throw new ToolExecutionResultPolicyError('TOOL_RESULT_COMPLEXITY_BLOCKED');
