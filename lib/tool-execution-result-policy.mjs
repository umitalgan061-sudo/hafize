...
function inspectValue(value, state, depth = 0) {
  ...
  if (Array.isArray(value)) {
    // Every element counts against the node budget, not just nested containers:
    // a single flat array or object with thousands of scalar entries is exactly the
    // oversized payload this bound exists to reject.
    for (const item of value) {
      if (++state.nodes > MAX_NODES) throw new ToolExecutionResultPolicyError('TOOL_RESULT_COMPLEXITY_BLOCKED');
      inspectValue(item, state, depth + 1);
    }
    return;
  }
  let prototype;
