...
const WRITE_FIELDS = new Set(['ownerId', 'kind', 'content', 'sourceType', 'sourceRef', 'sensitivity', 'explicitUserIntent']);
...
function requireExactFields(input, allowed) {
  if (!input || Array.isArray(input) || typeof input !== 'object') throw new Error('INVALID_MEMORY_COMMAND:input');
  for (const key of Object.keys(input)) if (!allowed.has(key)) throw new Error('INVALID_MEMORY_COMMAND:field');
}
...
function normalizeSourceRef(value) {
  ...
  const ref = cleanText(value, 'sourceRef', 300);
  if (containsPlaintextCredential(ref)) throw new Error('MEMORY_SOURCE_REF_CREDENTIAL_NOT_ALLOWED');
  return ref;
}

function normalizeKinds(value) {
  if (value == null) return [];
  if (!Array.isArray(value) || value.length > MAX_KINDS) throw new Error('INVALID_MEMORY_COMMAND:kinds');
...
export function normalizeMemoryWrite(input) {
  try {
    ...
    if (input.explicitUserIntent !== true) throw new Error('MEMORY_WRITE_REQUIRES_EXPLICIT_USER_INTENT');
