...
function fail(field) { throw new Error(`INVALID_OAUTH_TOKEN_STORE:${field}`); }
...
function recordKey(ownerId, provider) { return createHash('sha256').update(`${ownerId}\0${provider}`, 'utf8').digest('hex'); }
function assertInside(root, target) {
  const normalizedRoot = resolve(root);
  const normalizedTarget = resolve(target);
  if (!normalizedTarget.startsWith(`${normalizedRoot}${sep}`) && normalizedTarget !== normalizedRoot) fail('path');
...
export function createOAuthTokenFileStore({
  ...
  function pathFor(ownerId, provider) {
    ...
    return { owner, source, path: assertInside(root, join(root, `${recordKey(owner, source)}.json`)) };
  }

  async function ensureRoot() {
    await fs.mkdir(root, { recursive: true, mode: PRIVATE_DIRECTORY_MODE });
    if (supportsPosixModes) await fs.chmod(root, PRIVATE_DIRECTORY_MODE);
  }

  async function hardenExistingTarget(targetPath) {
  ...
  async function syncRootDirectory() {
    if (platform === 'win32') return;
    let handle;
    try {
      handle = await fs.open(root, 'r');
      if (typeof handle?.sync !== 'function' || typeof handle?.close !== 'function') throw new Error('invalid directory handle');
      await handle.sync();
      await handle.close();
      handle = null;
    } catch (error) {
      try { await handle?.close(); } catch {}
      if (UNSUPPORTED_DIRECTORY_SYNC.has(error?.code)) return;
      throw error;
    }
  }
  ...
  async function save({ ownerId, provider, tokenRecord } = {}) {
    ...
    try {
      ...
      await handle.close();
      handle = null;
      if (supportsPosixModes) await fs.chmod(temporary, PRIVATE_FILE_MODE);
      await fs.rename(temporary, target.path);
      if (supportsPosixModes) await fs.chmod(target.path, PRIVATE_FILE_MODE);
  ...
  async function load({ ownerId, provider } = {}) {
    const target = pathFor(ownerId, provider);
    await hardenExistingTarget(target.path);
    let metadata;
...
export const OAUTH_TOKEN_FILE_MODES = Object.freeze({ directory: PRIVATE_DIRECTORY_MODE, file: PRIVATE_FILE_MODE });
