...
function argumentObject(value, field) {
...
export function createOAuthTokenFileStore({
  ...
  async function save(input) {
    const { ownerId, provider, tokenRecord } = argumentObject(input, 'save');
  ...
  async function load(input) {
    const { ownerId, provider } = argumentObject(input, 'load');
    const target = pathFor(ownerId, provider);
    let metadata;
  ...
  async function remove(input) {
    const { ownerId, provider, explicitUserIntent } = argumentObject(input, 'remove');
    const target = pathFor(ownerId, provider);
