...
function strictObject(value: unknown, field: string, allowed: ReadonlySet<string> = new Set()): Record<string, any> {
...
function scopesOf(record: Record<string, any> | null | undefined): Set<string> {
...
function listUrl(params: Record<string, any>) {
...
function resolveRequest(operation: string, params: Record<string, any>, scopes: ReadonlySet<string>) {
...
export function createCanvaReadClient({ tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now(), maxJsonBytes = MAX_JSON_BYTES }: { tokenStore?: any; fetchImpl?: HafizeFetch; now?: () => number; maxJsonBytes?: number } = {}) {
  ...
  async function read({ ownerId, operation, params }: { ownerId?: string; operation?: string; params?: Record<string, any> } = {}) {
    ...
    if (Buffer.byteLength(serialized, 'utf8') > maxJsonBytes || serialized.includes(accessToken) || containsPlaintextCredential(serialized)) throw new Error('CANVA_READ_FAILED:response');
