...
function safeNow(now, minimum = 0) {
  ...
  if (!Number.isSafeInteger(value) || value < minimum) fail('INVALID_GOOGLE_TOKEN_REFRESH:now');
  return value;
}
...
export function createGoogleTokenRefresh({
  ...
  async function refreshOnce(owner) {
    ...
    try {
      ...
      if (lease && Number.isSafeInteger(currentExpiry) && currentExpiry > issuedAt + FRESH_MARGIN_MS) {
        return Object.freeze({ provider: 'google', ownerId: owner, scopes: Object.freeze(previousScopes), expiresAt: currentExpiry });
      }
      ...
      if (!Number.isSafeInteger(expiresAt)) fail('INVALID_GOOGLE_TOKEN_REFRESH:now');
      await lease?.renew();
  ...
  function refresh(input = {}) {
    if (!input || Array.isArray(input) || typeof input !== 'object') fail('INVALID_GOOGLE_TOKEN_REFRESH:input');
    const owner = text(input.ownerId, 'ownerId', { max: 128 });
