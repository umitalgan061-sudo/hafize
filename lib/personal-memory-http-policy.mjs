...
const WRITE_APPROVAL_FIELDS = new Set(['kind', 'content', 'sourceType', 'sourceRef', 'sensitivity', 'explicitUserIntent']);
const WRITE_FIELDS = new Set(['kind', 'content', 'sourceType', 'sourceRef', 'sensitivity', 'approvalReceipt']);
...
const MAX_APPROVAL_RECEIPT_LENGTH = 2048;
...
export function validateMemoryMutationBody(kind, body) {
  if (kind === 'write-approval') {
    if (!exactObject(body, WRITE_APPROVAL_FIELDS) || body.explicitUserIntent !== true) {
      return invalid('MEMORY_WRITE_REQUIRES_EXPLICIT_USER_INTENT');
    }
    return success(body);
  }
  if (kind === 'write') {
    if (!exactObject(body, WRITE_FIELDS)) return invalid('MEMORY_WRITE_APPROVAL_REQUIRED');
    const receipt = typeof body.approvalReceipt === 'string' ? body.approvalReceipt.trim() : '';
    if (!receipt || receipt.length > MAX_APPROVAL_RECEIPT_LENGTH) return invalid('MEMORY_WRITE_APPROVAL_REQUIRED');
    return success({ ...body, approvalReceipt: receipt });
  }
  if (kind === 'export') {
