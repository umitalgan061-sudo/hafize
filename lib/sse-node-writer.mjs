...
function normalizeMaxTotalBytes(value, maxChunkBytes = DEFAULT_MAX_CHUNK_BYTES) {
  if (value == null) return DEFAULT_MAX_TOTAL_BYTES;
  if (!Number.isInteger(value) || value < maxChunkBytes || value > MAX_TOTAL_BYTES) {
    fail('INVALID_SSE_NODE_WRITER_MAX_TOTAL_BYTES');
  }
  return value;
}
...
function normalizeDrainTimeoutMs(value) {
  ...
  if (!Number.isInteger(value) || value < 100 || value > 120_000) {
    fail('INVALID_SSE_NODE_WRITER_DRAIN_TIMEOUT');
  }
  return value;
}
...
function waitForDrain(response, signal, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (value) => {
      ...
      cleanup();
      if (value === 'timeout') {
        const error = new Error('SSE_OUTPUT_DRAIN_TIMEOUT');
        error.code = 'SSE_OUTPUT_DRAIN_TIMEOUT';
        reject(error);
        return;
      }
      resolve(value);
...
export function createSseNodeWriter({ response, signal, maxChunkBytes, maxTotalBytes, drainTimeoutMs } = {}) {
  const target = requireResponse(response);
  const chunkLimit = normalizeMaxChunkBytes(maxChunkBytes);
  const totalLimit = normalizeMaxTotalBytes(maxTotalBytes, chunkLimit);
  const drainTimeout = normalizeDrainTimeoutMs(drainTimeoutMs);
  let ended = false;
  ...
  function validateBudget(chunk) {
    const size = chunkByteLength(chunk);
    if (size > chunkLimit) fail('SSE_OUTPUT_CHUNK_TOO_LARGE');
    if (bytesWritten + size > totalLimit) fail('SSE_OUTPUT_TOO_LARGE');
    return size;
  }

  async function write(chunk) {
    if (!writable()) return false;
    const size = validateBudget(chunk);
    ...
    try {
      return await waitForDrain(target, signal, drainTimeout);
    } finally {
  ...
  function end(chunk) {
    ...
    let size = 0;
    if (chunk !== undefined) size = validateBudget(chunk);
    ended = true;
    try {
      target.end(chunk);
      bytesWritten += size;
