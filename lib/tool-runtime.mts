...
import { createBuiltinSkillsRuntimeSync } from './skills-runtime.mts';
import { CANVA_READ_TOOL_DEFINITION } from './canva-read-tool-boundary.mts';
import { GMAIL_READ_TOOL_DEFINITION } from './gmail-read-tool-boundary.mts';
import { normalizeToolCall, parseToolArguments, sanitizeToolError } from './tool-call-boundary.mts';
import { projectSafeToolExecutionResult } from './tool-execution-result-policy.mts';

const BUILTIN_SKILLS_RUNTIME = createBuiltinSkillsRuntimeSync();
...
const TOOL_CATALOG = new Map([
  ['runtime_status', {
    ...
    async execute(_args, context) {
      return { status: 'ok', traceId: context.traceId, agentId: context.agent.id, agentName: context.agent.name, nvidiaConfigured: Boolean(context.nvidiaConfigured), githubReadConfigured: Boolean(context.githubReadConfigured), availableAgents: (Array.isArray(context.registry?.agents) ? context.registry.agents : []).map(({ id, name, kind }) => ({ id, name, kind })) };
    }
  }],
  ['agent_delegate', {
    ...
    async execute(args, context) {
      ...
      if (!result?.ok) { const error = (new Error('AGENT_DELEGATION_FAILED') as HafizeCodedError); error.code = typeof result?.error === 'string' ? result.error : 'AGENT_DELEGATION_FAILED'; throw error; }
      return result.value;
    }
  }],
  ['github_read_file', {
    ...
    async execute(args, context) {
      if (typeof context.githubReadFile !== 'function') { const error = (new Error('GITHUB_NOT_CONFIGURED') as HafizeCodedError); error.code = 'GITHUB_NOT_CONFIGURED'; error.status = 503; throw error; }
  ...
  ['skill_invoke', {
    ...
    async execute(args, context) {
      ...
      if (!invocation) { const error = (new Error('UNKNOWN_SKILL') as HafizeCodedError); error.code = 'UNKNOWN_SKILL'; throw error; }
      return {
...
function normalizeAllowedPermissions(value) {
  ...
  return new Set(value.filter((permission) => typeof permission === 'string'));
}

export function getAllowedNvidiaTools(
  agent: unknown,
  context: UnvalidatedInput = {},
  options: { allowedPermissions?: unknown } = {}
) {
...
export async function executeNvidiaToolCall(agent, toolCall, context) {
  let normalizedCall;
  try { normalizedCall = normalizeToolCall(toolCall); } catch (error) { return { ok: false as const, error: sanitizeToolError(error).code }; }
  const entry = TOOL_CATALOG.get(normalizedCall.function.name) || null;
  if (!entry) return { ok: false as const, error: 'UNKNOWN_TOOL' };
  if (typeof entry.available === 'function' && !entry.available(context)) return { ok: false as const, error: 'TOOL_UNAVAILABLE' };
  const authorization = authorizeAgentTool(agent, entry.permission, { approvalGranted: Boolean(context.approvalGranted) });
  if (!authorization.allowed) return { ok: false as const, error: 'TOOL_NOT_AUTHORIZED', reason: authorization.reason };
  let args;
  try { args = parseToolArguments(normalizedCall.function.arguments); } catch (error) { return { ok: false as const, error: sanitizeToolError(error).code }; }
  try {
    return projectSafeToolExecutionResult({ ok: true as const, value: await entry.execute(args, context) });
  } catch (error) {
    const safe = sanitizeToolError(error);
    // `status` yalnızca sağlayıcı bir HTTP kodu verdiğinde eklenir.
    const result: { ok: false; error: string; status?: number } = { ok: false as const, error: safe.code };
