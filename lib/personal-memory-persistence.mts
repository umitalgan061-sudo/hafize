import { createPersonalMemoryStore } from './personal-memory-store.mts';
import {
  decryptPersonalMemorySnapshot,
  encryptPersonalMemorySnapshot
} from './personal-memory-encryption.mts';
...
export function createPersonalMemoryPersistence({ adapter, key, storeOptions = {}, createStore = createPersonalMemoryStore }: { adapter?: any; key?: unknown; storeOptions?: Record<string, any>; createStore?: Function } = {}) {
  ...
  function read(input: Record<string, any>) {
    requireOpen();
    return store.read(input);
  }

  function readForContext(input: Record<string, any>) {
    requireOpen();
    return store.readForContext(input);
  }

  function exportOwner(input: Record<string, any>) {
    requireOpen();
  ...
  function mutate(method: string, input: Record<string, any>) {
    const operation = queue.then(async () => {
      ...
      try {
        ...
        return { ok: false as const, error: 'MEMORY_PERSISTENCE_SAVE_FAILED' };
      }
      store = candidate;
      return result;
    });
  ...
  function deleteOwner(input) {
    ...
    try {
      ...
      return Promise.resolve({ ok: false as const, error: error.message });
    }
    const operation = queue.then(async () => {
      requireOpen();
      ...
      if (!deleted) return { ok: true as const, deleted: 0 };
