...
function upstreamFailure(error) {
  if (error?.code === 'CANVA_OAUTH_UPSTREAM_TIMEOUT') return new Error('CANVA_TOKEN_EXCHANGE_FAILED:timeout');
  if (error?.code === 'CANVA_OAUTH_UPSTREAM_CANCELLED') return new Error('CANVA_TOKEN_EXCHANGE_FAILED:cancelled');
  if (error?.code === 'CANVA_OAUTH_UPSTREAM_INVALID_RESPONSE') return new Error('CANVA_TOKEN_EXCHANGE_FAILED:response');
  return error;
}

export function createCanvaTokenExchange({
  clientId,
  clientSecret,
  tokenStore,
  ...
  async function exchange({ ownerId, code, verifier, redirectUri: callbackUri, expectedScopes, signal } = {}) {
    ...
    if (!result.ok) throw new Error('CANVA_TOKEN_EXCHANGE_FAILED:http');
    const tokens = tokenResponse(result.payload);
