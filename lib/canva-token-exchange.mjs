...
function requestObject(value, allowed) {
  ...
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new Error('INVALID_CANVA_TOKEN_EXCHANGE:request');
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) throw new Error(`INVALID_CANVA_TOKEN_EXCHANGE:request.${key}`);
  }
  return value;
}
