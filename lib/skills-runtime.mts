...
import { createSkillsRegistry } from './skills-registry.mts';
...
function buildRuntime(raw: unknown, allowedProjects: string[], createRegistry: typeof createSkillsRegistry = createSkillsRegistry) {
  ...
  function listForAgent(agent: Record<string, any>) {
    return registry.listForAgent(agent, { intersectTools: true });
  }

  function selectForAgent(agent: Record<string, any>, query: string, options?: Record<string, any>) {
    if (!agent || typeof agent !== 'object') fail('INVALID_SKILL_AGENT');
    return createSkillSelector(listForAgent(agent)).select(query, options);
  }

  function rankForAgent(agent: Record<string, any>, query: string) {
    if (!agent || typeof agent !== 'object') fail('INVALID_SKILL_AGENT');
    return createSkillSelector(listForAgent(agent)).rank(query);
  }

  function resolveForAgent({ agent, skillId, args = {}, approvalGranted = false }: { agent?: any; skillId?: string; args?: Record<string, any>; approvalGranted?: boolean } = {}) {
  ...
  function describePublic(agent: Record<string, any>) {
...
export async function createSkillsRuntime({ fileUrl = DEFAULT_BUILTIN_URL, allowedProjects = [], readFileImpl = readFile, createRegistry = createSkillsRegistry }: { fileUrl?: URL | string; allowedProjects?: string[]; readFileImpl?: Function; createRegistry?: typeof createSkillsRegistry } = {}) {
...
export function createBuiltinSkillsRuntimeSync({ fileUrl = DEFAULT_BUILTIN_URL, allowedProjects = [], readFileImpl = readTextFileSync, createRegistry = createSkillsRegistry }: { fileUrl?: URL | string; allowedProjects?: string[]; readFileImpl?: Function; createRegistry?: typeof createSkillsRegistry } = {}) {
  if (typeof readFileImpl !== 'function') fail('INVALID_SKILL_RUNTIME_READER');
  if (typeof createRegistry !== 'function') fail('INVALID_SKILL_RUNTIME_REGISTRY');
  let raw;
  try {
