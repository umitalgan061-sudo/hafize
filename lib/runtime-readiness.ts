...
export type RuntimeComponent = 'auth' | 'pwa' | 'skills' | 'memory' | 'schedule' | 'connectors' | 'model';
export interface ReadinessComponent { status: ReadinessStatus; detail: string; checkedAt: string }
export type ReadinessReport = Partial<Record<RuntimeComponent, { status?: unknown; detail?: unknown; checkedAt?: unknown }>>;
export interface RuntimeReadinessResult { ready: boolean; degraded: boolean; blocked: RuntimeComponent[]; warnings: RuntimeComponent[]; unknown: RuntimeComponent[]; components: Record<RuntimeComponent, ReadinessComponent> }
...
function normalizeComponent(name: unknown): RuntimeComponent {
...
export function normalizeReadinessReport(report: ReadinessReport = {}): Record<RuntimeComponent, ReadinessComponent> {
...
export function evaluateRuntimeReadiness(report: ReadinessReport = {}): RuntimeReadinessResult {
...
export function updateReadinessComponent(report: ReadinessReport, component: RuntimeComponent, status: unknown, detail = '', checkedAt = ''): Record<RuntimeComponent, ReadinessComponent> {
