...
function normalizeBrowserArgs(args, allowedOrigins) {
  ...
  if (!allowedOrigins.has(url.origin)) return { ok: false as const, error: 'DEVICE_BRIDGE_TARGET_NOT_ALLOWED' };
  return { ok: true as const, args: { url: url.href } };
}
...
function normalizeAppArgs(args, allowedApps) {
  ...
  if (!allowedApps.has(appId)) return { ok: false as const, error: 'DEVICE_BRIDGE_TARGET_NOT_ALLOWED' };
  return { ok: true as const, args: { appId } };
}
...
function normalizeSystemInfoArgs(args, allowedInfo) {
  ...
  if (requested.some((field) => !allowedInfo.has(field))) {
    return { ok: false as const, error: 'DEVICE_BRIDGE_TARGET_NOT_ALLOWED' };
  }
  return { ok: true as const, args: { fields: requested } };
}

export function createDeviceBridgePolicy({ allowedWebOrigins = [], allowedApps = [], allowedSystemInfo }: { allowedWebOrigins?: string[]; allowedApps?: string[]; allowedSystemInfo?: string[] } = {}) {
  ...
  function authorize(raw: Record<string, any> | null | undefined) {
    let input;
    try {
      ...
      return { ok: true as const, request: { operation, args: normalized.args } };
    } catch (error) {
