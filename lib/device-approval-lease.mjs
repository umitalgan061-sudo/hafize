...
/**
 * @param {{ action?: unknown; url?: unknown; appId?: unknown } | null | undefined} input
 * @returns {string}
 */
 ...
 *
 * @param {{ now?: () => number; randomId?: () => string }} [options]
 */
export function createDeviceApprovalLeaseStore({ now = () => Date.now(), randomId = randomUUID } = {}) {
  ...
  /**
   * @param {{ traceId?: string; action?: string; target?: string; ttlMs?: number }} [input]
   */
  ...
  /**
   * @param {{ token?: string; traceId?: string; action?: string; target?: string }} [input]
   */
  function consume({ token, traceId, action, target } = {}) {
    try {
      ...
      return { ok: true, approval: Object.freeze({ traceId: lease.traceId, action: lease.action, target: lease.target, issuedAt: lease.issuedAt, expiresAt: lease.expiresAt }) };
    } catch (error) {
      return fail(error.message);
    }
  }
