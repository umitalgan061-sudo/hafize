...
export function evaluateDeploymentReadiness({
  ...
  config,
  release,
  requiredComponents = ['runtime', 'config', 'release']
} = {}) {
  if (!Array.isArray(requiredComponents) || !requiredComponents.length) {
    throw new Error('INVALID_DEPLOYMENT_REQUIRED_COMPONENTS');
  }
  ...
  for (const item of requiredComponents) {
    ...
    if (duplicateNames.has(name)) throw new Error(`DUPLICATE_DEPLOYMENT_COMPONENT:${name}`);
...
export function assertDeploymentReleaseable(report) {
  if (!report || report.releaseable !== true || report.state !== 'ready') {
    throw new Error('DEPLOYMENT_NOT_RELEASEABLE');
  }
  return report;
}
