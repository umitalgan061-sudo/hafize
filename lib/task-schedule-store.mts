...
function clone(entry: HafizeScheduleEntry): HafizeScheduleEntry {
  return { ...entry };
}

function restoreSnapshot(initialSnapshot: Record<string, any> | null, capacity: number): HafizeScheduleEntry[] {
...
function nextScheduleNumber(entries: HafizeScheduleEntry[]) {
  let max = 0;
...
/**
 * Zamanlanmış görevlerin sınırlı, bellek içi deposu.
 */
export function createTaskScheduleStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), initialSnapshot = null }: { maxEntries?: number; now?: () => Date; initialSnapshot?: Record<string, any> | null } = {}) {
  ...
  function find(scheduleId: string) {
  ...
  function add(input: Record<string, any> = {}) {
    ...
    // `status` yerinde güncelleniyor, bu yüzden kayıt değiştirilebilir bir
    // görünüm olarak tutulur; dışarıya `clone()` ile okunur hâli veriliyor.
    const entry: { -readonly [K in keyof HafizeScheduleEntry]: HafizeScheduleEntry[K] } = {
  ...
  function claimDue({ limit = 1 }: { limit?: number } = {}) {
  ...
  function complete(scheduleId: string) {
  ...
  function fail(scheduleId: string, { error = 'SCHEDULE_EXECUTION_FAILED', retryAt = null }: { error?: string; retryAt?: string | null } = {}) {
    const entry = find(scheduleId);
    if (entry.status !== 'running') throw new Error('INVALID_TASK_SCHEDULE_TRANSITION');
  ...
  function defer(scheduleId: string, { runAt, error = 'SCHEDULE_LEASE_BUSY' }: { runAt?: string; error?: string } = {}) {
    const entry = find(scheduleId);
  ...
  /**
   * Kimlik verilirse tek kaydı, verilmezse tüm deponun anlık görüntüsünü
   * döndürür. Dar imza önce yazılır: `any` bir argüman ilk eşleşen aşırı
   * yüklemeyi seçtiği için ters sıra çağıranlara anlık görüntü tipini verirdi.
   */
  function read(scheduleId: string): HafizeScheduleEntry | null;
  function read(scheduleId?: null): { entries: HafizeScheduleEntry[] };
  function read(scheduleId: string | null = null): HafizeScheduleEntry | { entries: HafizeScheduleEntry[] } | null {
  ...
  return Object.freeze({ add, claimDue, complete, fail, defer, cancel, read, snapshot });
}
