...
function requireReadScope(record: Record<string, any> | null | undefined) {
...
function resolveUrl(operation: string, params: Record<string, any>) {
...
export function createGmailReadClient({ tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now(), maxJsonBytes = MAX_JSON_BYTES }: { tokenStore?: any; fetchImpl?: HafizeFetch; now?: () => number; maxJsonBytes?: number } = {}) {
  ...
  async function read(request: Record<string, any> = {}) {
    ...
    if (Buffer.byteLength(serialized, 'utf8') > maxJsonBytes || serialized.includes(accessToken) || containsPlaintextCredential(serialized)) throw new Error('GMAIL_READ_FAILED:response');
