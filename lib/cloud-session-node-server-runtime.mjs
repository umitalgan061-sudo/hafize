...
export function createCloudSessionNodeServerRuntime({
  ...
  const fields = [passwordHash, signingKey, subject, allowedOrigin, ttlRaw];
  ...
  if (typeof auth?.login !== 'function' || typeof auth?.authenticate !== 'function' || typeof auth?.logoutCookie !== 'function') fail('INVALID_CLOUD_SESSION_SERVER:auth');
