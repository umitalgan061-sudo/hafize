...
const TOOL_CATALOG = new Map([
  ...
  [
    'canva_read',
    {
      ...
      available(context) {
        return context.principal?.authenticated === true && typeof context.canvaReadTool?.execute === 'function';
      },
      definition: CANVA_READ_TOOL_DEFINITION,
      async execute(args, context) {
        return context.canvaReadTool.execute(args, { principal: context.principal });
      }
    }
  ]
]);
