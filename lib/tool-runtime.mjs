...
} from './tool-execution-result-policy.mjs';
...
const TOOL_CATALOG = new Map([
  ...
  [
    'gmail_read',
    {
      ...
      available(context) {
        return context.gmailReadAuthenticated === true && typeof context.gmailReadTool?.execute === 'function';
      },
      definition: GMAIL_READ_TOOL_DEFINITION,
      async execute(args, context) {
        return context.gmailReadTool.execute(args);
      }
    }
  ]
]);
...
export async function executeNvidiaToolCall(agent, toolCall, context) {
  ...
  if (!entry) return createToolFailureResult('UNKNOWN_TOOL');
  if (typeof entry.available === 'function' && !entry.available(context)) {
    return createToolFailureResult('TOOL_UNAVAILABLE');
  }
  ...
  if (!authorization.allowed) {
    return createToolFailureResult('TOOL_NOT_AUTHORIZED', { reason: authorization.reason });
  }
  ...
  try {
    ...
    return createToolFailureResult('INVALID_TOOL_ARGUMENTS');
  }

  try {
    const value = await executeWithSignal(entry, args, context);
    return createToolSuccessResult(value);
  } catch (error) {
    return createToolFailureFromError(error);
  }
}
