    ...
    'agent_delegate',
    {
      ...
      available(context) {
        return typeof context.delegateAgent === 'function'
          && Number.isInteger(context.delegationDepth)
          && context.delegationDepth < context.maxDelegationDepth;
      },
      definition: {
        type: 'function',
        function: {
          name: 'agent_delegate',
          description: 'Dar kapsamlı bir alt görevi uygun Hafize uzman ajanına devreder. Ana/aynı ajana delegasyon ve derinlik aşımı backend tarafından engellenir.',
          parameters: {
            type: 'object',
            properties: {
              agentId: {
                type: 'string',
                description: 'Hedef uzman ajanın registry id değeri.'
              },
              task: {
                type: 'string',
                description: 'Uzman ajana verilecek açık ve dar kapsamlı görev.'
              }
            },
      ...
      async execute(args, context) {
        if (typeof context.delegateAgent !== 'function') {
          const error = new Error('DELEGATION_UNAVAILABLE');
          error.code = 'DELEGATION_UNAVAILABLE';
          error.status = 503;
          throw error;
        }
        return context.delegateAgent({ agentId: args.agentId, task: args.task });
      }
    }
  ],
  [
