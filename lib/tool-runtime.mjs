        ...
        function: {
          ...
          parameters: {
            ...
            properties: {
              ...
              successCriteria: { type: 'array', description: 'Görevin tamamlanmış sayılması için gözlenebilir başarı ölçütleri.', maxItems: 8, items: { type: 'string', maxLength: 500 } },
              constraints: { type: 'array', description: 'Uzman ajanın görev sırasında aşmaması gereken kapsam veya davranış sınırları.', maxItems: 8, items: { type: 'string', maxLength: 500 } },
              evidenceRequired: { type: 'array', description: 'Uzman sonucun doğrulanması için beklenen kanıt veya kontrol maddeleri.', maxItems: 8, items: { type: 'string', maxLength: 500 } }
            },
            required: ['agentId', 'task'], additionalProperties: false
          }
        }
      },
      ...
      definition: {
        ...
        function: {
          ...
          parameters: {
            type: 'object',
            properties: {
              repository: { type: 'string', description: 'owner/repo biçiminde GitHub deposu.' },
              path: { type: 'string', description: 'Repo içindeki göreli metin dosyası yolu.' },
              ref: { type: 'string', description: 'Opsiyonel branch, tag veya commit ref.' }
            },
...
function cancellationError() {
  const error = new Error('TOOL_EXECUTION_CANCELLED');
  error.code = 'TOOL_EXECUTION_CANCELLED';
  return error;
}

function executeWithSignal(entry, args, context) {
  const signal = context?.signal;
  if (!validSignal(signal)) {
    const error = new Error('INVALID_TOOL_SIGNAL');
    error.code = 'INVALID_TOOL_SIGNAL';
    return Promise.reject(error);
  }
  if (!signal) return entry.execute(args, context);
  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.resolve()
      .then(() => entry.execute(args, context))
...
export async function executeNvidiaToolCall(agent, toolCall, context) {
  ...
  let args;
  try { args = parseArguments(toolCall.function.arguments); }
  catch { return { ok: false, error: 'INVALID_TOOL_ARGUMENTS' }; }

  try {
    const value = await executeWithSignal(entry, args, context);
    return { ok: true, value };
  } catch (error) {
    const result = { ok: false, error: typeof error?.code === 'string' && error.code ? error.code : 'TOOL_EXECUTION_FAILED' };
