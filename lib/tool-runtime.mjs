...
const TOOL_CATALOG = new Map([
  ...
  ['agent_delegate', {
    ...
    async execute(args, context) {
      ...
      if (!result?.ok) { const error = /** @type {HafizeCodedError} */ (new Error('AGENT_DELEGATION_FAILED')); error.code = typeof result?.error === 'string' ? result.error : 'AGENT_DELEGATION_FAILED'; throw error; }
      return result.value;
    }
  }],
  ['github_read_file', {
    ...
    async execute(args, context) {
      if (typeof context.githubReadFile !== 'function') { const error = /** @type {HafizeCodedError} */ (new Error('GITHUB_NOT_CONFIGURED')); error.code = 'GITHUB_NOT_CONFIGURED'; error.status = 503; throw error; }
  ...
  ['skill_invoke', {
    ...
    async execute(args, context) {
      ...
      if (!invocation) { const error = /** @type {HafizeCodedError} */ (new Error('UNKNOWN_SKILL')); error.code = 'UNKNOWN_SKILL'; throw error; }
      return {
...
export function getAllowedNvidiaTools(agent, context = {}, options = {}) {
...
export async function executeNvidiaToolCall(agent, toolCall, context) {
  let normalizedCall;
  try { normalizedCall = normalizeToolCall(toolCall); } catch (error) { return { ok: false, error: sanitizeToolError(error).code }; }
  ...
  let args;
  try { args = parseToolArguments(normalizedCall.function.arguments); } catch (error) { return { ok: false, error: sanitizeToolError(error).code }; }
  try {
    return projectSafeToolExecutionResult({ ok: true, value: await entry.execute(args, context) });
  } catch (error) {
    const safe = sanitizeToolError(error);
    const result = { ok: false, error: safe.code };
