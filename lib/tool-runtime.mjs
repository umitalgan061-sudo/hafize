...
const TOOL_CATALOG = new Map([
  ['runtime_status', {
    ...
    async execute(_args, context) {
      return { status: 'ok', traceId: context.traceId, agentId: context.agent.id, agentName: context.agent.name, nvidiaConfigured: Boolean(context.nvidiaConfigured), githubReadConfigured: Boolean(context.githubReadConfigured), availableAgents: context.registry.agents.map(({ id, name, kind }) => ({ id, name, kind })) };
    }
  }],
  ['agent_delegate', {
    ...
    async execute(args, context) {
      ...
      if (!result?.ok) { const error = new Error('AGENT_DELEGATION_FAILED'); error.code = typeof result?.error === 'string' ? result.error : 'AGENT_DELEGATION_FAILED'; throw error; }
      return result.value;
    }
  }],
  ['github_read_file', {
    ...
    async execute(args, context) {
      if (typeof context.githubReadFile !== 'function') { const error = new Error('GITHUB_NOT_CONFIGURED'); error.code = 'GITHUB_NOT_CONFIGURED'; error.status = 503; throw error; }
  ...
  ['skill_invoke', {
    ...
    available(context) { return typeof context.skillsRuntime?.resolveForAgent === 'function'; },
    ...
    async execute(args, context) {
      if (!context.skillsRuntime?.resolveForAgent) { const error = new Error('SKILLS_NOT_CONFIGURED'); error.code = 'SKILLS_NOT_CONFIGURED'; error.status = 503; throw error; }
      ...
      if (!invocation) { const error = new Error('UNKNOWN_SKILL'); error.code = 'UNKNOWN_SKILL'; throw error; }
      return {
...
export async function executeNvidiaToolCall(agent, toolCall, context) {
  ...
  try {
    return { ok: true, value: await entry.execute(args, context) };
  } catch (error) {
