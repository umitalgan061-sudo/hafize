...
export function createScheduleWorker({
  ...
  async function executeClaimed(schedule, signal = null) {
    ...
    const error = signal?.aborted ? WORKER_CANCELLED_ERROR : cleanErrorCode(result?.error);
  ...
  async function runDue({ limit = configuredBatchLimit, signal = null } = {}) {
    ...
    const results = await mapWithConcurrency(
      ...
      (schedule) => executeClaimed(schedule, signal)
    );
    return {
...
export { REFUNDABLE_INFRA_ERRORS, WORKER_CANCELLED_ERROR, mapWithConcurrency, normalizePositiveBoundedInteger, validSignal };
