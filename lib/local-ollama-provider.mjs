...
function normalizeBaseUrl(value) {
  ...
  if (!['localhost', '127.0.0.1', '::1', '[::1]'].includes(host)) fail('LOCAL_PROVIDER_MUST_BE_LOOPBACK');
...
function assertNoLocalToolRequest(input) {
  for (const field of LOCAL_TOOL_FIELDS) {
    if (Object.prototype.hasOwnProperty.call(input, field)) fail('LOCAL_PROVIDER_TOOLS_UNSUPPORTED');
  }
}
...
async function readBoundedText(response, maxBytes) {
  ...
  if (response?.body && typeof response.body.getReader === 'function') {
    ...
    try {
      ...
      if (!completed) {
        try { await reader.cancel?.('hafize-local-provider-read-failed'); } catch { /* cleanup is best effort */ }
      }
...
async function cancelStreamIterator(iterator, body) {
  try {
    ...
    // Cleanup is best effort and must not mask the consumer's return/error.
  }
}
