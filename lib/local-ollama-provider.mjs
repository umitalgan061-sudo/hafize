...
const ALLOWED_FIELDS = new Set(['model', 'messages', 'stream', 'max_tokens', 'temperature', 'top_p', 'tools', 'tool_choice']);
const LOCAL_TOOL_FIELDS = Object.freeze(['tools', 'tool_choice']);
...
function fail(code, status = 400) { throw makeError(code, status); }
function normalizeEnabled(value) { return value === true || (typeof value === 'string' && value.trim().toLowerCase() === 'true'); }

function normalizeBaseUrl(value) {
  let url;
  try { url = new URL(String(value || DEFAULT_BASE_URL)); } catch { fail('INVALID_LOCAL_PROVIDER_BASE_URL'); }
  if (url.protocol !== 'http:' || url.username || url.password || url.search || url.hash) fail('INVALID_LOCAL_PROVIDER_BASE_URL');
  if (!['localhost', '127.0.0.1', '::1'].includes(url.hostname.toLowerCase())) fail('LOCAL_PROVIDER_MUST_BE_LOOPBACK');
  return url.toString().replace(/\/+$/, '');
}
function safeModelId(value) { return typeof value === 'string' && value.length > 0 && value.length <= MAX_MODEL_LENGTH && SAFE_MODEL_ID.test(value); }
function normalizeModel(value) {
  ...
  if (!safeModelId(localModel)) fail('INVALID_LOCAL_PROVIDER_MODEL');
  return localModel;
}
function assertNoLocalToolRequest(input) {
  for (const field of LOCAL_TOOL_FIELDS) if (Object.prototype.hasOwnProperty.call(input, field)) fail('LOCAL_PROVIDER_TOOLS_UNSUPPORTED');
}
function normalizePayload(input) {
  ...
  if (!Array.isArray(input.messages) || input.messages.length < 1 || input.messages.length > 200) fail('INVALID_LOCAL_PROVIDER_MESSAGES');
  const messages = input.messages.map((message) => {
    if (!message || Array.isArray(message) || typeof message !== 'object') fail('INVALID_LOCAL_PROVIDER_MESSAGES');
    if (!['system', 'user', 'assistant', 'tool'].includes(message.role) || typeof message.content !== 'string') fail('INVALID_LOCAL_PROVIDER_MESSAGES');
  ...
  const payload = { model: normalizeModel(input.model), messages, stream: input.stream === true };
  if (Number.isInteger(input.max_tokens)) payload.max_tokens = Math.min(Math.max(input.max_tokens, 1), 32_768);
  if (typeof input.temperature === 'number' && Number.isFinite(input.temperature)) payload.temperature = Math.min(Math.max(input.temperature, 0), 2);
...
async function cancelResponseBody(response, reason) {
  try { await response?.body?.cancel?.(reason); } catch { /* cleanup is best effort */ }
}
async function requireMediaType(response, expected) {
  if (mediaType(response) === expected) return;
  await cancelResponseBody(response, 'hafize-local-provider-invalid-response-type');
  fail('INVALID_LOCAL_PROVIDER_RESPONSE_TYPE', 502);
}
...
async function readBoundedText(response, maxBytes) {
  ...
  if (declared !== null && declared > maxBytes) {
    await cancelResponseBody(response, 'hafize-local-provider-response-too-large');
  ...
  if (response?.body && typeof response.body.getReader === 'function') {
    ...
    try {
      while (true) {
        ...
        size += chunk.length;
        if (size > maxBytes) fail('LOCAL_PROVIDER_RESPONSE_TOO_LARGE', 502);
        chunks.push(chunk);
      }
    } finally {
      if (!completed) try { await reader.cancel?.('hafize-local-provider-read-failed'); } catch { /* best effort */ }
...
function normalizeListedModels(payload) {
  ...
  for (const item of items) {
    ...
    if (!safeModelId(id) || seen.has(id)) continue;
    seen.add(id);
...
async function withJsonDeadline(parentSignal, timeoutMs, execute) {
  ...
  if (parentSignal?.aborted) onParentAbort(); else parentSignal?.addEventListener?.('abort', onParentAbort, { once: true });
  const timer = setTimeout(() => { timedOut = true; controller.abort(); }, timeoutMs);
  timer.unref?.();
  try { return await execute(controller.signal); }
  catch (error) {
    if (timedOut) fail('LOCAL_PROVIDER_TIMEOUT', 504);
    if (parentSignal?.aborted || error?.name === 'AbortError') fail('LOCAL_PROVIDER_CANCELLED', 499);
    throw error;
  } finally {
    clearTimeout(timer);
...
async function cancelStreamIterator(iterator, body) {
  try {
    if (typeof iterator?.return === 'function') await iterator.return();
    else if (typeof body?.cancel === 'function') await body.cancel('hafize-local-provider-consumer-closed');
  } catch { /* cleanup is best effort */ }
}
async function nextStreamItem(iterator, signal) {
  if (!signal) return iterator.next();
  if (signal.aborted) throw makeError('LOCAL_PROVIDER_CANCELLED', 499);
  if (typeof signal.addEventListener !== 'function' || typeof signal.removeEventListener !== 'function') return iterator.next();
  let onAbort;
  const aborted = new Promise((_resolve, reject) => {
    onAbort = () => reject(makeError('LOCAL_PROVIDER_CANCELLED', 499));
    signal.addEventListener('abort', onAbort, { once: true });
  });
  try { return await Promise.race([iterator.next(), aborted]); }
  finally { signal.removeEventListener('abort', onAbort); }
}
function createBoundedStream(body, signal) {
  return Object.freeze({
    async *[Symbol.asyncIterator]() {
      ...
      try {
        ...
        if (!completed) await cancelStreamIterator(iterator, body);
      }
    }
  });
}
async function request(fetchImpl, apiBase, pathname, init, signal) {
  let response;
  try { response = await fetchImpl(`${apiBase}${pathname}`, { ...init, signal, redirect: 'error' }); }
  catch (error) {
    if (signal?.aborted || error?.name === 'AbortError') fail('LOCAL_PROVIDER_CANCELLED', 499);
    fail('LOCAL_PROVIDER_UNAVAILABLE', 502);
  }
  if (!response?.ok) {
    await cancelResponseBody(response, 'hafize-local-provider-http-error');
    fail('LOCAL_PROVIDER_FAILED', Number.isInteger(response?.status) ? response.status : 502);
  }
  return response;
}

export function isLocalProviderModel(model) { return typeof model === 'string' && model.trim().startsWith(MODEL_PREFIX); }
export function createLocalOllamaProvider({ enabled = false, baseUrl = DEFAULT_BASE_URL, fetchImpl = globalThis.fetch, jsonTimeoutMs } = {}) {
  ...
  async function complete(input, { signal } = {}) {
    ...
    return withJsonDeadline(signal, safeJsonTimeoutMs, async (requestSignal) => {
      const response = await request(fetchImpl, apiBase, '/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify(payload) }, requestSignal);
      await requireMediaType(response, 'application/json');
  ...
  async function stream(input, { signal } = {}) {
    ...
    const response = await request(fetchImpl, apiBase, '/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'text/event-stream' }, body: JSON.stringify(payload) }, signal);
    await requireMediaType(response, 'text/event-stream');
    if (!response.body || typeof response.body[Symbol.asyncIterator] !== 'function') {
      await cancelResponseBody(response, 'hafize-local-provider-invalid-stream');
      fail('INVALID_LOCAL_PROVIDER_STREAM', 502);
    }
    return createBoundedStream(response.body, signal);
  }
  async function listModels({ signal } = {}) {
    if (!configured) return [];
    try {
      return await withJsonDeadline(signal, safeJsonTimeoutMs, async (requestSignal) => {
        const response = await request(fetchImpl, apiBase, '/models', { method: 'GET', headers: { Accept: 'application/json' } }, requestSignal);
        await requireMediaType(response, 'application/json');
