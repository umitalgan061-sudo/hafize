...
import { createConnectorOwnerResolver } from './connector-owner-principal.mts';
import { createOAuthTokenStoreRuntime } from './oauth-token-store-runtime.mts';
import { createGmailReadClient } from './gmail-read-client.mts';
import { createGmailReadToolBoundary } from './gmail-read-tool-boundary.mts';
...
function envText(env: Record<string, string | undefined>, name: string) {
  const value = env[name];
  return typeof value === 'string' ? value.trim() : '';
}
function decodeOwnerKey(value: string | undefined) {
...
export function createGmailAgentRuntime({ env = process.env, fetchImpl = globalThis.fetch, createAuthenticator = createBearerPrincipalAuthenticator, createOwnerResolver = createConnectorOwnerResolver, createTokenStoreRuntime = createOAuthTokenStoreRuntime, createReadClient = createGmailReadClient, createBoundary = createGmailReadToolBoundary }: { env?: Record<string, string | undefined>; fetchImpl?: HafizeFetch; createAuthenticator?: Function; createOwnerResolver?: Function; createTokenStoreRuntime?: Function; createReadClient?: Function; createBoundary?: Function; } = {}) {
  ...
  if (configuredCount === 0) {
    return Object.freeze({
      ...
      async connectionStatus() { return Object.freeze({ ok: false as const, error: 'GMAIL_NOT_CONFIGURED' }); },
  ...
  function authenticate(headers: Record<string, any>) {
  ...
  async function connectionStatus({ headers }: { headers?: Record<string, any> } = {}) {
    const principal = authenticate(headers);
    if (!principal) return Object.freeze({ ok: false as const, error: 'AUTH_REQUIRED' });
    ...
    return Object.freeze({ ok: true as const, linked: record !== null });
  }
