...
function normalizeList(input = {}) {
  ...
  const status = input.status == null || input.status === '' ? 'all' : String(input.status).trim();
  const sort = input.sort == null || input.sort === '' ? 'runAt-asc' : String(input.sort).trim();
  const q = typeof input.q === 'string' ? input.q.trim().slice(0, 160) : '';
  const cursor = input.cursor == null ? null : String(input.cursor).trim();
  const limit = input.limit == null ? 50 : Number.parseInt(String(input.limit), 10);
  if (!LIST_STATUSES.has(status) || !LIST_SORTS.has(sort) || !Number.isInteger(limit) || limit < 1 || limit > 100 || (cursor && cursor.length > 1024)) return null;
  return { status, sort, q, cursor: cursor || null, limit };
}

function normalizeIds(scheduleIds) {
  if (!Array.isArray(scheduleIds)) return null;
  const ids = [...new Set(scheduleIds.map((value) => typeof value === 'string' ? value.trim() : '').filter(Boolean))];
  if (!ids.length || ids.length > MAX_BULK_IDS || ids.some((id) => id.length > 120)) return null;
  return ids;
}

export function createScheduleCommandBoundary({ store, registry, createTraceId } = {}) {
  if (typeof store?.add !== 'function' || typeof store?.read !== 'function' || typeof store?.snapshot !== 'function' || typeof store?.cancel !== 'function' || typeof store?.list !== 'function' || typeof store?.cancelMany !== 'function') throw new Error('INVALID_SCHEDULE_COMMAND_BOUNDARY:store');
  ...
  async function list({ principal, query = {} } = {}) {
    const ownerId = principalSubject(principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
    const options = normalizeList(query);
    if (!options) return fail('INVALID_SCHEDULE_COMMAND');
    try {
      const page = store.list({ ownerId, ...options });
      return { ok: true, schedules: page.entries.map(publicSchedule), total: page.total, hasMore: page.hasMore, nextCursor: page.nextCursor };
    } catch (error) { return fail(mapStoreError(error)); }
  }
  ...
  async function cancelMany({ principal, scheduleIds } = {}) {
    const ownerId = principalSubject(principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
    const ids = normalizeIds(scheduleIds);
    if (!ids) return fail('INVALID_SCHEDULE_COMMAND');
    try {
      const allowed = store.snapshot().entries.filter((entry) => entry.ownerId === ownerId && ids.includes(entry.scheduleId) && entry.status === 'scheduled').map((entry) => entry.scheduleId);
      const cancelled = store.cancelMany(allowed);
      return { ok: true, schedules: cancelled.map(publicSchedule), cancelled: cancelled.length };
    } catch (error) { return fail(mapStoreError(error)); }
  }

  return Object.freeze({ create, list, cancel, cancelMany });
}
