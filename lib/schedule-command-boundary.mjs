...
export function createScheduleCommandBoundary({ store, registry, createTraceId } = {}) {
  ...
  function create({ principal, input } = {}) {
  ...
  function list({ principal } = {}) {
    const ownerId = principalSubject(principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
    const schedules = store.snapshot().entries
      .filter((entry) => entry.ownerId === ownerId)
      .map(publicSchedule);
    return { ok: true, schedules };
  }

  function cancel({ principal, scheduleId } = {}) {
    const ownerId = principalSubject(principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
    ...
    try {
      return { ok: true, schedule: publicSchedule(store.cancel(id)) };
    } catch {
      return fail('SCHEDULE_COMMAND_FAILED');
    }
  }
