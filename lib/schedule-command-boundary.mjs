...
export function createScheduleCommandBoundary(options) {
  const { store, registry, createTraceId } = optionsOf(options);
  if (
  ...
  async function create(command) {
    const { principal, input } = optionsOf(command);
  ...
  async function list(command) {
    const ownerId = principalSubject(optionsOf(command).principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
    try {
  ...
  async function cancel(command) {
    const { principal, scheduleId } = optionsOf(command);
    const ownerId = principalSubject(principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
