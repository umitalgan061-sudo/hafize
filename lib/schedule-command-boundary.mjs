...
export function createScheduleCommandBoundary({ store, registry, createTraceId } = {}) {
  if (typeof store?.add !== 'function' || typeof store?.update !== 'function' || typeof store?.read !== 'function' || typeof store?.snapshot !== 'function' || typeof store?.cancel !== 'function') throw new Error('INVALID_SCHEDULE_COMMAND_BOUNDARY:store');
  ...
  async function create({ principal, input } = {}) {
    ...
    let traceId; try { traceId = createTraceId(); } catch { return fail('SCHEDULE_COMMAND_FAILED'); }
  ...
  async function update({ principal, scheduleId, input } = {}) {
    const ownerId = principalSubject(principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
    const id = typeof scheduleId === 'string' ? scheduleId.trim() : '';
    if (!id || !input || Array.isArray(input) || typeof input !== 'object') return fail('INVALID_SCHEDULE_COMMAND');
    for (const key of Object.keys(input)) if (!UPDATE_FIELDS.has(key)) return fail('INVALID_SCHEDULE_COMMAND');
    if (!Object.keys(input).length) return fail('INVALID_SCHEDULE_COMMAND');
    let current;
    ...
    if (current.status !== 'scheduled') return fail('SCHEDULE_NOT_EDITABLE');
    if (input.agentId !== undefined) {
      const agentId = typeof input.agentId === 'string' ? input.agentId.trim() : '';
      if (!registry.agents.some((item) => item?.id === agentId)) return fail('INVALID_AGENT');
    }
    ...
    try { return { ok: true, schedule: publicSchedule(await store.update(id, input)) }; }
    catch (error) { return fail(mapStoreError(error) === 'SCHEDULE_COMMAND_FAILED' && error?.message === 'INVALID_TASK_SCHEDULE_TRANSITION' ? 'SCHEDULE_NOT_EDITABLE' : mapStoreError(error)); }
  }
  ...
  async function cancel({ principal, scheduleId } = {}) {
    ...
    let current; try { current = await store.read(id); } catch { return fail('SCHEDULE_COMMAND_FAILED'); }
    if (!current || current.ownerId !== ownerId) return fail('SCHEDULE_NOT_FOUND');
  ...
  return Object.freeze({ create, update, list, cancel });
}
