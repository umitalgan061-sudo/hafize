...
// Varsayılan parametre yalnız `undefined` için devreye girer; `null` veya
// nesne olmayan bir komut destructuring sırasında ham TypeError fırlatırdı.
// Bu sınır authorization kapısıdır ve hatalı girdide de sözleşmeli sonuç döner.
...
export function createScheduleCommandBoundary({ store, registry, createTraceId } = {}) {
  ...
  async function create(command) {
    const { principal, input } = commandObject(command);
  ...
  async function list(command) {
    const { principal } = commandObject(command);
    const ownerId = principalSubject(principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
    try {
  ...
  async function cancel(command) {
    const { principal, scheduleId } = commandObject(command);
    const ownerId = principalSubject(principal);
    if (!ownerId) return fail('AUTH_REQUIRED');
