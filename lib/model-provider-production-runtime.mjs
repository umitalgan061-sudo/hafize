  ...
  return Object.freeze({
    async *[Symbol.asyncIterator]() {
      ...
      function closeIterator() {
        if (iteratorClosed || typeof iterator?.return !== 'function') return false;
        iteratorClosed = true;
        try {
      ...
      try {
        ...
        if (!iterator || typeof iterator.next !== 'function') throw providerError('INVALID_PROVIDER_STREAM', 502);
        while (true) {
        ...
        if (!completed) {
          closeIterator();
          if (!abortRuntime?.signal?.aborted) {
