...
function publicError(error) {
  ...
  // Upstream provider bodies echo the submitted payload and can carry request
  // ids, internal endpoints and prompt content. The client only renders the
  // stable code, so the raw detail is kept server-side for logs and never
  // serialized into the public body — matching the streamed failure path.
  if (error?.message === 'NVIDIA_CHAT_ERROR') return { status: safeStatus(error), error: 'NVIDIA_CHAT_ERROR' };
...
function setSafeJsonHeaders(res) {
  try {
