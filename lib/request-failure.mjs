...
function publicError(error) {
  ...
  if (error?.message === 'NVIDIA_CHAT_ERROR') {
    // Upstream diagnostic text can echo prompt content or request metadata, so only the
    // stable public code crosses the boundary; the detail stays server-side.
    return { status: safeStatus(error), error: 'NVIDIA_CHAT_ERROR' };
  }
...
export const REQUEST_FAILURE_CONTRACT = Object.freeze({
  // Bound kept for server-side capture of provider diagnostics; it is never sent to clients.
