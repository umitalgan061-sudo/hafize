...
function publicError(error) {
  ...
  if (error?.message === 'NVIDIA_CHAT_ERROR') {
    // The upstream body can echo request content or provider internals, so only
    // the stable public code and a validated status leave the process. The
    // bounded detail stays on the error object for server-side diagnostics.
