...
function requireExactRoster(registry, ids) {
  if (registry.agents.length !== REQUIRED_AGENT_ROSTER.size) {
    throw new Error('INVALID_AGENT_REGISTRY:roster.count');
  }
  for (const [id, expectedKind] of REQUIRED_AGENT_ROSTER) {
    if (!ids.has(id)) throw new Error(`INVALID_AGENT_REGISTRY:roster.missing:${id}`);
    const agent = registry.agents.find((item) => item.id === id);
    if (agent?.kind !== expectedKind) {
      throw new Error(`INVALID_AGENT_REGISTRY:roster.kind:${id}:${expectedKind}`);
    }
    const canDelegate = Array.isArray(agent.toolPolicy?.allow) && agent.toolPolicy.allow.includes('agent.delegate');
    if (expectedKind === 'selector' && !canDelegate) {
      throw new Error(`INVALID_AGENT_REGISTRY:roster.selectorDelegation:${id}`);
    }
    if (expectedKind === 'specialist' && canDelegate) {
      throw new Error(`INVALID_AGENT_REGISTRY:roster.specialistDelegation:${id}`);
    }
  }
  if (registry.defaultAgent !== REQUIRED_DEFAULT_AGENT) {
    throw new Error(`INVALID_AGENT_REGISTRY:roster.default:${REQUIRED_DEFAULT_AGENT}`);
  }
}
...
export async function loadAgentRegistry(fileUrl = DEFAULT_REGISTRY_URL) {
  ...
  for (const agent of registry.agents) {
    ...
    requireString(agent?.kind, `${id}.kind`);
  ...
  requireExactRoster(registry, ids);
  return registry;
}
