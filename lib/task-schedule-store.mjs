...
const ADD_FIELDS = new Set(['traceId', 'agentId', 'task', 'runAt', 'maxAttempts', 'ownerId', 'idempotencyKey']);
const SNAPSHOT_FIELDS = new Set(['entries']);
const ENTRY_FIELDS = new Set([
  'scheduleId', 'traceId', 'ownerId', 'idempotencyKey', 'agentId', 'task', 'runAt', 'status', 'attempts',
...
function restoreSnapshot(initialSnapshot, capacity) {
  ...
  return initialSnapshot.entries.map((raw) => {
    ...
    return {
      ...
      idempotencyKey: raw.idempotencyKey == null ? null : cleanText(raw.idempotencyKey, 'idempotencyKey', 200),
...
export function createTaskScheduleStore({
  ...
  function add(input = {}) {
    ...
    const safeOwnerId = ownerId == null ? null : cleanText(ownerId, 'ownerId', 200);
    const safeIdempotencyKey = idempotencyKey == null
      ? null
      : cleanText(idempotencyKey, 'idempotencyKey', 200);
    const safeAgentId = cleanText(agentId, 'agentId', 120);
    const safeTask = cleanText(task, 'task', 20000);
    const safeRunAt = toIso(runAt, 'runAt');
    ...
    if (safeIdempotencyKey) {
      const existing = entries.find((entry) => (
        entry.ownerId === safeOwnerId && entry.idempotencyKey === safeIdempotencyKey
      ));
      if (existing) {
        const sameRequest = existing.agentId === safeAgentId
          && existing.task === safeTask
          && existing.runAt === safeRunAt
          && existing.maxAttempts === attempts;
        if (!sameRequest) throw new Error('TASK_SCHEDULE_IDEMPOTENCY_CONFLICT');
