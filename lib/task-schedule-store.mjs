...
function decodeCursor(value) {
  ...
  try {
    ...
    return decoded;
  } catch { throw new Error('INVALID_TASK_SCHEDULE:cursor'); }
}
...
function nextScheduleNumber(entries) {
  let max = 0n;
  for (const entry of entries) { const numeric = scheduleNumber(entry.scheduleId); if (numeric != null && numeric > max) max = numeric; }
  return max + 1n;
}
...
export function createTaskScheduleStore({ maxEntries, now = () => new Date(), initialSnapshot = null } = {}) {
  ...
  function claimDue({ limit = 1 } = {}) {
    const claimLimit = Number.isInteger(limit) ? Math.min(Math.max(limit, 1), MAX_CLAIM_BATCH) : 1; const claimedAt = currentDate(); const claimedIso = claimedAt.toISOString();
    ...
    for (const entry of due) { entry.status = 'running'; entry.attempts += 1; entry.updatedAt = claimedIso; } return due.map(clone);
  }
  ...
  function fail(scheduleId, { error = 'SCHEDULE_EXECUTION_FAILED', retryAt = null } = {}) {
    const entry = find(scheduleId); if (entry.status !== 'running') throw new Error('INVALID_TASK_SCHEDULE_TRANSITION'); const safeError = cleanErrorCode(error); const updatedAt = currentDate(); const canRetry = entry.attempts < entry.maxAttempts && retryAt != null; let nextRunAt = null;
  ...
  function defer(scheduleId, { runAt, error = 'SCHEDULE_LEASE_BUSY' } = {}) {
    const entry = find(scheduleId); if (entry.status !== 'running' || entry.attempts < 1) throw new Error('INVALID_TASK_SCHEDULE_TRANSITION'); const updatedAt = currentDate(); const nextRunAt = toIso(runAt, 'runAt'); if (Date.parse(nextRunAt) <= updatedAt.getTime()) throw new Error('INVALID_TASK_SCHEDULE:runAt');
  ...
  function cancelMany(scheduleIds = [], ownerId = null) {
    if (!Array.isArray(scheduleIds) || scheduleIds.length < 1 || scheduleIds.length > MAX_BULK_CANCEL) throw new Error('INVALID_TASK_SCHEDULE:scheduleIds');
    const uniqueIds = [...new Set(scheduleIds.map((value) => typeof value === 'string' ? value.trim() : '').filter(Boolean))]; if (!uniqueIds.length || uniqueIds.length > MAX_BULK_CANCEL) throw new Error('INVALID_TASK_SCHEDULE:scheduleIds');
    const nowIso = currentDate().toISOString(); const selected = new Set(uniqueIds); const updated = [];
    for (const entry of entries) { if (selected.has(entry.scheduleId) && (ownerId == null || entry.ownerId === ownerId) && entry.status === 'scheduled') { entry.status = 'cancelled'; entry.updatedAt = nowIso; updated.push(clone(entry)); } }
    return updated;
  }
  function list({ ownerId = null, status = 'all', q = '', limit = DEFAULT_LIST_LIMIT, cursor = null, sort = 'runAt-asc' } = {}) {
    const safeStatus = normalizeStatus(status); const safeQuery = normalizeQuery(q); const safeLimit = normalizeLimit(limit); const safeSort = normalizeSort(sort); const safeCursor = decodeCursor(cursor); if (safeCursor && safeCursor.sort !== safeSort) throw new Error('INVALID_TASK_SCHEDULE:cursor');
    const query = safeQuery.toLocaleLowerCase('tr-TR'); const filtered = entries.filter((entry) => (ownerId == null || entry.ownerId === ownerId) && (safeStatus === 'all' || entry.status === safeStatus) && (!query || searchable(entry).includes(query))).sort((a, b) => compareEntries(a, b, safeSort));
    const total = filtered.length; const start = safeCursor ? filtered.findIndex((entry) => afterCursor(entry, safeCursor)) : 0; const offset = start < 0 ? total : start; const page = filtered.slice(offset, offset + safeLimit); const hasMore = offset + page.length < total; const last = page.at(-1);
    const nextCursor = hasMore && last ? encodeCursor({ sort: safeSort, key: cursorKey(last, safeSort), id: last.scheduleId }) : null; return { entries: page.map(clone), total, hasMore, nextCursor };
  }
