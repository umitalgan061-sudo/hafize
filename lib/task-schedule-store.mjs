...
const ADD_FIELDS = new Set(['traceId', 'agentId', 'task', 'runAt', 'maxAttempts', 'retryDelayMs', 'ownerId']);
const SNAPSHOT_FIELDS = new Set(['entries']);
const ENTRY_FIELDS = new Set([
  'scheduleId', 'traceId', 'ownerId', 'agentId', 'task', 'runAt', 'status', 'attempts',
  'maxAttempts', 'retryDelayMs', 'lastError', 'createdAt', 'updatedAt'
]);
...
function normalizeRetryDelay(value = DEFAULT_RETRY_DELAY_MS) {
  if (!Number.isSafeInteger(value) || value < MIN_RETRY_DELAY_MS || value > MAX_RETRY_DELAY_MS) {
    throw new Error('INVALID_TASK_SCHEDULE:retryDelayMs');
  }
  return value;
}
...
function normalizeClaimLimit(value) {
  ...
  if (!Number.isSafeInteger(value) || value < 1) throw new Error('INVALID_TASK_SCHEDULE:limit');
  return value;
}
...
function restoreSnapshot(initialSnapshot, capacity = null) {
  ...
  return initialSnapshot.entries.map((raw) => {
    ...
    return {
      ...
      attempts,
      maxAttempts,
      retryDelayMs: normalizeRetryDelay(raw.retryDelayMs ?? DEFAULT_RETRY_DELAY_MS),
      lastError,
...
export function createTaskScheduleStore({
  ...
  const capacity = maxEntries == null
    ? null
    : (Number.isSafeInteger(maxEntries) && maxEntries > 0 ? maxEntries : null);
  if (maxEntries != null && capacity == null) throw new Error('INVALID_TASK_SCHEDULE:maxEntries');
  ...
  function add(input = {}) {
    ...
    const {
      traceId, agentId, task, runAt, maxAttempts = DEFAULT_MAX_ATTEMPTS,
      retryDelayMs = DEFAULT_RETRY_DELAY_MS, ownerId = null
    } = input;
    ...
    const entry = {
      ...
      retryDelayMs: normalizeRetryDelay(retryDelayMs),
      lastError: null,
      createdAt,
      updatedAt: null
    };
  ...
  function claimDue({ limit = null } = {}) {
    ...
    if (claimLimit != null) due = due.slice(0, claimLimit);
  ...
  function reschedule(scheduleId, input = {}) {
    const entry = find(scheduleId);
    if (entry.status !== 'scheduled') throw new Error('TASK_SCHEDULE_NOT_RESCHEDULABLE');
    if (!input || Array.isArray(input) || typeof input !== 'object') throw new Error('INVALID_TASK_SCHEDULE:timing');
    const hasRunAt = Object.hasOwn(input, 'runAt');
    const hasRetryDelay = Object.hasOwn(input, 'retryDelayMs');
    if (!hasRunAt && !hasRetryDelay) throw new Error('INVALID_TASK_SCHEDULE:timing');
    if (hasRunAt) entry.runAt = toIso(input.runAt, 'runAt');
    if (hasRetryDelay) entry.retryDelayMs = normalizeRetryDelay(input.retryDelayMs);
    entry.updatedAt = currentDate().toISOString();
    return clone(entry);
  }
...
export const TASK_SCHEDULE_TIMING_LIMITS = Object.freeze({
  defaultRetryDelayMs: DEFAULT_RETRY_DELAY_MS,
  minRetryDelayMs: MIN_RETRY_DELAY_MS,
