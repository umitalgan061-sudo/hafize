...
function summarize(entries, ownerId = null, capacity = DEFAULT_MAX_ENTRIES) {
  ...
  return { total, counts, due, retrying, capacity: Number.isFinite(capacity) ? 'bounded' : 'unbounded' };
}
...
export function createTaskScheduleStore({ maxEntries, now = () => new Date(), initialSnapshot = null } = {}) {
  ...
  function add(input = {}) { if (!input || Array.isArray(input) || typeof input !== 'object') throw new Error('INVALID_TASK_SCHEDULE:input'); for (const key of Object.keys(input)) if (!ADD_FIELDS.has(key)) throw new Error('INVALID_TASK_SCHEDULE:field'); const { traceId, agentId, task, runAt, maxAttempts = DEFAULT_MAX_ATTEMPTS, ownerId = null } = input; if (Number.isFinite(capacity) && entries.length >= capacity) throw new Error('TASK_SCHEDULE_FULL'); const attempts = Number.isInteger(maxAttempts) ? Math.min(Math.max(maxAttempts, 1), MAX_ATTEMPTS) : DEFAULT_MAX_ATTEMPTS; const createdAt = currentDate().toISOString(); const entry = { scheduleId: `schedule_${nextId++}`, traceId: cleanText(traceId, 'traceId', 128), ownerId: ownerId == null ? null : cleanText(ownerId, 'ownerId', 200), agentId: cleanText(agentId, 'agentId', 120), task: cleanText(task, 'task', 20000), runAt: toIso(runAt, 'runAt'), status: 'scheduled', attempts: 0, maxAttempts: attempts, lastError: null, createdAt, updatedAt: null }; entries.push(entry); return clone(entry); }
  ...
  function stats(ownerId = null) { return summarize(entries, ownerId, capacity); }
