...
const UPDATE_FIELDS = new Set(['agentId', 'task', 'runAt', 'maxAttempts']);
...
function clone(entry) { return { ...entry }; }
...
function restoreSnapshot(initialSnapshot, capacity) {
  ...
  if (initialSnapshot.entries.length > capacity) throw new Error('INVALID_TASK_SCHEDULE_SNAPSHOT:capacity');
  const ids = new Set();
  return initialSnapshot.entries.map((raw) => {
    ...
    if (!idMatch || !Number.isSafeInteger(numericId) || ids.has(scheduleId)) throw new Error('INVALID_TASK_SCHEDULE_SNAPSHOT:scheduleId');
    ids.add(scheduleId);
    ...
    return {
      ...
      runAt: toIso(raw.runAt, 'runAt'), status: raw.status, attempts, maxAttempts, lastError,
      createdAt: toIso(raw.createdAt, 'createdAt'), updatedAt: raw.updatedAt == null ? null : toIso(raw.updatedAt, 'updatedAt')
    };
  });
}
...
export function createTaskScheduleStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), initialSnapshot = null } = {}) {
  const capacity = Number.isInteger(maxEntries) ? Math.min(Math.max(maxEntries, 1), 1024) : DEFAULT_MAX_ENTRIES;
  ...
  function add(input = {}) {
    ...
    const entry = {
      scheduleId: `schedule_${nextId++}`, traceId: cleanText(traceId, 'traceId', 128), ownerId: ownerId == null ? null : cleanText(ownerId, 'ownerId', 200),
      agentId: cleanText(agentId, 'agentId', 120), task: cleanText(task, 'task', 20000), runAt: toIso(runAt, 'runAt'),
      status: 'scheduled', attempts: 0, maxAttempts: attempts, lastError: null, createdAt, updatedAt: null
    };
    entries.push(entry);
    return clone(entry);
  }
  function update(scheduleId, input = {}) {
    const entry = find(scheduleId);
    if (entry.status !== 'scheduled') throw new Error('INVALID_TASK_SCHEDULE_TRANSITION');
    if (!input || Array.isArray(input) || typeof input !== 'object') throw new Error('INVALID_TASK_SCHEDULE:input');
    for (const key of Object.keys(input)) if (!UPDATE_FIELDS.has(key)) throw new Error('INVALID_TASK_SCHEDULE:field');
    if (!Object.keys(input).length) throw new Error('INVALID_TASK_SCHEDULE:input');
    const nextAgentId = input.agentId === undefined ? entry.agentId : cleanText(input.agentId, 'agentId', 120);
    const nextTask = input.task === undefined ? entry.task : cleanText(input.task, 'task', 20000);
    const nextRunAt = input.runAt === undefined ? entry.runAt : toIso(input.runAt, 'runAt');
    const now = currentDate();
    if (Date.parse(nextRunAt) <= now.getTime()) throw new Error('INVALID_TASK_SCHEDULE:runAt');
    const nextMaxAttempts = input.maxAttempts === undefined ? entry.maxAttempts : input.maxAttempts;
    if (!Number.isInteger(nextMaxAttempts) || nextMaxAttempts < Math.max(1, entry.attempts + 1) || nextMaxAttempts > MAX_ATTEMPTS) throw new Error('INVALID_TASK_SCHEDULE:maxAttempts');
    entry.agentId = nextAgentId; entry.task = nextTask; entry.runAt = nextRunAt;
    entry.maxAttempts = nextMaxAttempts; entry.updatedAt = now.toISOString(); entry.lastError = null;
    return clone(entry);
  }
  function claimDue({ limit = 1 } = {}) {
    ...
    const due = entries.filter((entry) => entry.status === 'scheduled' && Date.parse(entry.runAt) <= claimedAt.getTime()).sort((a,b) => a.runAt.localeCompare(b.runAt) || a.scheduleId.localeCompare(b.scheduleId)).slice(0, claimLimit);
  ...
  function fail(scheduleId, { error = 'SCHEDULE_EXECUTION_FAILED', retryAt = null } = {}) {
    ...
    let nextRunAt = null; if (canRetry) { nextRunAt = toIso(retryAt, 'retryAt'); if (Date.parse(nextRunAt) <= updatedAt.getTime()) throw new Error('INVALID_TASK_SCHEDULE:retryAt'); }
    entry.lastError = safeError; entry.updatedAt = updatedAt.toISOString();
    if (canRetry) { entry.status = 'scheduled'; entry.runAt = nextRunAt; } else entry.status = 'failed';
    return clone(entry);
  }
  ...
  return Object.freeze({ add, update, claimDue, complete, fail, defer, cancel, read, snapshot });
}
