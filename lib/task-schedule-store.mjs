...
function toIso(value, label) {
  const date = value instanceof Date ? new Date(value.getTime()) : new Date(/** @type {string | number} */ (value));
  if (Number.isNaN(date.getTime())) throw new Error(`INVALID_TASK_SCHEDULE:${label}`);
  return date.toISOString();
}

/**
 * @param {HafizeScheduleEntry} entry
 * @returns {HafizeScheduleEntry}
 */
...
/**
 * @param {Record<string, any> | null} initialSnapshot
...
/** @param {HafizeScheduleEntry[]} entries */
 ...
 *
 * @param {{ maxEntries?: number; now?: () => Date; initialSnapshot?: Record<string, any> | null }} [options]
 */
export function createTaskScheduleStore({
  ...
  /**
   * @param {string} scheduleId
   * @param {{ error?: string; retryAt?: string | null }} [options]
   */
  ...
  /**
   * @param {string} scheduleId
   * @param {{ runAt?: string; error?: string }} [options]
   */
   ...
   * @overload
   * @param {string} scheduleId
   * @returns {HafizeScheduleEntry | null}
   *
   * @overload
   * @param {null} [scheduleId]
   * @returns {{ entries: HafizeScheduleEntry[] }}
   *
