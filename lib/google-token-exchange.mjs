...
function assertNotCancelled(signal) {
  if (signal?.aborted) fail('GOOGLE_TOKEN_EXCHANGE_FAILED:cancelled');
}
...
export function createGoogleTokenExchange({
  ...
  async function resolveRefreshToken(owner, returned, signal) {
  ...
  async function exchange({
    ...
    const refreshToken = await resolveRefreshToken(owner, tokens.refreshToken, signal);
