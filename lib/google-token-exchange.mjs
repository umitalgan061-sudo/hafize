...
function requestObject(value) {
  ...
  if (Array.isArray(value) || typeof value !== 'object') throw new Error('INVALID_GOOGLE_TOKEN_EXCHANGE:request');
  return value;
}
...
export function createGoogleTokenExchange({ clientId, clientSecret, tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now() } = {}) {
  ...
  async function exchange(input) {
    const { ownerId, code, verifier, redirectUri: callbackUri } = requestObject(input);
