...
function requestObject(value, allowed) {
  ...
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new Error('INVALID_GOOGLE_TOKEN_EXCHANGE:request');
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) throw new Error(`INVALID_GOOGLE_TOKEN_EXCHANGE:request.${key}`);
  }
  return value;
}
...
export function createGoogleTokenExchange({ clientId, clientSecret, tokenStore, fetchImpl = globalThis.fetch, now = () => Date.now() } = {}) {
  ...
  async function exchange(request) {
    const { ownerId, code, verifier, redirectUri: callbackUri } = requestObject(
      request,
      new Set(['ownerId', 'code', 'verifier', 'redirectUri'])
    );
