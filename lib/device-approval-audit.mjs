...
function sanitizeBrowserTarget(value) {
  ...
  let parsed;
  try { parsed = new URL(raw); } catch { throw new Error('INVALID_DEVICE_APPROVAL_AUDIT:target'); }
  if (parsed.protocol !== 'https:' || parsed.username || parsed.password) throw new Error('INVALID_DEVICE_APPROVAL_AUDIT:target');
  return { displayTarget: `${parsed.origin}${parsed.pathname}`, queryRedacted: Boolean(parsed.search || parsed.hash) };
}

function sanitizeAppTarget(value) {
  const appId = cleanString(value, 'target', 64).toLowerCase();
  if (!/^[a-z0-9][a-z0-9._-]{1,63}$/.test(appId)) throw new Error('INVALID_DEVICE_APPROVAL_AUDIT:target');
...
export function createDeviceApprovalAuditEvent({ stage, traceId, action, target, timestamp = Date.now() } = {}) {
  ...
  const safeTarget = normalizedAction === 'browser.open' ? sanitizeBrowserTarget(target) : sanitizeAppTarget(target);
  return Object.freeze({
    type: 'device_approval', stage: normalizedStage, traceId: normalizedTraceId,
    action: normalizedAction, target: safeTarget.displayTarget,
    queryRedacted: safeTarget.queryRedacted, timestamp: Number(timestamp)
  });
}

export const DEVICE_APPROVAL_AUDIT_CONTRACT = Object.freeze({
  stages: Object.freeze([...STAGES]), actions: Object.freeze([...ACTIONS]),
  includesApprovalToken: false, includesRawQueryOrFragment: false, traceIdRequired: true
});
