...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // A query has to select, not just order: ranking alone still returns every memory
    // the owner has, so an unrelated personal record would reach the model's context
    // on any search. Without a query the recency ranking stands on its own.
    const relevant = safeQuery ? ranked.filter((item) => item.metrics.lexical > 0) : ranked;
