...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // Recency alone must not surface unrelated memories: a query only returns
    // records it actually matches, so retrieval never widens the context.
    const relevant = safeQuery ? ranked.filter((item) => item.metrics.lexical > 0 || item.metrics.kindSignal > 0) : ranked;
