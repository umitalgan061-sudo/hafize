...
import { MEMORY_QUALITY_DEFAULTS, rankMemoryRecords } from './memory-quality.mjs';
...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // A query is a lexical match, not just a sort hint: a record that matches
    // nothing must not be handed back (and later injected into model context).
    const ranked = rankMemoryRecords(candidates, safeQuery, { limit: MEMORY_QUALITY_DEFAULTS.maxResults, now: now().getTime() });
    const matched = safeQuery
      ? ranked.filter(({ metrics }) => metrics.lexical > 0 || metrics.kindSignal > 0)
      : ranked;
    const byId = new Map(candidates.map((entry) => [entry.memoryId, entry]));
    const records = matched.slice(0, limit).map(({ memoryId }) => clone(byId.get(memoryId))).filter(Boolean);
