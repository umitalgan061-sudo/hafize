...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    const ranked = rankMemoryRecords(candidates, safeQuery, { limit, now: now().getTime() });
    // Sıralama tek başına filtre değildir: sorgu verildiğinde hiçbir terimi
    // eşleşmeyen kayıtlar da listede kalıyordu; bu, alakasız kişisel notların
    // model bağlamına sızmasına yol açar. Sorgu varsa yalnız eşleşenler döner.
    const matched = safeQuery ? ranked.filter((item) => item.metrics.lexical > 0) : ranked;
    const byId = new Map(candidates.map((entry) => [entry.memoryId, entry]));
    const records = matched.map(({ memoryId }) => clone(byId.get(memoryId))).filter(Boolean);
