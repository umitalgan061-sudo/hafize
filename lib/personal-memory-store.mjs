...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // A query is a filter, not just an ordering: unrelated personal memories must
    // never ride along into a search result or into model context.
    const matched = safeQuery ? ranked.filter(({ metrics }) => metrics.lexical > 0) : ranked;
