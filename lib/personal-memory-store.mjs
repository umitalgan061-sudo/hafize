...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // A specific query must not pull unrelated personal memories into context, so
    // records with no lexical match are dropped; an empty query still browses.
    const relevant = safeQuery ? ranked.filter((item) => Number(item.metrics?.lexical) > 0) : ranked;
