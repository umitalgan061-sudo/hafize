...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // A read is a search, not a dump: records that the query does not touch stay
    // out of the result so unrelated personal memory never reaches model context
    // just because it is recent.
    const records = ranked
      .filter(({ metrics }) => metrics.lexical > 0)
