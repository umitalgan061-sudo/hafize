...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // A query narrows the result set: records the query does not match are not
    // returned just because there is room left in the limit.
    const relevant = safeQuery ? ranked.filter(({ metrics }) => metrics.lexical > 0 || metrics.kindSignal > 0) : ranked;
