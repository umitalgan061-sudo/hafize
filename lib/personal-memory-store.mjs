...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // Retrieval is a search, not a dump: records with no lexical overlap with the query are
    // dropped so unrelated memories never reach a caller (or the model context).
    const records = ranked
      .filter((item) => item.metrics.lexical > 0)
      .map(({ memoryId }) => clone(byId.get(memoryId)))
      .filter(Boolean);
