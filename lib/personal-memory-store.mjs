...
import { rankMemoryRecords } from './memory-quality.mjs';
...
function validateRestoredRecord(raw) {
  if (!raw || Array.isArray(raw) || typeof raw !== 'object') throw new Error('INVALID_MEMORY_STORE_SNAPSHOT:record');
...
function restoreSnapshot(snapshot, capacity) {
  ...
  if (!snapshot || Array.isArray(snapshot) || typeof snapshot !== 'object') throw new Error('INVALID_MEMORY_STORE_SNAPSHOT:input');
  for (const key of Object.keys(snapshot)) if (!SNAPSHOT_FIELDS.has(key)) throw new Error('INVALID_MEMORY_STORE_SNAPSHOT:field');
  if (snapshot.schemaVersion !== SCHEMA_VERSION || !Array.isArray(snapshot.entries)) throw new Error('INVALID_MEMORY_STORE_SNAPSHOT:schema');
...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function currentIso() { return toIso(now(), 'now'); }
  function nextMemoryId() {
    let suffix;
    try { suffix = String(createId()).trim(); } catch { throw new Error('MEMORY_ID_GENERATION_FAILED'); }
  ...
  function snapshot() { return { schemaVersion: SCHEMA_VERSION, entries: entries.map(clone) }; }
  ...
  function read(input) {
    ...
    const candidates = entries.filter((entry) => entry.ownerId === ownerId && (!allowedKinds || allowedKinds.has(entry.kind)));
    // A search must actually search: without a relevance floor every stored
    // memory of the owner came back for any query, and readForContext would
    // then inject unrelated personal data into the model context on every turn.
    // Recency and kind still only order the records that matched a query term.
    const ranked = rankMemoryRecords(candidates, safeQuery, { limit, now: now().getTime(), requireLexicalMatch: Boolean(safeQuery) });
    const byId = new Map(candidates.map((entry) => [entry.memoryId, entry]));
    const records = ranked.map(({ memoryId }) => clone(byId.get(memoryId))).filter(Boolean);
