...
export function createPersonalMemoryStore({ maxEntries = DEFAULT_MAX_ENTRIES, now = () => new Date(), createId = randomUUID, initialSnapshot = null } = {}) {
  ...
  function read(input) {
    ...
    // Retrieval answers a query: a record with no lexical or kind signal is not
    // an answer, and passing it on would widen personal data exposure (the
    // agent context path reads through here too).
    const ranked = rankMemoryRecords(candidates, safeQuery, { limit, now: now().getTime() })
      .filter(({ metrics }) => metrics.lexical > 0 || metrics.kindSignal > 0);
