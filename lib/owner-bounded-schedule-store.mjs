...
function countOwnedActiveEntries(snapshot, ownerId) {
  if (!snapshot || !Array.isArray(snapshot.entries)) fail('INVALID_OWNER_SCHEDULE_SNAPSHOT');
  let count = 0;
  for (const entry of snapshot.entries) {
    if (entry?.ownerId === ownerId && ACTIVE_STATUSES.has(entry?.status)) count += 1;
  }
  return count;
}
...
export function createOwnerBoundedScheduleStore({ store, maxEntriesPerOwner = DEFAULT_OWNER_MAX_ENTRIES } = {}) {
  ...
  function add(input = {}) {
    ...
    return withOwnerQueue(ownerId, async () => {
      ...
      if (countOwnedActiveEntries(snapshot, ownerId) >= ownerLimit) fail('TASK_SCHEDULE_FULL');
