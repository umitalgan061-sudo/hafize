...
import { containsSecretMaterial, normalizeSkillManifest } from './skills-manifest.mts';
...
function normalizeProjectId(value: unknown, allowedProjects: ReadonlySet<string>) {
...
function normalizeArguments(skill: Record<string, any>, input: Record<string, any> = {}) {
...
function classifyTools(agent: Record<string, any>, tools: readonly string[], approvalGranted: boolean, intersectTools: boolean) {
  const granted = [];
  ...
  for (const tool of tools) {
    ...
    else if (!intersectTools) fail(`SKILL_TOOL_ESCALATION:${tool}`);
  }
  // Intersection mode never widens a policy: a tool the agent cannot use is
  // dropped, and a skill with nothing left to run on is refused.
  if (intersectTools && !granted.length && !approvalPending.length) fail('SKILL_NO_AUTHORIZED_TOOL');
  return { granted: Object.freeze(granted), approvalPending };
}

function buildSkillPrompt(skill: Record<string, any>, values: Record<string, any>, grantedTools: readonly string[]) {
  ...
  lines.push(
    '',
    `Bu skill'in bu ajan için etkin araçları: ${grantedTools.join(', ') || 'yok'}.`,
...
/**
 * Beceri manifestlerini kaydeder ve ajan izinleriyle kesiştirir.
 */
export function createSkillsRegistry({ allowedProjects = [] }: { allowedProjects?: string[] } = {}) {
  ...
  function register(rawManifest: unknown, { source, projectId }: { source?: string; projectId?: string } = {}) {
  ...
  function listForAgent(agent: Record<string, any>, { intersectTools = false }: { intersectTools?: boolean } = {}) {
    const visible = [];
    for (const entry of skills.values()) {
      ...
      const usable = (decision) => decision.allowed || decision.reason === 'approval_required';
      if (intersectTools ? !decisions.some(usable) : decisions.some((decision) => !usable(decision))) continue;
  ...
  function resolveInvocation({ agent, name, args, approvalGranted = false, intersectTools = false }: { agent?: any; name?: string; args?: Record<string, any>; approvalGranted?: boolean; intersectTools?: boolean } = {}) {
    ...
    const { granted, approvalPending } = classifyTools(agent, skill.manifest.allowedTools, approvalGranted === true, intersectTools === true);
    ...
    return Object.freeze({
      ...
      tools: granted,
      arguments: values,
      prompt: buildSkillPrompt(skill, values, granted)
    });
  }
