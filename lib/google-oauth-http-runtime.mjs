...
async function readStartJson(request, headers, fallbackReadJson) {
  ...
  for await (const raw of request) {
    ...
    size += chunk.length;
    if (size > MAX_START_BODY_BYTES) {
      try { request.resume?.(); } catch { /* response closes the oversized request */ }
      fail('GOOGLE_OAUTH_HTTP_BODY_TOO_LARGE');
    }
    chunks.push(chunk);
  }
...
export async function createGoogleOAuthHttpRuntime({
  ...
  try {
    ...
    async function handleStart({ request, url, headers }) {
      ...
      const body = await readStartJson(request, headers, readJson);
