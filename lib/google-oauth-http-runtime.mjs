...
function normalizeConfiguredOrigin(value) {
  let url;
  try { url = new URL(value); } catch { fail(`INVALID_GOOGLE_OAUTH_HTTP_RUNTIME:${SESSION_ORIGIN_ENV}`); }
  if (url.protocol !== 'https:' || url.username || url.password || url.pathname !== '/' || url.search || url.hash) {
    fail(`INVALID_GOOGLE_OAUTH_HTTP_RUNTIME:${SESSION_ORIGIN_ENV}`);
  }
  return url.origin;
}
...
function publicError(error) {
  ...
  if (code === 'GOOGLE_OAUTH_HTTP_ORIGIN_REQUIRED') return result(403, { error: 'ORIGIN_REQUIRED' });
...
export async function createGoogleOAuthHttpRuntime({
  ...
  const startOrigin = normalizeConfiguredOrigin(requireText(env, SESSION_ORIGIN_ENV, 512));
  ...
  if (!sessionRuntime?.configured || typeof authenticator?.authenticate !== 'function') fail('INVALID_GOOGLE_OAUTH_HTTP_RUNTIME:cloudSession');
