...
function normalizeEnvelope(input: Record<string, any> | null | undefined) {
...
function normalizeSnapshot(snapshot: Record<string, any> | null | undefined) {
...
export function encryptMemorySnapshot({ ownerId, snapshot, key, random = randomBytes }: { ownerId?: string; snapshot?: unknown; key?: unknown; random?: (size: number) => Uint8Array } = {}) {
  try {
    ...
    return { ok: true as const, envelope: { version: ENVELOPE_VERSION, algorithm: ALGORITHM, iv: iv.toString('base64'), tag: tag.toString('base64'), ciphertext: ciphertext.toString('base64') } };
  } catch (error) {
    return fail(error.message);
  }
}

export function decryptMemorySnapshot({ ownerId, envelope, key }: { ownerId?: string; envelope?: Record<string, any>; key?: unknown } = {}) {
  try {
    const encryptionKey = normalizeKey(key);
    ...
    return { ok: true as const, snapshot: normalizeSnapshot(parsed) };
  } catch (error) {
...
export function createEncryptedMemoryPersistence({ readEnvelope, writeEnvelope, deleteEnvelope, keyProvider }: { readEnvelope?: (ownerId: string) => Promise<unknown>; writeEnvelope?: (ownerId: string, envelope: unknown) => Promise<unknown>; deleteEnvelope?: (ownerId: string) => Promise<unknown>; keyProvider?: (ownerId: string) => unknown; } = {}) {
