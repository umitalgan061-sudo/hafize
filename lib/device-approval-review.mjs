...
import { createDeviceApprovalAuditEvent } from './device-approval-audit.mjs';
...
function normalizeReviewTtl(value) {
  ...
  if (!Number.isInteger(value) || value < 1 || value > MAX_REVIEW_TTL_MS) throw new Error('INVALID_DEVICE_REVIEW_TTL');
  return value;
}
...
function normalizeRequest(input) {
  ...
  if (action === 'browser.open') {
    ...
    return { request, target: deviceApprovalTargetForRequest(request), presentation: { action, title: 'Tarayıcıda bağlantı açılsın mı?', target: `${url.origin}${url.pathname}`, detail: url.search || url.hash ? 'Sorgu ve fragment değerleri güvenlik için onay ekranında gizlenir.' : null } };
  }
  ...
  return { request, target: deviceApprovalTargetForRequest(request), presentation: { action, title: 'Uygulama açılsın mı?', target: appId, detail: null } };
}

export function createDeviceApprovalReviewStore({ approvalStore, now = () => Date.now(), randomId = randomUUID } = {}) {
  ...
  function pruneExpired(currentTime = Number(now())) {
    ...
    let removed = 0;
    for (const [reviewId, entry] of reviews) if (entry.expiresAt <= currentTime) { reviews.delete(reviewId); removed += 1; }
    return removed;
  }
  ...
  function begin({ traceId, request, ttlMs } = {}) {
    try {
      ...
      reviews.set(reviewId, Object.freeze({ reviewId, traceId: normalizedTraceId, action: normalized.request.action, target: normalized.target, issuedAt, expiresAt }));
      return {
        ok: true,
        review: Object.freeze({ id: reviewId, ...normalized.presentation, detail: normalized.presentation.detail || undefined, expiresAt, requiresExplicitConfirmation: true }),
        audit: createDeviceApprovalAuditEvent({ stage: 'review_started', traceId: normalizedTraceId, action: normalized.request.action, target: normalized.target, timestamp: issuedAt })
      };
    } catch (error) {
  ...
  function confirm({ reviewId, traceId } = {}) {
    try {
      ...
      const approval = approvalStore.issue({ traceId: entry.traceId, action: entry.action, target: entry.target });
      if (!approval?.ok) return fail('DEVICE_REVIEW_APPROVAL_ISSUE_FAILED');
      return {
        ok: true,
        approval: approval.lease,
        audit: createDeviceApprovalAuditEvent({ stage: 'approval_issued', traceId: entry.traceId, action: entry.action, target: entry.target, timestamp: currentTime })
      };
    } catch (error) {
      return fail(error.message);
    }
...
export const DEVICE_APPROVAL_REVIEW_CONTRACT = Object.freeze({ actions: Object.freeze([...REVIEW_ACTIONS]), defaultTtlMs: REVIEW_TTL_MS, maxTtlMs: MAX_REVIEW_TTL_MS, singleUseReview: true, queryValuesVisible: false, explicitConfirmationRequired: true, redactedAuditRequired: true });
