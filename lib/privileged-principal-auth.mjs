...
const CLOUD_FIELDS = Object.freeze([PASSWORD_HASH_ENV, SIGNING_KEY_ENV, SUBJECT_ENV, ORIGIN_ENV]);
...
function readCloudConfig(env) {
  ...
  if (values.every((value) => !value) && !previousSigningKey) return null;
  ...
  return Object.freeze({ values, previousSigningKey, ttlMs, origin: normalizeOrigin(values[3]) });
}
...
function createOptionalCloud(env, injectedAuthenticator) {
  ...
  try {
    const authenticator = createRevocableCloudSessionAuth({
      ...
      ...(config.previousSigningKey ? { previousSigningKey: config.previousSigningKey } : {}),
...
export const PRIVILEGED_PRINCIPAL_AUTH_LIMITS = Object.freeze({
  ...
  minTtlMs: MIN_TTL_MS,
  maxTtlMs: MAX_TTL_MS,
  maxCloudVerificationSigningKeys: 2
});
