...
const REVOCATION_FILE_ENV = 'HAFIZE_CLOUD_SESSION_REVOCATION_FILE';
...
function createOptionalBearer(env, { tokenEnv, subjectEnv }) {
  ...
  try { return createBearerPrincipalAuthenticator({ token, subject }); }
  catch { fail('bearerConfig'); }
}
...
function readCloudConfig(env) {
  ...
  const revocationFilePath = envText(env, REVOCATION_FILE_ENV, 1024);
  ...
  return Object.freeze({ values, ttlMs, origin: normalizeOrigin(values[3]), revocationFilePath });
}
...
function createOptionalCloud(env, injectedAuthenticator) {
  ...
  try {
    const authenticator = createRevocableCloudSessionAuth({
      ...
      ttlMs: config.ttlMs,
      ...(config.revocationFilePath ? { revocationFilePath: config.revocationFilePath } : {})
    });
    return Object.freeze({ authenticator, origin: config.origin });
  } catch { fail('cloudConfig'); }
}
...
export function createPrivilegedPrincipalAuthenticator({
  ...
  return Object.freeze({ authenticate, modes: Object.freeze({ bearer: Boolean(bearer), cloudSession: Boolean(cloud) }) });
}
...
export const PRIVILEGED_PRINCIPAL_AUTH_LIMITS = Object.freeze({
  ...
  minTtlMs: MIN_TTL_MS,
  maxTtlMs: MAX_TTL_MS,
  revocationFileEnv: REVOCATION_FILE_ENV
});
