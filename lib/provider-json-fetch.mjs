...
export async function fetchBoundedProviderJson({
  ...
  try {
    ...
    try {
      ...
      if (controller.signal.aborted) fail(errorPrefix, 'timeout');
      if (error?.code?.startsWith?.(`${errorPrefix}:`)) throw error;
      fail(errorPrefix, 'network');
    }
    if (!response?.ok) fail(errorPrefix, 'http');
    return await readJson(response, safeMaxBytes, errorPrefix);
  } catch (error) {
    if (controller.signal.aborted && !error?.code?.startsWith?.(`${errorPrefix}:`)) fail(errorPrefix, 'timeout');
    throw error;
  } finally {
    clearTimeout(timer);
  }
