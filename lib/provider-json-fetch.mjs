...
const RESPONSE_BODY_CLOSED = Symbol('responseBodyClosed');

function fail(prefix, reason, { responseBodyClosed = false } = {}) {
  const error = new Error(`${prefix}:${reason}`);
  error.code = error.message;
  if (responseBodyClosed) error[RESPONSE_BODY_CLOSED] = true;
  throw error;
}
  ...
  try {
    while (true) {
      ...
      if (size > maxBytes) {
        ...
        fail(prefix, 'response-too-large', { responseBodyClosed });
      }
      chunks.push(chunk);
    }
  } finally {
...
export async function fetchBoundedProviderJson({
  ...
  try {
    ...
    try {
      ...
      if (!error?.[RESPONSE_BODY_CLOSED]) await cancelResponseBody(response);
      throw error;
    }
  } catch (error) {
