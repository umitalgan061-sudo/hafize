...
function contentDigest(content) {
  return createHash('sha256').update('hafize:github-write-content:v1\0').update(content, 'utf8').digest('base64url');
}
...
function canonicalCommand(command) {
  ...
  if (command.operation === 'file.update') {
    return JSON.stringify([
      'file.update', command.repository, command.branch, command.path,
      command.expectedBlobSha, command.commitMessage, contentDigest(command.content)
    ]);
  }
...
function parseToken(token, secret) {
  ...
  let actualMac;
  try { actualMac = Buffer.from(encodedMac, 'base64url'); } catch { fail('GITHUB_WRITE_APPROVAL_INVALID'); }
  const expectedMac = sign(secret, body);
  if (actualMac.length !== expectedMac.length || !timingSafeEqual(actualMac, expectedMac)) fail('GITHUB_WRITE_APPROVAL_INVALID');
  let payload;
  try { payload = JSON.parse(Buffer.from(body, 'base64url').toString('utf8')); } catch { fail('GITHUB_WRITE_APPROVAL_INVALID'); }
...
export function createGitHubWriteApprovalBoundary({
  ...
  if (typeof replayStore?.claim !== 'function') fail('INVALID_GITHUB_WRITE_APPROVAL_REPLAY_STORE');
  ...
  function prepare(input, { principal } = {}) {
    ...
    const payload = { v: 1, n: nonceBytes.toString('base64url'), e: issuedAt + ttlMs, o: ownerId, d: commandDigest(command) };
  ...
  function consume(input, { principal, approvalToken } = {}) {
    ...
    if (payload.e > at + MAX_TTL_MS || payload.o !== ownerId || payload.d !== commandDigest(command)) fail('GITHUB_WRITE_APPROVAL_MISMATCH');
    const finish = (claimed) => {
      if (!claimed) fail('GITHUB_WRITE_APPROVAL_REPLAYED');
      return command;
    };
...
export const GITHUB_WRITE_APPROVAL_LIMITS = Object.freeze({ defaultTtlMs: DEFAULT_TTL_MS, maxTtlMs: MAX_TTL_MS });
