...
const exchange = createGoogleTokenExchange({
  ...
  fetchImpl: async (url, options) => {
    ...
    return { ok: true, async json() { return { access_token: 'access-token-value', refresh_token: 'refresh-token-value', token_type: 'Bearer', expires_in: 3600, scope: 'openid email https://www.googleapis.com/auth/gmail.readonly' }; } };
  },
  now: () => 1_700_000_000_000
});

const receipt = await exchange.exchange({ ownerId: 'user:123', code: 'authorization-code', verifier: 'a'.repeat(43), redirectUri: 'https://hafize.example.test/oauth/google' });
...
assert.equal(saved[0].tokenRecord.accessToken, 'access-token-value');
assert.equal(saved[0].tokenRecord.refreshToken, 'refresh-token-value');
...
const publicClient = createGoogleTokenExchange({
  clientId: 'public-client', tokenStore: { async save() {} },
  fetchImpl: async (url, options) => { withoutSecretRequests.push(options.body); return { ok: true, async json() { return { access_token: 'token-value', token_type: 'bearer', expires_in: 120 }; } }; }
});
...
for (const invalidFactory of [{}, { clientId: '', tokenStore: { save() {} } }, { clientId: 'id', tokenStore: {} }, { clientId: 'id', tokenStore: { save() {} },
  fetchImpl: null }]) assert.throws(() => createGoogleTokenExchange(invalidFactory), /INVALID_GOOGLE_TOKEN_EXCHANGE/);
