...
await context.canvaReadTool.execute({ operation: 'design.get', params: { designId: 'DAF42' } }, { signal: controller.signal });
const executeCalls = calls.filter((item) => item[0] === 'execute');
assert.equal(executeCalls.length, 2);
assert.equal(executeCalls[1][2].principal, principal);
assert.equal(executeCalls[1][2].signal, controller.signal);
