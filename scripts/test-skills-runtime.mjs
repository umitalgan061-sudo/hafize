...
// The general agent has no repo.read permission, so the read-only code inspection
// skill stays hidden for it; visibility follows the agent tool policy, not the catalog.
...
assert.throws(() => runtime.resolveForAgent({ agent: general, skillId: 'code-inspection', args: { focus: 'auth' } }), /SKILL_TOOL_ESCALATION:repo.read/);
