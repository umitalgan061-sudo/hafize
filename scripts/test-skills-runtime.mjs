...
// Discovery follows each agent's tool policy: hafize-general has no repo.read, so the
// repo-reading skill stays invisible to it while the reviewer sees exactly that one.
...
// Selection never reaches past the agent's own discovery surface.
...
// Lexical selection only: a paraphrase that shares no term with the manifest text stays unmatched.
assert.equal(runtime.selectForAgent(general, 'hangi servisler hazır?', { minScore: 0.3 }), null);
...
// createSkillsRuntime is async, so its guards reject rather than throw synchronously.
