...
// Discovery is gated by the agent's own tool policy: hafize-general has no
// repo.read grant, so the repo-reading skill stays hidden from it, and the
// read-only reviewer sees only that one.
...
// Selection is deterministic and trigger-driven, so a declared trigger selects the
// skill while an unrelated free-form question stays below the score floor.
assert.equal(runtime.selectForAgent(general, 'runtime kontrol').name, 'runtime-diagnostics');
assert.equal(runtime.selectForAgent(general, 'hangi servisler hazır?', { minScore: 0.2 }), null);
...
// createSkillsRuntime is async, so its guards surface as rejections.
