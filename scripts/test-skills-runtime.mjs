...
// The primary agent has no repo.read grant, so the repository inspection skill
// stays hidden from it and is only offered to the reviewer specialist.
...
// Selection is scoped by policy: the repository skill is only selectable for an
// agent that actually holds repo.read.
...
// Selection keys off declared triggers, so the diagnostics trigger is used here.
...
// createSkillsRuntime is async: its validation surfaces as a rejection.
