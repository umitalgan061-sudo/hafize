...
// Least privilege: the primary agent only sees skills its own tool policy can run, so
// repo-reading work stays with the specialists it delegates to.
...
// Selection never surfaces a skill the agent cannot run, whatever the query matches.
assert.equal(runtime.selectForAgent(general, 'kod incele', { minScore: 0.3 }), null);
assert.equal(runtime.rankForAgent(general, 'kod incele').some(({ name }) => name === 'code-inspection'), false);
...
// createSkillsRuntime is async: its guards surface as rejections, not synchronous throws.
