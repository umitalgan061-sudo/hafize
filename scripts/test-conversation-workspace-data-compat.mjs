...
// The matrix must stay broad and free of duplicate cases.
assert.ok(compatibilityCases.length >= 70, 'compatibility matrix stays broad');
assert.equal(new Set(compatibilityCases.map((item) => item.name)).size, compatibilityCases.length, 'compatibility cases are unique');
