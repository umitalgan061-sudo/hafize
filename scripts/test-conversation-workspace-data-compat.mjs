...
const fragments = [
  ...
  ['byId.has(conversation.id) ? { ...conversation, id: createId(\'import\') }', 'collision path is recognized'],
  ...
  ['const normalized = normalizeConversationList(incoming)', 'normalized input is canonicalized'],
...
// Coverage floor plus uniqueness: the table is the documented compatibility
// matrix, so a case must never be silently dropped or duplicated.
assert.equal(compatibilityCases.length >= 70, true);
assert.equal(new Set(compatibilityCases.map((item) => item.name)).size, compatibilityCases.length);
