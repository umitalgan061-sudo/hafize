...
let stopped = 0;
stopStream({ getTracks: () => [{ stop: () => { stopped += 1; } }, { stop: () => { stopped += 1; } }] });
...
function createDocument({ width = 1600, height = 900, blobType = 'image/jpeg' } = {}) {
  ...
  return {
    ...
    createElement(tag) {
      if (tag === 'video') return {
        muted: false, playsInline: false, srcObject: null, videoWidth: width, videoHeight: height,
        play: async () => undefined, addEventListener() {}, removeEventListener() {}
      };
      if (tag === 'canvas') return {
        width: 0, height: 0,
        getContext: () => ({ drawImage: (...args) => drawn.push(args.slice(1)) }),
        toBlob: (callback, type, quality) => { assert.equal(type, 'image/jpeg'); assert.equal(quality, 0.82); callback({ type: blobType, size: 123 }); }
      };
...
await assert.rejects(
  captureScreenFrame({ mediaDevices: { getDisplayMedia: async () => { captureCalls += 1; return captureStream; } }, document }),
  /SCREEN_CAPTURE_REQUIRES_EXPLICIT_USER_INTENT/
);
assert.equal(captureCalls, 0, 'missing explicit intent must fail before getDisplayMedia');
assert.equal(captureStopped, 0);

const capture = await captureScreenFrame({
  mediaDevices: { getDisplayMedia: async (value) => { captureCalls += 1; assert.deepEqual(value, { video: { frameRate: { ideal: 1, max: 5 } }, audio: false }); return captureStream; } },
  document,
...
assert.deepEqual(capture.metadata, { explicitUserIntent: true, mimeType: 'image/jpeg', byteLength: 123, width: 1280, height: 720 });
...
await assert.rejects(
  captureScreenFrame({ mediaDevices: { getDisplayMedia: async () => { const error = new Error('denied'); error.name = 'NotAllowedError'; throw error; } }, document, explicitUserIntent: true }),
...
console.log('screen share contract OK: explicit user intent, metadata allowlist, minimization and track cleanup');
