...
function createHarness({ preexisting = false, includeMessages = true } = {}) {
  ...
  return { registry, documentRef, MutationObserverImpl, observers, historyHead, messages, existingTrigger, existingControl };
}

{
  ...
  assert.equal(trigger.removed, true, 'controller-created trigger should be removed');
  assert.equal(control.removed, true, 'controller-created control should be removed');
  assert.equal(controller.destroy(), false);
  assert.equal(controller.open(), false, 'destroyed controller must be inert');
...

{
  ...
  const controller = find.createController(harness);
  assert.equal(controller.mount(), true);
  const { wrapper, input, previous, next, close } = harness.existingControl;
  assert.equal(harness.existingTrigger.listenerCount('click'), 1);
  assert.equal(input.listenerCount('input'), 1);
  assert.equal(controller.destroy(), true);
  assert.equal(harness.existingTrigger.removed, false, 'host-owned trigger must survive teardown');
  assert.equal(wrapper.removed, false, 'host-owned control must survive teardown');
  assert.equal(harness.registry.get(find.TRIGGER_ID), harness.existingTrigger);
  assert.equal(harness.registry.get(find.CONTROL_ID), wrapper);
...

{
  ...
  const controller = find.createController(harness);
  assert.equal(controller.mount(), false, 'missing messages surface must fail closed');
  assert.equal(controller.isMounted(), false);
