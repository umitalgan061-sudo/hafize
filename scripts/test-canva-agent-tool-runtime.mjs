...
const result = await executeNvidiaToolCall(agent, {
  id: 'call_canva_read_1', type: 'function', function: { name: 'canva_read', arguments: JSON.stringify({ operation: 'design.get', params: { designId: 'DAF123' } }) }
...
assert.deepEqual(await executeNvidiaToolCall(agent, {
  id: 'call_canva_read_2', type: 'function', function: { name: 'canva_read', arguments: '{}' }
}, { canvaReadTool, canvaReadAuthenticated: false }), { ok: false, error: 'TOOL_UNAVAILABLE' });
assert.deepEqual(await executeNvidiaToolCall(agent, {
  id: 'call_canva_read_3', type: 'function', function: { name: 'canva_read', arguments: '[]' }
