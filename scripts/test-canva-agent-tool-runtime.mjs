...
const result = await executeNvidiaToolCall(agent, { id: 'call-4', function: { name: 'canva_read', arguments: JSON.stringify({ operation: 'design.get', params: { designId: 'DAF123' } }) }
...
assert.deepEqual(await executeNvidiaToolCall(agent, { id: 'call-5', function: { name: 'canva_read', arguments: '{}' }
}, { canvaReadTool, canvaReadAuthenticated: false }), { ok: false, error: 'TOOL_UNAVAILABLE' });
assert.deepEqual(await executeNvidiaToolCall(agent, { id: 'call-6', function: { name: 'canva_read', arguments: '[]' }
