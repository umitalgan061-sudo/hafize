...
assert.equal(source.includes("client.post(PREPARE_PATH, { command }, { signal: request.controller.signal })"), true);
assert.equal(source.includes("client.post(EXECUTE_PATH, { command: snapshot.command, approvalToken: snapshot.approvalToken }, { signal: request.controller.signal })"), true);
assert.equal(source.includes("addListener(approveButton, 'click', execute)"), true);
assert.equal(source.includes("addListener(form, 'submit', prepare)"), true);
