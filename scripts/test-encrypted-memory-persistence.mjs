...
const encrypted = encryptMemorySnapshot({ ownerId: 'user-alice', snapshot, key, random: (size) => {
...
assert.deepEqual(decryptMemorySnapshot({ ownerId: 'user-alice', envelope: encrypted.envelope, key }), { ok: true, snapshot });
assert.deepEqual(decryptMemorySnapshot({ ownerId: 'user-bob', envelope: encrypted.envelope, key }), { ok: false, error: 'MEMORY_DECRYPT_FAILED' });
assert.deepEqual(decryptMemorySnapshot({ ownerId: 'user-alice', envelope: encrypted.envelope, key: otherKey }), { ok: false, error: 'MEMORY_DECRYPT_FAILED' });

const tampered = { ...encrypted.envelope, ciphertext: Buffer.from('tampered').toString('base64') };
assert.deepEqual(decryptMemorySnapshot({ ownerId: 'user-alice', envelope: tampered, key }), { ok: false, error: 'MEMORY_DECRYPT_FAILED' });
...
const persistence = createEncryptedMemoryPersistence({
  keyProvider: async (ownerId) => { calls.push(['key', ownerId]); return key; },
  readEnvelope: async (ownerId) => { calls.push(['read', ownerId]); return records.get(ownerId) ?? null; },
  writeEnvelope: async (ownerId, envelope) => { calls.push(['write', ownerId]); records.set(ownerId, envelope); },
  deleteEnvelope: async (ownerId) => { calls.push(['delete', ownerId]); records.delete(ownerId); }
});
...
for (const options of [{}, { readEnvelope: async () => null }, { readEnvelope: async () => null, writeEnvelope: async () => {} }]) {
