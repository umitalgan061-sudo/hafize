...
// Display-mode contracts live in the stylesheet, ARIA contracts in the module
// that builds the DOM.
...
for (const [fragment, label] of ariaContracts) assertAttributeDeclared(workspace, fragment, label);
...
const shortcutBehaviorContracts = [
  ...
  ['matchesEscape(event)', 'Escape handling is centralized'],
...
const checkedContracts = responsiveContracts.length
  + cssAccessibilityContracts.length
  + ariaContracts.length
  + shortcutVisualContracts.length
  + shortcutBehaviorContracts.length;
console.log(`conversation-workspace-navigation: responsive, accessibility, shortcut, shell-order and no-network contracts passed (${checkedContracts} checks)`);
