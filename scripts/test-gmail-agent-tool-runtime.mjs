...
const result = await executeNvidiaToolCall(agent, {
  id: 'call_gmail_1', function: { name: 'gmail_read', arguments: JSON.stringify({ operation: 'message.get', params: { messageId: 'm1' } }) }
...
assert.deepEqual(await executeNvidiaToolCall(agent, {
  id: 'call_gmail_2', function: { name: 'gmail_read', arguments: '{}' }
}, { gmailReadTool, gmailReadAuthenticated: false }), { ok: false, error: 'TOOL_UNAVAILABLE' });
assert.deepEqual(await executeNvidiaToolCall(agent, {
  id: 'call_gmail_3', function: { name: 'gmail_read', arguments: '[]' }
