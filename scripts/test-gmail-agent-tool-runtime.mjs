...
const result = await executeNvidiaToolCall(agent, {
  id: 'call_1', type: 'function', function: { name: 'gmail_read', arguments: JSON.stringify({ operation: 'message.get', params: { messageId: 'm1' } }) }
...
assert.deepEqual(await executeNvidiaToolCall(agent, {
  id: 'call_2', type: 'function', function: { name: 'gmail_read', arguments: '{}' }
}, { gmailReadTool, gmailReadAuthenticated: false }), { ok: false, error: 'TOOL_UNAVAILABLE' });
assert.deepEqual(await executeNvidiaToolCall(agent, {
  id: 'call_3', type: 'function', function: { name: 'gmail_read', arguments: '[]' }
