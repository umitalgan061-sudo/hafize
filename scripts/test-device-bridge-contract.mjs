...
for (const options of [{}, { systemInfo: async () => ({}) }, { systemInfo: async () => ({}), openExternal: async () => {} }, { systemInfo: async () => ({}), openExternal: async () => {}, openApp: async () => {},
  allowedApps: 'browser.chrome' }]) assert.throws(() => createDeviceBridge(options), /INVALID_DEVICE_BRIDGE/);
...
for (const forged of [{ action: 'browser.open', url: 'https://example.com', explicitUserIntent: true }, { action: 'app.open', appId: 'browser.chrome', explicitUserIntent: true }, { action: 'shell.run' }, { action: 'system.info',
  url: 'https://example.com' }]) assert.equal(authorizeDeviceToolRequest(hafize, forged, { approvalGranted: true }).ok, false);
...
assert.deepEqual(await executeDeviceToolRequest(hafize, { action: 'browser.open', url: 'https://example.com/a' }, { traceId, approvalToken: issued.lease.token, approvalStore, deviceBridge: boundaryBridge }), { ok: false,
  error: 'DEVICE_APPROVAL_NOT_FOUND', reason: 'approval_required' });
const wrongTarget = approvalStore.issue({ traceId, action: 'app.open', target: 'browser.chrome', ttlMs: 1000 });
assert.deepEqual(await executeDeviceToolRequest(hafize, { action: 'app.open', appId: 'editor.vscode' }, { traceId, approvalToken: wrongTarget.lease.token, approvalStore, deviceBridge: boundaryBridge }), { ok: false,
  error: 'DEVICE_APPROVAL_TARGET_MISMATCH', reason: 'approval_required' });
...
const audit = createDeviceApprovalAuditEvent({ stage: 'approval_issued', traceId, action: 'browser.open', target: 'https://example.com/private?token=secret#fragment', timestamp: clock });
assert.equal(audit.target, 'https://example.com/private');
assert.equal(audit.queryRedacted, true);
assert.equal(JSON.stringify(audit).includes('secret'), false);
assert.equal(JSON.stringify(audit).includes('fragment'), false);
assert.throws(() => createDeviceApprovalAuditEvent({ stage: 'approval_issued', traceId, action: 'browser.open', target: 'http://example.com' }), /INVALID_DEVICE_APPROVAL_AUDIT/);
