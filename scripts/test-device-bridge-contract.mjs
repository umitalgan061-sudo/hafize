...
for (const options of /** @type {any[]} */ ([{}, { systemInfo: async () => ({}) }, { systemInfo: async () => ({}), openExternal: async () => {} }, { systemInfo: async () => ({}), openExternal: async () => {}, openApp: async () => {}, allowedApps: 'browser.chrome' }])) assert.throws(() => createDeviceBridge(options), /INVALID_DEVICE_BRIDGE/);
...
const wrongTraceReview = /** @type {any} */ (reviewStore.begin({ traceId, request: { action: 'app.open', appId: 'Browser.Chrome' } }));
...
assert.equal(reviewStore.size(), 0);
const expiredReview = /** @type {any} */ (reviewStore.begin({ traceId, request: { action: 'app.open', appId: 'browser.chrome' }, ttlMs: 5 }));
clock += 6;
assert.deepEqual(reviewStore.confirm({ reviewId: expiredReview.review.id, traceId }), { ok: false, error: 'DEVICE_REVIEW_EXPIRED' });
const cancelledReview = /** @type {any} */ (reviewStore.begin({ traceId, request: { action: 'app.open', appId: 'browser.chrome' } }));
