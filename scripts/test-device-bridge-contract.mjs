...
const calls = [];
const bridge = createDeviceBridge({ allowedApps: [
  'browser.chrome',
  'editor.vscode'
], systemInfo: async () => ({ platform: 'win32', arch: 'x64', release: '10.0.99999', hostname: 'desktop' }), openExternal: async (url) => calls.push(['external', url]), openApp: async (appId) => calls.push(['app', appId]) });
...
for (const options of [
  ...
  { systemInfo: async () => ({}), openExternal: async () => {}, openApp: async () => {}, allowedApps: 'browser.chrome' }
...
assert.deepEqual(
  await executeDeviceToolRequest(hafize, { action: 'browser.open', url: 'https://example.com/a' }, { traceId, approvalToken: issued.lease.token, approvalStore, deviceBridge: boundaryBridge }),
...
assert.deepEqual(
  await executeDeviceToolRequest(hafize, { action: 'app.open', appId: 'editor.vscode' }, { traceId, approvalToken: wrongTarget.lease.token, approvalStore, deviceBridge: boundaryBridge }),
...
for (const bad of [
  { traceId, request: { action: 'shell.run' } },
  { traceId, request: { action: 'browser.open', url: 'http://example.com' } },
  { traceId, request: { action: 'browser.open', url: 'https://user:pass@example.com' } },
  { traceId, request: { action: 'app.open', appId: '../terminal' } }
]) assert.equal(reviewStore.begin(bad).ok, false);
