...
for (const token of ['PromptLibraryCommandPalette','MAX_RESULTS = 12','MAX_QUERY = 120','score(item, query)','results(query)','role','listbox','aria-selected','ArrowDown','ArrowUp','Escape','Enter']) assert.match(text, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
