...
for (const token of ['PromptLibraryCommandPalette','MAX_RESULTS = 12','MAX_QUERY = 120','score(item, normalized)','searchPromptLibrary(query','role','listbox','aria-selected','ArrowDown','ArrowUp','Escape','Enter']) assert.match(text, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
