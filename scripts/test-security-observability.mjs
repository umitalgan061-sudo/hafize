...
const result = logger.record({
  ...
  outcome: 'blocked',
  metadata: {
    provider: 'session', authToken: 'secret-value', Authorization: 'Bearer hidden', visible: 'kept', count: 3, ok: true, dropped: null
  }
});
...
const supplied = logger.record({ event: 'request.finished', requestId: ' client-request-1 ', route: '/api/chat?token=should-not-be-parsed#fragment', method: 'get', outcome: 'success' });
assert.equal(supplied.requestId, 'client-request-1');
assert.equal(supplied.route, '/api/chat');
assert.equal(events[1].includes('should-not-be-parsed'), false);
const fullUrl = logger.record({ event: 'request.finished', route: 'https://example.invalid/api/agent/run?access_token=hidden', method: 'get' });
assert.equal(fullUrl.route, '/api/agent/run');
