...
try {
  ...
  const envelope = { schemaVersion: 1, snapshot: { entries: [{ scheduleId: 'schedule_1', task: 'Gizli görev metni' }] } };
  ...
  assert.throws(() => createEncryptedFileScheduleAdapter({ filePath, key, maxFileBytes: 100 }), /INVALID_ENCRYPTED_SCHEDULE_ADAPTER:maxFileBytes/);
} finally {
  key.fill(0);
  otherKey.fill(0);
}
