...
const router = createModelProviderRouter({
  nvidiaComplete: async (payload, signal) => { calls.push(['nvidia', payload, signal]); return { id: 'nvidia-result' }; },
  localComplete: async (payload, signal) => { calls.push(['local', payload, signal]); return { id: 'local-result' }; },
  localEnabled: true
});
...
assert.deepEqual(await router.complete({ payload: { model: 'm' }, signal }), { provider: 'nvidia', result: { id: 'nvidia-result' } });
assert.deepEqual(await router.complete({ provider: 'local', payload: { model: 'm' }, signal }), { provider: 'local', result: { id: 'local-result' } });
