...
assert.match(smartFill, /clamp\([^)]*MAX_VALUE\)/, 'values are clamped to MAX_VALUE');
assert.match(smartFill, /localStorage/);
assert.match(smartFill, /hafize\.prompt-library\.smart-fill\.v1/, 'presets live under the documented storage key');
assert.match(smartFill, /navigator\?\.clipboard/);
assert.match(smartFill, /setAttribute\('aria-live', 'polite'\)/);
// Focus trap: Tab cycles inside the dialog and Shift+Tab wraps backwards.
assert.match(smartFill, /event\.key !== 'Tab'/, 'the dialog traps Tab');
assert.match(smartFill, /event\.shiftKey/, 'Shift+Tab wraps backwards through the dialog');
// Cmd/Ctrl shortcuts belong to the palette and keyboard modules; a modal that
// also claimed them would shadow them while it is open.
assert.doesNotMatch(smartFill, /metaKey/, 'the dialog claims no Cmd/Ctrl shortcut');
assert.doesNotMatch(smartFill, /ctrlKey/, 'the dialog claims no Cmd/Ctrl shortcut');
