...
const registry = {
  policy: { maxDelegationDepth: 2, maxParallelAgents: 3 },
  agents: [
    {
      id: 'parent',
      name: 'Parent',
      kind: 'primary',
      toolPolicy: { default: 'deny', allow: ['agent.delegate'], approvalRequired: [] }
    },
    {
      id: 'specialist',
      name: 'Specialist',
      kind: 'specialist',
      toolPolicy: { default: 'deny', allow: ['repo.read'], approvalRequired: [] }
    }
  ]
};
...
const runLedger = {
  snapshot() {
    return { entries: ledgerEntries.slice() };
  },
  recordDelegationStart(agentId, { parentTaskId }) {
    const taskId = `delegation-${++sequence}`;
    ledgerEntries.push({ action: 'agent.delegate', taskId, agentId, parentTaskId });
    return { taskId };
  },
  recordDelegationFinish(taskId, result) {
    ledgerEntries.push({ action: 'agent.delegate.finish', taskId, ...result });
  }
};
...
const delegator = createAgentDelegator({
  registry,
  traceId: 'trace-1',
  parentAgent: registry.agents[0],
  parentTaskId: 'root-1',
  runLedger,
  lifecycle,
  parentSignal: parentAbort.signal,
  async executeAgent({ signal }) {
...
assert.equal(lifecycle.get('delegation-1').state, 'cancelled');
assert.equal(ledgerEntries.at(-1).ok, false);
assert.equal(ledgerEntries.at(-1).error, 'DELEGATION_CANCELLED');

const successLifecycle = createAgentLifecycle({ maxConcurrent: 2 });
const successDelegator = createAgentDelegator({
  registry,
  traceId: 'trace-2',
  parentAgent: registry.agents[0],
  parentTaskId: 'root-2',
...
const succeeded = await successDelegator.delegate({
  agentId: 'specialist',
  task: 'test sonucu üret',
  successCriteria: ['test çalışsın'],
  constraints: ['kapsam dışına çıkma'],
  evidenceRequired: ['exit 0']
});
assert.equal(succeeded.ok, true);
assert.equal(succeeded.value.content, 'verified');
const successRun = successLifecycle.get('delegation-2');
assert.equal(successRun.state, 'completed');
