...
let sequence = 0;
// Fan-out is counted per ledger, so a case that targets another limit needs its own.
...
const runLedger = createLedgerStub();
...
let hold;
const limitedDelegator = createAgentDelegator({ registry, traceId: 'trace-00000003', parentAgent: registry.agents[0], parentTaskId: 'root-3', runLedger: createLedgerStub(), lifecycle: limited, async executeAgent() { await new Promise((resolve) => { hold = resolve; }); return { ok: true, content: 'held' }; } });
