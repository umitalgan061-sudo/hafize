...
let sequence = 0;
// Each agent run owns its ledger, exactly as createAgentRunLedger() is used in the
// runtime — the delegation budget is per run, so sharing one ledger across unrelated
// delegators would spend another run's budget.
function createFakeRunLedger() {
  const entries = [];
  return {
    entries,
    ...
    recordDelegationStart(agentId, { parentTaskId }) {
      ...
      entries.push({ action: 'agent.delegate', taskId, agentId, parentTaskId, status: 'running' });
      return { taskId };
    },
    recordDelegationFinish(taskId, result) {
      const entry = entries.find((item) => item.taskId === taskId);
      if (entry) entry.status = result?.ok ? 'completed' : 'failed';
      entries.push({ action: 'agent.delegate.finish', taskId, ...result });
    }
  };
}
const runLedger = createFakeRunLedger();
...
const successDelegator = createAgentDelegator({ registry, traceId: 'trace-2', parentAgent: registry.agents[0], parentTaskId: 'root-2', runLedger: createFakeRunLedger(), lifecycle: successLifecycle, async executeAgent() { return { ok: true, content: 'verified' }; } });
