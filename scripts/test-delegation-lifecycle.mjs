...
// The fan-out budget is counted per ledger, so every independent parent run
// needs its own ledger instance.
...
// The shared ledger has now spent its fan-out budget (maxParallelAgents: 3).
assert.deepEqual(
  await successDelegator.delegate({ agentId: 'specialist', task: 'over budget' }),
