...
let hold;
// Fan-out is counted from the run ledger, and the shared ledger above has
// already reached maxParallelAgents. Use a fresh ledger so this case actually
// exercises the lifecycle concurrency limit rather than tripping fan-out first.
let limitedSequence = 0;
const limitedEntries = [];
const limitedLedger = {
  snapshot() { return { entries: limitedEntries.slice() }; },
  recordDelegationStart(agentId, { parentTaskId }) { const taskId = `delegation-${++limitedSequence}`; limitedEntries.push({ action: 'agent.delegate', taskId, agentId, parentTaskId }); return { taskId }; },
