...
let sequence = 0;
// Fan-out bütçesi tek bir task ledger'daki tüm `agent.delegate` kayıtlarından
// hesaplanır (docs/NESTED_DELEGATION.md). Bu yüzden ledger'ı paylaşan senaryolar
// bütçeyi de paylaşır; lifecycle eşzamanlılığı ayrı bir ledger ile ölçülür.
function createTestLedger(prefix = 'delegation') {
  const entries = [];
  return {
    entries,
    snapshot() { return { entries: entries.slice() }; },
    recordDelegationStart(agentId, { parentTaskId }) { const taskId = `${prefix}-${++sequence}`; entries.push({ action: 'agent.delegate', taskId, agentId, parentTaskId }); return { taskId }; },
    recordDelegationFinish(taskId, result) { entries.push({ action: 'agent.delegate.finish', taskId, ...result }); }
  };
}
const runLedger = createTestLedger();
...
let hold;
const limitedDelegator = createAgentDelegator({ registry, traceId: 'trace-3', parentAgent: registry.agents[0], parentTaskId: 'root-3', runLedger: createTestLedger('limited'), lifecycle: limited, async executeAgent() { await new Promise((resolve) => { hold = resolve; }); return { ok: true, content: 'held' }; } });
