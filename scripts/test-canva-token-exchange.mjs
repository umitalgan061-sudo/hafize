    ...
    return {
      ...
      async json() {
        return {
          ...
          token_type: 'Bearer',
          expires_in: 14400,
          scope: 'asset:read design:meta:read asset:read'
        };
      }
    };
  }
...
assert.deepEqual(saves, [{
  ...
  tokenType: 'Bearer',
  scopes: ['asset:read', 'design:meta:read'],
...
const badResponses = [
  null,
  {},
  { access_token: 'a', refresh_token: 'r', token_type: 'MAC', expires_in: 14400 },
  { access_token: 'a', refresh_token: 'r', token_type: 'Bearer', expires_in: 1 },
  { access_token: 'a', refresh_token: '', token_type: 'Bearer', expires_in: 14400 },
  { access_token: 'a', refresh_token: 'r', token_type: 'Bearer', expires_in: 14400, scope: 'x'.repeat(5000) }
];
for (const payload of badResponses) {
  const bad = createCanvaTokenExchange({
    clientId: 'id', clientSecret: 'secret', tokenStore: { save: async () => {} },
...
const httpFailure = createCanvaTokenExchange({
  clientId: 'id', clientSecret: 'secret', tokenStore: { save: async () => {} },
