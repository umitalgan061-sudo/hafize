...
class FakeElement {
  ...
  matches(selector) {
    ...
    if (selector === '[data-github-draft-pr-create]') return this.getAttribute('data-github-draft-pr-create') !== null;
...
function fixture({ documentKeydownThrows = false } = {}) {
  ...
  return { documentRef, documentListeners, rootRef, body, card, dispatched };
}

function controls(f) {
  return {
    shell: f.card.querySelector('[data-github-draft-pr-create]'),
    inputs: f.card.findAll('.github-draft-pr-input'),
    approve: f.card.querySelector('.github-draft-pr-approve'),
    status: f.card.querySelector('.github-draft-pr-status')
  };
}
...

{
  ...
  const fetchImpl = async (path, options) => {
    ...
    return { ok: true, status: 200, json: async () => ({ receipt: { operation: 'pr.create', repository: api.REPOSITORY, prNumber: 395 } }) };
  };
  ...
  c.inputs[2].value = 'feat: lifecycle safe PR';
  ...
  assert.equal(calls[1].body.command.draft, true, 'execute preserves draft-only contract');
  assert.equal(c.status.dataset.state, 'success');
  assert.equal(c.inputs[0].value, '');
  assert.equal(c.inputs[2].value, '');
  assert.equal(f.dispatched.length, 1);
  assert.equal(f.dispatched[0].detail.prNumber, 395);
  assert.equal(controller.destroy(), true);
  assert.equal(controller.destroy(), false);
  assert.equal(f.card.querySelector('[data-github-draft-pr-create]'), null);
...

{
  ...
  second.destroy();
  assert.ok(f.card.querySelector('[data-github-draft-pr-create]'), 'second controller cannot remove first shell');
  first.destroy();
  ...
  assert.equal(third.mount(), true, 'destroy releases ownership for clean remount');
  third.destroy();
}

{
  ...
  assert.equal(f.card.querySelector('[data-github-draft-pr-create]'), null, 'partial shell rolled back');
  assert.equal((f.documentListeners.get('keydown') || []).length, 0);
  controller.destroy();
}
...

{
  ...
  c.inputs[1].value = 'main';
  c.inputs[2].value = 'feat: first';
  const first = controller.prepare();
  await Promise.resolve();
  c.inputs[0].value = 'second';
  c.inputs[2].value = 'feat: second';
...
console.log('github draft PR create controller lifecycle tests passed');
