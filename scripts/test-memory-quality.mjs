...
const records = [
  ...
  { memoryId: 'memory_beta12345', ownerId: 'u1', kind: 'decision', content: 'Projede küçük ve geri alınabilir PR tercih ediliyor.', sourceType: 'user_statement', sourceRef: null, createdAt: '2026-08-01T12:00:00.000Z', updatedAt: null },
...
// read() ranks the owner's records by relevance instead of hard-filtering on the query,
// so the match leads and other owners are never present at all.
assert.equal(read.ok, true); assert.equal(read.records.length, 2);
assert.ok(read.records.every((record) => record.ownerId === 'u1'));
