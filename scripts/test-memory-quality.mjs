...
assert.equal(store.write({ ownerId: 'u1', kind: 'preference', content: 'Kahve sütlü ve şekersiz.', sourceType: 'user_statement', sourceRef: null, sensitivity: 'personal', explicitUserIntent: true }).ok, true);
assert.equal(store.write({ ownerId: 'u1', kind: 'project', content: 'PR küçük tutulacak.', sourceType: 'user_statement', sourceRef: null, sensitivity: 'personal', explicitUserIntent: true }).ok, true);
assert.equal(store.write({ ownerId: 'u2', kind: 'preference', content: 'Kahve başka kullanıcıya ait.', sourceType: 'user_statement', sourceRef: null, sensitivity: 'personal', explicitUserIntent: true }).ok, true);
const read = store.read({ ownerId: 'u1', query: 'kahve', limit: 5 });
// read() is owner-scoped and relevance-ranked; it does not drop weaker matches.
