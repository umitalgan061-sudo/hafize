...
const registry = { agents: [{ id: 'hafize-general', name: 'Hafize' }, { id: 'agency-code-reviewer', name: 'Code Reviewer' }] };
let traceNo = 0;
const commands = createScheduleCommandBoundary({ store, registry, createTraceId: () => `trace-server-${++traceNo}` });
...
assert.deepEqual(await commands.create({ principal: alice, input: { agentId: 'missing-agent', task: 'Çalışmamalı.', runAt: '2026-08-13T06:00:00.000Z' } }), { ok: false, error: 'INVALID_AGENT' });

const created = await commands.create({ principal: alice, input: { agentId: 'hafize-general', task: 'Sabah özetini hazırla.', runAt: '2026-08-13T06:00:00.000Z', maxAttempts: 2 } });
...
const bobCreated = await commands.create({ principal: bob, input: { agentId: 'agency-code-reviewer', task: 'Repo özetini çıkar.', runAt: '2026-08-13T07:00:00.000Z' } });
assert.equal(bobCreated.ok, true);
store.add({ traceId: 'trace-internal', agentId: 'hafize-general', task: 'Internal task', runAt: '2026-08-13T08:00:00.000Z' });
...
assert.equal(aliceList.hasMore, false);
...
assert.deepEqual(await commands.cancel({ principal: alice, scheduleId: created.schedule.scheduleId }), { ok: false, error: 'SCHEDULE_NOT_CANCELLABLE' });

const bulkOne = await commands.create({ principal: alice, input: { agentId: 'hafize-general', task: 'bulk 1', runAt: '2026-08-13T10:00:00Z' } });
const bulkTwo = await commands.create({ principal: alice, input: { agentId: 'hafize-general', task: 'bulk 2', runAt: '2026-08-13T11:00:00Z' } });
const foreign = await commands.create({ principal: bob, input: { agentId: 'hafize-general', task: 'foreign', runAt: '2026-08-13T12:00:00Z' } });
const bulk = await commands.cancelMany({ principal: alice, scheduleIds: [bulkOne.schedule.scheduleId, bulkTwo.schedule.scheduleId, foreign.schedule.scheduleId, 'missing-id'] });
assert.equal(bulk.ok, true);
assert.equal(bulk.cancelled, 2);
assert.deepEqual(bulk.schedules.map((entry) => entry.scheduleId), [bulkOne.schedule.scheduleId, bulkTwo.schedule.scheduleId]);
assert.equal(store.read(foreign.schedule.scheduleId).status, 'scheduled');
assert.deepEqual(await commands.cancelMany({ principal: alice, scheduleIds: [] }), { ok: false, error: 'INVALID_SCHEDULE_COMMAND' });
...
assert.equal((await tiny.create({ principal: alice, input: { agentId: 'hafize-general', task: '1', runAt: '2026-08-13T00:00:00Z' } })).ok, true);
assert.deepEqual(await tiny.create({ principal: alice, input: { agentId: 'hafize-general', task: '2', runAt: '2026-08-13T01:00:00Z' } }), { ok: false, error: 'SCHEDULE_CAPACITY_REACHED' });

const asyncEntries = [];
let asyncNextId = 1;
const asyncStore = {
  async add(input) { const entry = { scheduleId: `schedule_${asyncNextId++}`, ...input, status: 'scheduled', attempts: 0, maxAttempts: input.maxAttempts ?? 1, lastError: null, createdAt: '2026-08-12T11:00:00.000Z', updatedAt: null }; asyncEntries.push(entry); return { ...entry }; },
...
const asyncCreated = await asyncCommands.create({ principal: alice, input: { agentId: 'hafize-general', task: 'Persisted schedule.', runAt: '2026-08-13T09:00:00.000Z' } });
