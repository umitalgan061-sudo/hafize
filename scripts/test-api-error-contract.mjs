...
// Without a request id the envelope simply omits the field; unknown codes collapse to INTERNAL_ERROR.
assert.deepEqual(mapToolFailureToApiError({ error: 'UPSTREAM', status: 504 }), { error: 'INTERNAL_ERROR', status: 504, message: '' });
assert.deepEqual(mapToolFailureToApiError({ error: 'UPSTREAM', status: 504 }, 'r2'), { error: 'INTERNAL_ERROR', status: 504, message: '', requestId: 'r2' });
