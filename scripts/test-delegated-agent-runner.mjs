...
const ledger = createAgentRunLedger({ traceId: 'trace-0000001', agentId: primary.id });
...
const invalidFirst = await runDelegatedAgent({
  ...
  traceId: 'trace-0000003',
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-0000003', agentId: primary.id }),
  model: 'mock-model',
  complete: async () => ({
    choices: [{
      finish_reason: 'tool_calls',
...
const noTools = await runDelegatedAgent({
  agent: reviewer,
  task: 'Kısa incele.',
  traceId: 'trace-0000002',
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-0000002', agentId: primary.id }),
  model: 'mock-model',
