...
const ledger = createAgentRunLedger({ traceId: 'trace-00000001', agentId: primary.id });
...
const invalidFirst = await runDelegatedAgent({
  ...
  traceId: 'trace-00000003',
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-00000003', agentId: primary.id }),
  model: 'mock-model',
  complete: async () => ({
    choices: [{
      finish_reason: 'tool_calls',
...
const noTools = await runDelegatedAgent({
  ...
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-00000002', agentId: primary.id }),
  model: 'mock-model',
