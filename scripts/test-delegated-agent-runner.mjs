...
const ledger = createAgentRunLedger({ traceId: 'trace-000001', agentId: primary.id });
...
const invalidFirst = await runDelegatedAgent({
  ...
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-000003', agentId: primary.id }),
  model: 'mock-model',
  complete: async () => ({
    choices: [{
      finish_reason: 'tool_calls',
...
const noTools = await runDelegatedAgent({
  ...
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-000002', agentId: primary.id }),
  model: 'mock-model',
