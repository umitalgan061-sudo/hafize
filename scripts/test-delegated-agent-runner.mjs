...
const ledger = createAgentRunLedger({ traceId: 'trace-runner-1', agentId: primary.id });
...
const invalidFirst = await runDelegatedAgent({
  ...
  traceId: 'trace-runner-3',
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-runner-3', agentId: primary.id }),
  model: 'mock-model',
  complete: async () => ({
    choices: [{
      finish_reason: 'tool_calls',
...
const noTools = await runDelegatedAgent({
  agent: reviewer,
  task: 'Kısa incele.',
  traceId: 'trace-runner-2',
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-runner-2', agentId: primary.id }),
  model: 'mock-model',
