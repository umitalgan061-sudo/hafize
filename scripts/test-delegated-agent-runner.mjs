...
const ledger = createAgentRunLedger({ traceId: 'trace-0001', agentId: primary.id });
...
const invalidFirst = await runDelegatedAgent({
  ...
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-0003', agentId: primary.id }),
  model: 'mock-model',
  complete: async () => ({
    choices: [{
      finish_reason: 'tool_calls',
...
const noTools = await runDelegatedAgent({
  ...
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-0002', agentId: primary.id }),
  model: 'mock-model',
