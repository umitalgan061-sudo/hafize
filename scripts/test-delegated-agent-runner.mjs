...
const ledger = createAgentRunLedger({ traceId: 'trace-run-0001', agentId: primary.id });
...
const invalidFirst = await runDelegatedAgent({
  ...
  traceId: 'trace-run-0003',
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-run-0003', agentId: primary.id }),
  model: 'mock-model',
  complete: async () => ({
    choices: [{
      finish_reason: 'tool_calls',
...
const noTools = await runDelegatedAgent({
  agent: reviewer,
  task: 'Kısa incele.',
  traceId: 'trace-run-0002',
  parentTaskId: 'task_parent',
  registry,
  runLedger: createAgentRunLedger({ traceId: 'trace-run-0002', agentId: primary.id }),
  model: 'mock-model',
