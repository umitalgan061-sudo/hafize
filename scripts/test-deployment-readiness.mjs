...
const unknown = evaluateDeploymentReadiness({
...
const invalid = evaluateDeploymentReadiness({
  runtime: { state: 'ready' },
  config: { state: 'impossible' },
  release: { state: 'ready' }
});
...
assert.throws(() => evaluateDeploymentReadiness({
  requiredComponents: ['runtime', 'runtime'],
  runtime: { state: 'ready' }
}), /DUPLICATE_DEPLOYMENT_COMPONENT/);
assert.throws(() => evaluateDeploymentReadiness({ requiredComponents: [''] }), /INVALID_DEPLOYMENT_COMPONENT_NAME/);

const optional = evaluateDeploymentReadiness({
  runtime: { state: 'ready' },
  config: { state: 'ready' },
  release: { state: 'ready' },
...
assert.equal(optional.releaseable, true);
assert.equal(optional.components.length, 1);

const bounded = evaluateDeploymentReadiness({
  runtime: { state: 'blocked', findings: [{ code: 'X', detail: 'a'.repeat(500) }] },
  config: { state: 'ready' },
  release: { state: 'ready' }
});
assert.equal(bounded.components[0].findings[0].detail.length, 160);

const noLeak = JSON.stringify(blocked);
assert.equal(noLeak.includes('token'), false);
