...
const confirmBrowser = await api.handle(
  { method: 'POST', pathname: '/api/device/reviews/review-123/confirm', body: { request: { action: 'browser.open', url: 'https://example.com/private?token=server-only' } } },
    ...
    async beginReview() {
      return {
        ...
        review: {
          ...
          action: 'browser.open',
          title: 'Onay',
          target: 'https://example.com/safe?token=leak',
          expiresAt: 1,
          requiresExplicitConfirmation: true
        }
      };
...
const unsafeSystemInfo = createDeviceActionHttpBoundary({
  runtime: {
    ...
    async executeReadOnly() {
      return {
        ok: true,
        value: { ok: true, action: 'system.info', info: { platform: 'linux', arch: 'x64', username: 'secret-user' } }
      };
    }
  }
});
