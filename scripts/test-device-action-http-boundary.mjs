...
const info = await api.handle({ method: 'POST', pathname: '/api/device/system-info', body: {} }, { agent, principal, traceId });
...
const confirmBrowser = await api.handle({ method: 'POST', pathname: '/api/device/reviews/review-123/confirm', body: { request: { action: 'browser.open', url: 'https://example.com/private?token=server-only' } } },
...
const confirmApp = await api.handle({ method: 'POST', pathname: '/api/device/reviews/review-123/confirm', body: { request: { action: 'app.open', appId: 'Browser.Chrome' } } }, { agent, principal, traceId });
...
const leakyRuntime = createDeviceActionHttpBoundary({ runtime: { ...runtime,
  async beginReview() { return { ok: true, review: { id: 'review-123', action: 'browser.open', title: 'Onay', target: 'https://example.com/safe', expiresAt: 1, requiresExplicitConfirmation: true, approvalToken: 'must-not-pass' } }; } } });
...
const unsafeRuntimeReview = createDeviceActionHttpBoundary({ runtime: { ...runtime,
  async beginReview() { return { ok: true, review: { id: 'review-123', action: 'browser.open', title: 'Onay', target: 'https://example.com/safe?token=leak', expiresAt: 1, requiresExplicitConfirmation: true } }; } } });
...
const unsafeSystemInfo = createDeviceActionHttpBoundary({ runtime: { ...runtime,
  async executeReadOnly() { return { ok: true, value: { ok: true, action: 'system.info', info: { platform: 'linux', arch: 'x64', username: 'secret-user' } } }; } } });
