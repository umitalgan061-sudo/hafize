...
const command = { operation: 'branch.create', repository: 'umitalgan061-sudo/hafize', branch: 'hafize/opt-in-e2e', baseRef: 'main' };
let requestBody = { command };
const writes = [];
const network = [];
const fetchImpl = async (url, init = {}) => {
  network.push({ url: String(url), method: init.method || 'GET', authorization: init.headers?.Authorization });
  if (String(url).endsWith('/git/ref/heads/main')) return { ok: true, async json() { return { object: { sha: 'a'.repeat(40) } }; } };
  if (String(url).endsWith('/git/refs')) return { ok: true, async json() { return { object: { sha: 'b'.repeat(40) } }; } };
...
requestBody = { command };
const unauthorized = await enabled.handle({ request: {}, response: new Response(), method: 'POST', pathname: '/api/github/write/prepare', headers: {} });
