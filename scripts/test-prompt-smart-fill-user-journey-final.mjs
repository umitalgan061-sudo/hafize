...
const journey = [
  ['open', /function mount\(/],
  ['variable detection', /export function variableNames\(/],
  ['field rendering', /activeInputs = new Map/],
  ['live preview', /const renderPreview = \(\): void =>/],
  ['preset load', /const found = readPresets\(activePrompt\.id\)\.find\(/],
  ['preset save', /save\.addEventListener\('click'/],
  ['copy', /copy\.addEventListener\('click', async \(\) =>/],
  ['insert', /const insertIntoComposer = \(\): void =>/],
...
assert.match(source, /event\.preventDefault\(\);\s*event\.stopImmediatePropagation\(\);\s*openFor\(/);
