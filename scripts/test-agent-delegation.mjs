...
const toolSuccess = await executeNvidiaToolCall(
  primary,
  { id: 'call-delegate-1', function: { name: 'agent_delegate', arguments: JSON.stringify({ agentId: reviewer.id, task: 'Kontrol et.' }) } },
...
const toolFailure = await executeNvidiaToolCall(
  primary,
  { id: 'call-delegate-2', function: { name: 'agent_delegate', arguments: JSON.stringify({ agentId: reviewer.id, task: 'Kontrol et.' }) } },
