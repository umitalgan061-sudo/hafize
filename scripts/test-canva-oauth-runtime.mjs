...
const starts = [];
const flowRuntime = {
  async start(input) { starts.push(input); return { authorizationUrl: 'https://www.canva.com/api/oauth/authorize?x=1', state: 'state-1' }; },
  async finish() { return { ok: true, provider: 'canva', code: 'code-1', verifier: 'server-only', redirectUri: 'https://hafize.example/oauth/canva', scopes: ['profile:read'] }; }
};
const runtime = createCanvaOAuthRuntime({ clientId: 'client-1', redirectUri: 'https://hafize.example/oauth/canva', flowRuntime });
const started = await runtime.start({ capabilities: ['profile.read'] });
...
const mismatch = createCanvaOAuthRuntime({ clientId: 'x', redirectUri: 'https://hafize.example/callback', flowRuntime: { start: flowRuntime.start, finish: async () => ({ provider: 'google' }) } });
await assert.rejects(() => mismatch.finish({}), /CANVA_OAUTH_FLOW_PROVIDER_MISMATCH/);
