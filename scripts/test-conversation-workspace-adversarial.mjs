...
const securityAssertions = [
  ...
  ['storage.getItem(STORAGE_KEY)', 'history read stays on the storage boundary'],
...
const modelChecks = [
  ...
  [/constants:\s*Object\.freeze\(\{[\s\S]*?MAX_CONVERSATIONS[\s\S]*?\}\)/, 'public constants report conversation cap']
];
