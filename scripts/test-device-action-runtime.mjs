...
const crossOwner = await runtime.confirmAndExecute(hafize, browserRequest, { reviewId: browserReview.review.id, traceId, principal: ownerB });
assert.deepEqual(crossOwner, { ok: false, error: 'DEVICE_REVIEW_OWNER_MISMATCH' });
assert.equal(runtime.activeReviewCount('owner-a'), 1);
assert.equal(calls.length, 0);

const browserResult = await runtime.confirmAndExecute(hafize, browserRequest, { reviewId: browserReview.review.id, traceId, principal: ownerA });
...
const replay = await runtime.confirmAndExecute(hafize, browserRequest, { reviewId: browserReview.review.id, traceId, principal: ownerA });
...
const wrongTarget = await runtime.confirmAndExecute(hafize, { action: 'app.open', appId: 'editor.vscode' }, { reviewId: appReview.review.id, traceId, principal: ownerA });
...
const wrongTrace = await runtime.confirmAndExecute(hafize, { action: 'app.open', appId: 'browser.chrome' }, { reviewId: wrongTraceReview.review.id, traceId: 'trace-device-runtime-999', principal: ownerA });
...
const auditFailRuntime = createDeviceActionRuntime({ deviceBridge: bridge, ownerResolver, approvalStore: auditFailApproval, reviewStore: auditFailReviews, auditSink: async () => { throw new Error('sink down'); } });
...
const blockedExecution = await confirmAuditRuntime.confirmAndExecute(hafize, { action: 'app.open', appId: 'browser.chrome' }, { reviewId: prepared.review.id, traceId, principal: ownerA });
