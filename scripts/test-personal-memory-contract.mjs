...
assert.deepEqual(normalizeMemoryWrite(validWrite),
...
for (const kind of ['identity', 'preference', 'project', 'note']) assert.equal(normalizeMemoryWrite({ ownerId: 'owner', kind, content: `kind:${kind}`, sourceType: 'user_import', sensitivity: 'personal', explicitUserIntent: true }).ok,
  true);
for (const invalidKind of ['', 'credential', 'other']) assert.deepEqual(normalizeMemoryWrite({ ownerId: 'owner', kind: invalidKind, content: 'x', sourceType: 'user_note', sensitivity: 'personal', explicitUserIntent: true }),
