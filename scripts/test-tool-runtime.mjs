...
const safeGmailActivity = JSON.stringify(getPublicToolActivity('gmail_read', {
  ok: true,
  value: { subject: 'Banka şifre sıfırlama', snippet: 'kod: 123456', from: 'private@example.com' }
}));
assert.equal(safeGmailActivity.includes('Banka'), false);
assert.equal(safeGmailActivity.includes('123456'), false);
assert.equal(safeGmailActivity.includes('private@example.com'), false);
assert.equal(safeGmailActivity.includes('"state":"success"'), true);
