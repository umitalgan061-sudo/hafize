...
// Delegation is only offered when a delegate executor is actually wired in, so
// permission alone does not put agent_delegate in front of the model.
...
assert.deepEqual(
  getAllowedNvidiaTools(hafize, { githubReadConfigured: true, delegateAgent: async () => ({ ok: true, value: {} }) }).map((tool) => tool.function.name),
