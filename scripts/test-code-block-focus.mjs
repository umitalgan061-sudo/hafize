...
assert.match(loader, /HafizeCodeBlockFocus/); assert.match(loader, /\/code-block-focus\.js/); assert.match(sw, /CURRENT_CACHE = `\$\{CACHE_PREFIX\}v\d+`/); assert.match(sw, /'\/code-block-focus\.js'/); assert.match(sw, /pathname\.startsWith\('\/api\/'\).*network-only/s);
