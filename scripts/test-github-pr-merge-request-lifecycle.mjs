...

{
  ...
  await assert.rejects(pending, (error) => {
    ...
    assert.equal(error.code, 'GITHUB_PR_MERGE_TIMEOUT');
    return true;
  });
...

{
  ...
  outer.abort('closed');
  await assert.rejects(pending, (error) => {
    assert.equal(error.code, 'GITHUB_PR_MERGE_CANCELLED');
    return true;
  });
  assert.equal(capture.options.signal.aborted, true);
...

{
  ...
  await assert.rejects(
    ...
    (error) => error?.code === 'GITHUB_PR_MERGE_CANCELLED'
  );
...

{
  ...
  const result = await client.post(api.PREPARE_PATH, { command: { operation: 'pr.merge' } });
  assert.equal(result.response.ok, true);
  assert.deepEqual(result.payload, { ok: true });
  assert.deepEqual(capturedBody, { command: { operation: 'pr.merge' } });
...

{
  ...
  await assert.rejects(client.post('/api/not-allowed', {}), /INVALID_GITHUB_PR_MERGE_PATH/);
  await assert.rejects(client.post(api.PREPARE_PATH, {}, { signal: 'bad' }), /INVALID_GITHUB_PR_MERGE_SIGNAL/);
  assert.throws(() => api.createClient({ fetchImpl: okFetch, timeoutMs: 0 }), /INVALID_GITHUB_PR_MERGE_TIMEOUT/);
  assert.throws(() => api.createClient({ fetchImpl: okFetch, timeoutMs: 120_001 }), /INVALID_GITHUB_PR_MERGE_TIMEOUT/);
  assert.throws(() => api.createClient({ fetchImpl: okFetch, AbortControllerImpl: null }), /INVALID_GITHUB_PR_MERGE_ABORT_CONTROLLER/);
}

console.log('github PR merge request lifecycle tests passed');
