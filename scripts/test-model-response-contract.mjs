...
assert.throws(
  () => normalizeNvidiaChatCompletion({ choices: [{ message: { role: 'assistant', tool_calls: [{ id: 'x', function: { name: 'x', arguments: 'x'.repeat(MODEL_RESPONSE_CONTRACT.maxToolArgumentLength + 1) } }] } }] }),
  /INVALID_MODEL_TOOL_CALL/
);
assert.throws(
  () => normalizeNvidiaChatCompletion({ choices: [{ message: { role: 'assistant', tool_calls: Array.from({ length: MODEL_RESPONSE_CONTRACT.maxToolCalls + 1 }, () => ({ id: 'x', function: { name: 'x', arguments: '{}' } })) } }] }),
  /INVALID_MODEL_TOOL_CALLS/
);
