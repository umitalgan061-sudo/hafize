...
assert.deepEqual(
  normalizeConsolidationApproval({ ownerId: 'owner_1', plan: tampered, approvedGroupIds: ['g1'], explicitUserIntent: true }),
  { ok: false, error: 'MEMORY_CONSOLIDATION_KEEP_RECORD_PROTECTED' }
);

const approval = normalizeConsolidationApproval({ ownerId: 'owner_1', plan, approvedGroupIds: [groupId], explicitUserIntent: true });
assert.deepEqual(approval, { ok: true, commands: [{ ownerId: 'owner_1', memoryId: 'memory_old00001', exactMatch: true }] });
