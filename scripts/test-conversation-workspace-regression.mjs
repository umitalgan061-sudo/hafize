...
for (const selector of coexistenceSelectors) {
  if (selector.startsWith('.conversation-workspace') || selector === '.workspace-row-check') {
    ...
    // Modules may name the class with or without the leading dot.
    const combined = historyManagement + historySearch + historyExport;
    assert.ok(
      combined.includes(selector) || combined.includes(`'${selector.slice(1)}'`),
      `existing module still mentions ${selector}`
    );
  }
}
...
for (const key of storageKeys) {
  const consumers = [workspace, drafts, historyManagement, historySearch, historyExport, settingsWorkspace, uiShell].filter((text) => text.includes(key));
...
for (const fragment of accessibilityFragments) {
  // Attributes are applied with setAttribute(), not written as markup.
  const parsed = /^([a-z-]+)="(.*)"$/s.exec(fragment);
  assert.ok(parsed, `attribute fragment must be name="value": ${fragment}`);
  const applied = `setAttribute('${parsed[1]}', '${parsed[2]}')`;
  assert.ok(
    workspace.includes(fragment) || workspace.includes(applied),
    `accessibility retained: ${fragment}`
  );
}
...
const existingBehaviorFragments = [
  ...
  [historySearch, "searchUi.input.focus()", 'history search shortcut boundary retained'],
