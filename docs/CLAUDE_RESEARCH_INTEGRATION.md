...
İlk uygulama `lib/skill-manifest.mjs` ve `lib/skill-registry.mjs` ile başlatıldı; sözleşme `docs/SKILLS_REGISTRY.md` içinde. Sıradaki adım bu katmanın server akışına ve tool runtime'a bağlanmasıdır.
