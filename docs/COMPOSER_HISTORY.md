...
- Geçmiş modu kapalıyken yazı alanında imleç en baştaysa `ArrowUp`, aktif sohbetteki son kullanıcı mesajıyla geçmiş modunu başlatır.
- Geçmiş modu açıkken ardışık `ArrowUp` / `ArrowDown`, geri çağrılan metindeki caret konumundan bağımsız olarak eski/yeni kayıtlar arasında gezinir.
- En yeni kaydın ötesindeki `ArrowDown`, gezinme başlamadan önceki taslağı aynen geri yükler ve geçmiş modundan çıkar.
...
- `ArrowLeft`, `ArrowRight`, `Home`, `End`, `PageUp` veya `PageDown` history modunu kapatır ve normal caret hareketine bırakılır.
...
- Geçmiş moduna ilk girişte `ArrowUp` yalnız selection çökmüşken ve caret `0` konumundayken yakalanır; çok satırlı metinde normal yukarı hareket aksi halde bozulmaz.
- Geçmiş modu başladıktan sonra yalnız `ArrowUp` / `ArrowDown` geçmiş gezinmesine ayrılır; caret taşıyan diğer gezinme tuşları modu sonlandırır.
