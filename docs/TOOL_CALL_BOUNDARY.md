...
Boundary'nin kendi reddi de kod taşır: eksik çağrı kimliği `INVALID_TOOL_CALL_ID`, geçersiz veya bozuk JSON gövde `INVALID_TOOL_ARGUMENTS` olarak döner. Model'in ürettiği bozuk JSON istemcinin isteği sayılmaz; parser metni sınırın dışına çıkmaz. Bu red nedenleri jenerik `TOOL_EXECUTION_FAILED` altında gizlenmez; provider detayı yine dışarı çıkmaz.
