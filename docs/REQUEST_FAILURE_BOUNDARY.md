...
Invalid or successful upstream statuses are not accepted as public failure status codes; the fallback is 502. Upstream provider text never reaches the client: JSON and streamed failures both carry only the stable public error code. `REQUEST_FAILURE_CONTRACT.maxNvidiaDetail` is the bound `server.mjs` applies when it captures upstream text for internal diagnostics.
