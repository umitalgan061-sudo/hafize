...
Invalid or successful upstream statuses are not accepted as public failure status codes; the fallback is 502. Provider diagnostic text is never returned to the client on any path: JSON and streamed failures both carry only the stable public error code. `REQUEST_FAILURE_CONTRACT.maxNvidiaDetail` stays as the bound for server-side diagnostics.
