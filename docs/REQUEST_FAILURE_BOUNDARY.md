...
Invalid or successful upstream statuses are not accepted as public failure status codes; the fallback is 502. Provider diagnostic text never reaches the client: JSON and streamed failures both carry only the stable public error code, and the bounded 1200 character detail stays on the error object for server-side diagnostics.
