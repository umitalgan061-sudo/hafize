...
Invalid or successful upstream statuses are not accepted as public failure status codes; the fallback is 502. Upstream provider diagnostic text is never copied into a public failure body — JSON and streamed failures both carry only the stable public error code — so provider bodies stay server-side for logging.
