...
Invalid or successful upstream statuses are not accepted as public failure status codes; the fallback is 502. Provider diagnostic text never reaches the client: the server bounds it to 1200 characters for its own logs, while both JSON and streamed failures carry only the stable public error code.
