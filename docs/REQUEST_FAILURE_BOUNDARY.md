...
Invalid or successful upstream statuses are not accepted as public failure status codes; the fallback is 502. Upstream provider diagnostic text is never echoed to the client on any path: JSON and streamed failures both carry only the stable public error code, because provider text can contain account, quota or routing detail. `REQUEST_FAILURE_CONTRACT.providerDetailExposed` records that guarantee.
