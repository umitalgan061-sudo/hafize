...
- `lease == null`: `executor.executeAgentTask` worker sözleşmesine yansıtan bir sarmalayıcıyla kullanılır ve `leaseGuarded: false` döner.
- `lease` verilmişse: aynı sarmalayıcı `createScheduleLeaseGuardedExecutor` ile sarılır ve `leaseGuarded: true` döner.

Her iki modda da worker'a yalnız `ok`, `error` ve `retryAt` alanları ulaşır; `content`,
`taskLedger`, `leaseStatus` ve `deduplicated` gibi iç metadata worker yüzeyine çıkmaz.
