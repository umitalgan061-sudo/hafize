...
`read()` bir sorgu ile çağrıldığında lexical eşleşmesi olmayan kayıtlar sonuçtan düşürülür; sıralama katmanı tek başına filtre değildir. Böylece belirli bir sorgu, aynı kullanıcıya ait ilgisiz kişisel kayıtları model bağlamına taşımaz. Sorgu boş olduğunda kullanıcının kayıtları sıralanarak listelenmeye devam eder.
